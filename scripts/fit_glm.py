...
if __name__=="__main__":
    ...
    licks_vec, timestamps = np.histogram(reward_times, bins=dff_trace_timestamps)
    ...
    to_drop.append([
        'image_{}'.format(i) for i in range(8)
    ])
    ...
    for ind_model, model_label in enumerate(drop_labels):
        ...
        for ind_test_split, test_split in tqdm(enumerate(splits), total=len(splits),
        ...
        all_W.update({model_label:all_W_this_model.mean(axis=2).tolist()})

    #  all_cells = session.['cell_specimen_id'].unique()
    all_cell_ids = dff_trace_arr.coords['cell_specimen_id'].values
    all_W.update({'cell_specimen_id':all_cell_ids.tolist()})
    ...
    results_dict = df_result.to_dict()
    output_dict = {'var_explained':results_dict}
    ...
    output_dict.update({'all_W': all_W})
