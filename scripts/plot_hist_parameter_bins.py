# Make plots that demonstrate how different glm coefficients covary.
...
def plot_hist_parameter_bins(df, parameter_primary, parameters_cont, parameters_categ, norm_by_param_primary=0, dosavefig_fign=[0,'']):
    ...
    if r>0: # e.g false-alarms might be all 0; so we cant have even 2 bins.
    ...
    #     print(np.unique(hist_inds))
        ...
        ncols = 7 # max(len(parameters_cont), len(parameters_categ))
        nrows_cont = int(np.ceil(len(parameters_cont)/ncols))

        # nbins0+1; 1: for categorical params
        fig,ax = plt.subplots(nrows_cont+1, ncols,figsize=(20, 7*nrows_cont))
...
#                 print(hind, col, len(topc))
        ...
        #             print(bn)
                    ...
                    ###### plot the histogram
                    row = hind - 1
                    row_ax = int(np.floor(col/ncols)) #0 # max(0, int(np.ceil(col/5)-1)) # 0
                    col_ax = int(np.remainder(col, ncols))
        #             print(col, row_ax)

                    # plot the center of bins
                    ax[row_ax, col_ax].bar(bin_edges[0:-1]+binEvery/2., hist, color=cols[row], label=f'{hist_bins_mean[row]}', alpha=.35, width=r/nbins)  # col
                    ...
                    ax[row_ax, col_ax].set_xlim([np.min(df[parameter])-mnx, np.max(df[parameter])+mnx])

                    ax[row_ax, col_ax].set_xlabel(f'{parameter}')
        #             ax[row_ax, col].set_title(f'bins of {parameter_primary}')
                    ...
                    if col==0:
                        ax[row_ax, col_ax].set_ylabel('Fraction cells')
    ...
    #             print(hind,parameter)
                ...
                row_ax = -1 #1 #max(np.ceil((0+len(parameters_cont))/5), int(np.ceil((col+len(parameters_cont))/5)-1)) # 1 # 2
                col_ax = int(np.remainder(col, ncols))
    #             print(col, row_ax)
    ...
    #             print(x0, xlabs)
                ax[row_ax, col_ax].set_xticks(x0)
                ax[row_ax, col_ax].set_xticklabels(xlabs, rotation=45, fontsize=11)
                ax[row_ax, col_ax].set_xlabel(f'{parameter}')
    #             ax[row_ax, col].set_title(f'bins of {parameter_primary}')
    ...
    #             ax[row_ax, col].set_title('parameter = {}\n{}'.format(parameter, parameter_primary))
        ...
        ### plot a histogram of the primary parameter on the last subplot
    #     plt.figure(figsize=(2,2))
    #     print(bn0)
        ax[row_ax, -1].bar(hist_bins_mean, hist_vals, width=.1) # bn0 + binEvery0/2.
    #     ax[row_ax, -1].set_title(f'{hist_vals} : number of cells per bin of {parameter_primary}')
        ...
        plt.suptitle(f'bins of {parameter_primary}', y=.9, fontsize=15)
    ...
    #         fign = os.path.join(dir0, dir_now, nam+fmt)     
        ...
        print(f'Note: no histogram will be plotted, because there is only 1 single value in {parameter_primary}')
