...
def fraction_same(across_df):
    ...
    for dropout in dropouts:
        across_df[dropout + '_same'] = across_df[dropout +
                                                 '_within'] == across_df[dropout + '_across']
...
def plot_dropout(across_df, dropout, ax):
    ...
    for elevel in experience_levels:
        ...
        ax.plot(-eacross_df[dropout + '_within'], -
                eacross_df[dropout + '_across'], 'o', color=colors[elevel])
...
def load_cells(glm_version, clean_df=True, fold=None, save_output=False):
        ...
        try:
            ...
            if clean_df:
                columns = [
                    'ophys_experiment_id',
                    'cell_specimen_id'] + [
                    x for x in score_df.columns if (
                        '_within' in x) or (
                        '_across' in x)]
                score_df = score_df[columns]
            else:
                score_df = score_df.drop(
                    columns=['fit_index'], errors='ignore')
            dfs.append(score_df)
    ...
    across_df['identifier'] = [
        str(x) +
        '_' +
        str(y) for (
            x,
            y) in zip(
            across_df['ophys_experiment_id'],
            across_df['cell_specimen_id'])]
    cells_table['identifier'] = [
        str(x) +
        '_' +
        str(y) for (
            x,
            y) in zip(
            cells_table['ophys_experiment_id'],
            cells_table['cell_specimen_id'])]
    across_df = pd.merge(
        across_df.drop(
            columns=[
                'ophys_experiment_id',
    ...
    for kernel in kernels:
        assert np.all(across_df[kernel + '_across'] <=
                      0), "Dropout scores must be negative"

    # Assert that across session dropout scores are less than equal to within cells
    # need to use a numerical tolerance because we divide by number of
    # timesteps
    tol = 1e-8
    for kernel in kernels:
        assert np.all(across_df[kernel + '_within'] <= across_df[kernel + '_across'] +
                      tol), 'Across session dropouts must be less than or equal to within session dropouts'
    ...
    assert len(across_df) + len(fail_df) == len(cells) * \
        3, "incorrect number of cells"
    assert len(across_df['cell_specimen_id'].unique(
    )) + len(fail_df['cell_specimen_id'].unique()) == len(cells), "incorrect number of cells"
...
def across_session_normalization(cell_specimen_id, glm_version, do_folds=True):
    ...
    if not do_folds:
        score_df = compute_across_session_dropouts(
            data, run_params, cell_specimen_id)
        filename = '/allen/programs/braintv/workgroups/nc-ophys/visual_behavior/ophys_glm/v_'\
    ...
    if do_folds:
        for fold in range(0, 5):
            score_df = compute_cv_across_session_dropouts(
                data, run_params, cell_specimen_id, fold)
            filename = '/allen/programs/braintv/workgroups/nc-ophys/visual_behavior/ophys_glm/v_'\
...
def get_across_session_data(run_params, cell_specimen_id):
    ...
    cells_table = cells_table.query('not passive').copy()
    cells_table = cells_table[cells_table['cell_specimen_id']
                              == cell_specimen_id]
    cells_table = cells_table.query(
        'last_familiar_active or first_novel or second_novel_active')
...
def compute_across_session_dropouts(data, run_params, cell_specimen_id):
    ...
    for oeid in score_df.index.values:
        ...
        results_df = gft.build_dataframe_from_dropouts(
            data[str(oeid) + '_fit'], run_params)

        # Iterate over dropouts
        for dropout in dropouts:
            score_df.at[oeid, dropout] = results_df.loc[cell_specimen_id][dropout +
                                                                          '__avg_cv_adjvar_test']
            score_df.at[oeid, dropout +
                        '_fc'] = results_df.loc[cell_specimen_id][dropout +
                                                                  '__avg_cv_adjvar_test_full_comparison']
            score_df.at[oeid, dropout +
                        '_within'] = results_df.loc[cell_specimen_id][dropout +
                                                                      '__adj_dropout']

            # Get number of timestamps each kernel was active
            dropped_kernels = set(
                run_params['dropouts'][dropout]['dropped_kernels'])
            design_kernels = set(
                data[str(oeid) + '_design'].kernel_dict.keys())
            X = data[str(
                oeid) + '_design'].get_X(kernels=design_kernels.intersection(dropped_kernels))
            score_df.at[oeid, dropout +
                        '_timestamps'] = np.sum(np.sum(np.abs(X.values), axis=1) > 0)

    # Iterate over dropouts and compute coding scores
    for dropout in dropouts:

        # Adjust variance explained based on number of timestamps
        score_df[dropout + '_pt'] = score_df[dropout] / \
            score_df[dropout + '_timestamps']
        score_df[dropout + '_fc_pt'] = score_df[dropout + '_fc'] / \
            score_df[dropout + '_timestamps']
        ...
        score_df[dropout + '_across'] = -(score_df[dropout + '_fc_pt'] -
                                          score_df[dropout + '_pt']) / (score_df[dropout + '_max'])
        score_df.loc[score_df[dropout + '_across']
                     > 0, dropout + '_across'] = 0

        # Cleaning step for low VE dropouts
        score_df.loc[score_df[dropout + '_within']
                     == 0, dropout + '_across'] = 0
...
def compare_across_df(across_df, dropout):
    ...
    '''
    plt.figure()
    plt.plot(np.sort(across_df[dropout + '_within'])
             [::-1], 'bo-', label='within')
    plt.plot(np.sort(across_df[dropout + '_across'])
             [::-1], 'rx-', label='across')
...
def append_kernel_excitation_across(weights_df, across_df):
    ...
    for kernel in excited_kernels:
        ...
        across_df.loc[across_df[kernel + '_excited']
                      != True, kernel + '_across_positive'] = 0
        across_df.loc[across_df[kernel + '_excited'],
                      kernel + '_across_negative'] = 0
        across_df[kernel + '_across_signed'] = across_df[kernel +
                                                         '_across_positive'] - across_df[kernel + '_across_negative']

    for kernel in excited_kernels:
        assert np.all(across_df[kernel + '_across_positive']
                      <= 0), "Dropout scores must be negative"
        assert np.all(across_df[kernel + '_across_negative']
                      <= 0), "Dropout scores must be negative"
    return across_df
...
def build_cv_across_dataframe_from_dropouts(fit, run_params, fold):
    ...
    for model_label in fit['dropouts'].keys():
        ...
        results[model_label +
                "__avg_cv_var_train"] = fit['dropouts'][model_label]['cv_var_train'][:, fold]
        if compute_with_infs:
            results[model_label +
                    "__avg_cv_var_train_raw"] = fit['dropouts'][model_label]['cv_var_train'][:, fold]
            results[model_label +
                    "__avg_cv_var_test_raw"] = fit['dropouts'][model_label]['cv_var_test'][:, fold]
            results[model_label +
                    "__avg_cv_var_test_full_comparison_raw"] = fit['dropouts']['Full']['cv_var_test'][:, fold]
        ...
        results[model_label +
                "__avg_cv_var_test_full_comparison"] = full_temp[:, fold]
        ...
        results[model_label +
                "__avg_cv_adjvar_train"] = fit['dropouts'][model_label]['cv_adjvar_train'][:, fold]
        if compute_with_infs:
            results[model_label +
                    "__avg_cv_adjvar_test_raw"] = fit['dropouts'][model_label]['cv_adjvar_test'][:, fold]
        ...
        full_temp[np.isinf(full_temp)] = 0
        results[model_label +
                "__avg_cv_adjvar_test_full_comparison"] = full_temp[:, fold]
...
def compute_cv_across_session_dropouts(
        data, run_params, cell_specimen_id, fold):
    '''
        Computes the across session dropout scores
        data                a dictionary containing the session object, fit dictionary,
    ...
    for oeid in score_df.index.values:
        # Get the full comparison values and test values
        results_df = build_cv_across_dataframe_from_dropouts(
            data[str(oeid) + '_fit'], run_params, fold)
