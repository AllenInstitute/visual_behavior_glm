...
def load_fit_pkl(run_params, ophys_experiment_id):
    ...
    filenamepkl = os.path.join(
        ...
        str(ophys_experiment_id) + '.pkl')
    filenamepbz2 = os.path.join(
        run_params['experiment_output_dir'],
        str(ophys_experiment_id) + '.pbz2')
    print(filenamepbz2)
...
def log_error(error_dict, keys_to_check=[]):
    ...
    db.update_or_create(
        ...
        # keys to check to determine whether an entry already exists.
        # Overwrites if an entry is found with matching keys
    )
    conn.close()
...
def build_kernel_df(glm, cell_specimen_id):
    ...
    for ii, kernel_name in enumerate(kernel_list):
        ...
        # get the weight matrix for the weights associated with this kernel and
        # cell (dims = 1 x n_weights)
        kernel_weight_names = [
            w for w in glm.W.weights.values if w.startswith(kernel_name)]
        w_kernel = np.expand_dims(glm.W.loc[dict(
            weights=kernel_weight_names, cell_specimen_id=cell_specimen_id)], axis=0)

        # Version 19 had an edge case where the design matrix gets re-loaded
        # with an extra weight
        if kernel_name.startswith('image') & (
                np.shape(kernel)[0] != np.shape(w_kernel)[1]):
            print('Hack, need to fix bug')
            kernel = kernel[0:-1, :]

        # calculate kernel output as w_kernel * kernel (dims = 1 x n_timestamps)
        # add to list of dataframes with cols: timestamps, kernel_outputs,
        # kernel_name
        kernel_df.append(
            pd.DataFrame({
    ...
    # return the concatenated dataframe (concatenating a list of dataframes
    # makes a single dataframe)
...
def generate_results_summary_adj(glm):
    ...
    '''
    # Get list of columns to look at, removing the non-adjusted dropouts, and
    # training scores
    ...
    for cell_specimen_id in glm.results.index.values:
        ...
        results_summary = pd.DataFrame(
            glm.results.loc[cell_specimen_id][test_cols]).reset_index().rename(
            columns={
                cell_specimen_id: 'variance_explained',
                'index': 'dropout_name'})

        # For each dropout, separate the name of the dropout from the type of
        # information
        ...
        results_summary = pd.pivot_table(
            results_summary.drop(
                columns=['dropout_name']),
            index=['dropout'],
            columns=['type'],
            values=['variance_explained'],
            dropna=False)
        results_summary.columns = results_summary.columns.droplevel()
        results_summary = results_summary.rename(
            columns={
...
def generate_results_summary_nonadj(glm):
    ...
    # training scores
    test_cols = [
        col for col in glm.results.columns if (
            (not col.endswith('train')) & (
                'adj' not in col) & (
                'session' not in col) & (
                    'cell' not in col))]
...
def generate_results_summary_non_cleaned(glm):
    '''
        Returns a dataframe with summary information from the glm object
    '''
    # Preserving the old functionality for now, but filtering out the adjusted
    # variance columns
    test_cols = [
        col for col in glm.results.columns if (
            col.endswith('test') & (
    ...
    for cell_specimen_id in glm.results.index.values:
        ...
        def calculate_fractional_change(row):
            ...
            return (row['variance_explained'] -
                    full_model_performance) / full_model_performance
...
def log_results_to_mongo(glm):
    ...
    for df, collection in zip([full_results, results_summary], [
                              'results_full', 'results_summary']):
...
def get_weights_matrix_from_mongo(ophys_experiment_id, glm_version):
    ...
    if w_matrix_lookup_table.count_documents(lookup_table_document) == 0:
        warnings.warn(
            'there is no record of a the weights matrix for oeid {}, glm_version {}'.format(
...
def log_weights_matrix_to_mongo(glm):
    ...
    if w_matrix_lookup_table.count_documents(lookup_table_document) >= 1:
        # if weights matrix for this experiment/version has already been
        # logged, we need to replace it
        lookup_result = list(
            w_matrix_lookup_table.find(lookup_table_document))[0]
...
def get_experiment_table(glm_version, include_4x2_data=False):
    ...
    # merge in dropout count (average dropout count per experiment - should be
    # same for all cells)
    experiment_table_merged = experiment_table_merged.merge(
        pd.DataFrame(
            dropout_count.groupby('ophys_experiment_id')['dropout_count'].mean()).reset_index(),
        left_on='ophys_experiment_id',
        right_on='ophys_experiment_id',
        how='left')
...
def retrieve_results(
        ...
        results_type='full',
        return_list=None,
        merge_in_experiment_metadata=True,
        remove_invalid_rois=True,
        verbose=False,
        allow_old_rois=True,
        invalid_only=False,
    ...
    '''
    assert not (
        invalid_only & remove_invalid_rois), "Cannot remove invalid rois and only return invalid rois"
    ...
    conn = db.Database('visual_behavior_data')
    database = 'ophys_glm'
    results = pd.DataFrame(list(conn[database]['results_{}'.format(
        results_type)].find(search_dict, return_dict)))
    ...
    if remove_invalid_rois:
        ...
        if 'cell_roi_id' in results:
            ...
            raise Exception(
                'cell_roi_id not in database, and allow_old_rois=False')
    elif invalid_only:
        if verbose:
    ...
    if ('variance_explained' in results) and (
            np.sum(results['variance_explained'].isnull()) > 0):
...
def get_glm_version_summary(
    ...
    if compact:
        dropouts = [
            ...
            'behavioral',
            'task']
        return_list = np.concatenate(
            [[x + '__avg_cv_var_test', x + '__avg_cv_var_train'] for x in dropouts])
        return_list = np.concatenate(
            [return_list, ['ophys_experiment_id', 'cell_roi_id', 'cre_line', 'glm_version']])
    ...
    for glm_version in versions_to_compare:
        ...
        results_list.append(
            retrieve_results(
                ...
                remove_invalid_rois=remove_invalid_rois))
    results = pd.concat(results_list, sort=True)

    # Display summary statistics
    summary_table = results.groupby(['cre_line', 'glm_version'])[
        'Full__avg_cv_var_test'].mean()
    print('')
    print(summary_table)
    ...
    if save_results:
        summary_table.to_csv(
            '/allen/programs/braintv/workgroups/nc-ophys/visual_behavior/ophys_glm/version_comparisons/summary_table.csv',
            header=True)

    return results


def get_glm_version_comparison_table(
        versions_to_compare,
        results=None,
    ...
    if results is None:
        ...
        for glm_version in versions_to_compare:
            print(glm_version)
            results_list.append(retrieve_results(
...
def build_pivoted_results_summary(
        value_to_use,
    ...
    if results_summary is not None:
        ...
        )) == 1, 'number of glm_versions in the results summary cannot exceed 1'
    ...
    # merge in other identifying columns, leaving out those that will have
    # more than one unique value per cell
...
def summarize_variance_explained(results=None):
    ...
    return results.groupby(['glm_version', 'cre_line'])[
        'Full_avg_cv_var_test'].describe()
...
def get_experiment_inventory(results=None):
    ...
    experiments_table = loading.get_platform_paper_experiment_table(
        add_extra_columns=False, include_4x2_data=False)
...
def build_weights_df(
    ...
    weights_df = pd.merge(
        weights_df, results_pivoted, on=[
            'cell_specimen_id', 'ophys_experiment_id'], suffixes=(
            '_weights', ''))

    # If we didn't compute dropout scores, then there won't be redundant
    # columns, so the weights won't get appended with _weights
    ...
    for kernel in tqdm(kernels, desc='Interpolating kernels'):
        weights_df = interpolate_kernels(
            weights_df, run_params, kernel, normalize=normalize)
    ...
    weights_df['preferred_image_weights'] = weights_df.apply(
        lambda x: compute_preferred_kernel(
            [
                x['image0_weights'],
                x['image1_weights'],
                x['image2_weights'],
    ...
    weights_df['omissions_excited'] = weights_df.apply(
        lambda x: kernel_excitation(x['omissions_weights']), axis=1)
    weights_df['hits_excited'] = weights_df.apply(
        lambda x: kernel_excitation(x['hits_weights']), axis=1)
    weights_df['misses_excited'] = weights_df.apply(
        lambda x: kernel_excitation(x['misses_weights']), axis=1)
    weights_df['task_excited'] = weights_df.apply(
        lambda x: kernel_excitation(x['task_weights']), axis=1)
    weights_df['all-images_excited'] = weights_df.apply(
        lambda x: kernel_excitation(x['all-images_weights']), axis=1)
...
def interpolate_kernels(weights_df, run_params, kernel_name, normalize=False):
    ...
    length_mismatch = len(time_vec) != np.max(
        np.unique([np.size(x) for x in df]))
...
def compute_weight_index(weights_df):
    ...
    for k in kernels:
        weights_df[k + '_index'] = [np.sum(np.abs(x))
                                    for x in weights_df[k].values]

    weights_df['all-images_weights_index'] = weights_df['image0_weights_index'] + weights_df['image1_weights_index'] + weights_df['image2_weights_index'] + \
        weights_df['image3_weights_index'] + weights_df['image4_weights_index'] + weights_df['image5_weights_index'] + weights_df['image6_weights_index'] + weights_df['image7_weights_index']
    return weights_df
...
def append_kernel_excitation(weights_df, results_pivoted):
    ...
    for kernel in excited_kernels:
        ...
        results_pivoted.loc[results_pivoted[kernel +
                                            '_excited'] != True, kernel + '_positive'] = 0
        results_pivoted.loc[results_pivoted[kernel +
                                            '_excited'], kernel + '_negative'] = 0
        results_pivoted[kernel + '_signed'] = results_pivoted[kernel +
                                                              '_positive'] - results_pivoted[kernel + '_negative']
...
def get_matched_cell_ids_across_sessions(
        results_pivoted_sel,
        session_numbers,
        novelty=None):
    '''
    ...
    # check for retakes first. You cannot match cells if there are more than
    # one of the same session type.
    ophys_session_ids = []
    tmp = results_pivoted_sel[['mouse_id', 'session_number']].drop_duplicates()
    session_N = tmp.groupby(['mouse_id', 'session_number'])[
        'session_number'].value_counts()
...
def clean_glm_dropout_scores(
        results_pivoted,
        run_params,
        in_session_numbers=None):
    '''
...
def inventories_to_table(inventories):
    ...
    for version in summary:
        ...
        summary[version]['Complete'] = (
            summary[version]['missing_experiments'] == 0) & (
            summary[version]['missing_rois'] == 0)
...
def inventory_glm_version(
    ...
    cell_table = loading.get_cell_table(
        platform_paper_only=platform_paper_only,
        add_extra_columns=False,
        include_4x2_data=include_4x2_data).reset_index()
    ...
    # Compute list of rois and experiments that we have fit that are in the
    # dataset
    ...
    # Extra experiments, these could be old experiments that have since been
    # failed, or out of scope experiments
    ...
    # get any experiments for which the ROI count is incomplete. These are
    # 'incomplete_experiments'
    if valid_rois_only:
        incomplete_experiments = set()
        additional_missing_cells = list(set(cell_table.query('ophys_experiment_id in {}'.format(list(
            glm_results['ophys_experiment_id'].unique())))['cell_roi_id']) - set(glm_results['cell_roi_id']))
        for missing_cell in additional_missing_cells:
            associated_oeid = cell_table.query(
                'cell_roi_id == @missing_cell').iloc[0]['ophys_experiment_id']
...
def select_experiments_for_testing(returns='experiment_ids'):
    ...
    test_experiments = pd.read_csv(
        '/allen/programs/braintv/workgroups/nc-ophys/visual_behavior/ophys_glm/experiments_for_testing.csv')
...
def get_normalized_results_pivoted(
        glm_version=None,
        kind='max',
        cutoff=-np.inf):
    '''
    ...
    if kind == 'max':
        abs_max_df = results_pivoted[[
            'variance_explained_full', 'cell_specimen_id']].groupby('cell_specimen_id').max()
        results_pivoted_normalized = results_pivoted.join(
            abs_max_df, on='cell_specimen_id', rsuffix='_max').copy()

        for column in results_pivoted.columns:
            if column not in col_to_exclude:
                results_pivoted_normalized.loc[results_pivoted_normalized[column]
                                               > 0, column] = 0
                results_pivoted_normalized[column] = results_pivoted_normalized[column].divide(
                    results_pivoted_normalized['variance_explained_full_max'])
...
def get_kernel_weights(glm, kernel_name, cell_specimen_id):
    ...
    # get the weights
    w_kernel = glm.W.loc[dict(
        weights=kernel_weight_names, cell_specimen_id=cell_specimen_id)]
    ...
    offset_int = int(
        round(
            glm.design.kernel_dict[kernel_name]['offset_seconds'] /
            timestep))
...
def get_sem_thresholds(results_pivoted, alpha=0.05, metric='SEM'):
    ...
    thresholds = {}
    for cre in cres:
        thresholds[cre] = results_pivoted.query(
            'cre_line ==@cre')['variance_explained_full_sem'].quantile(1 - alpha)
    return thresholds
...
def compare_sem_thresholds(results_pivoted):
    ...
    for cre in cres:
        ...
        frac = (cre_slice['variance_explained_full']
                < 0.005).astype(int).mean()
    ...
    for cre in cres:
        cre_slice = results_pivoted.query('cre_line == @cre')
        frac = (cre_slice['variance_explained_full_sem'] >
                cre_slice['variance_explained_full']).astype(int).mean()
        print('{}: {}'.format(cre[0:3], np.round(frac, 3)))
...
def save_targeted_restart_table(run_params, results, save_table=True):
    ...
    nan_oeids = results[results['variance_explained'].isnull(
...
def make_table_of_nan_cells(run_params, results, save_table=True):
    ...
    '''
    nan_cells = results[results['variance_explained'].isnull()].query(
        'dropout=="Full"').copy()
    if save_table:
...
def check_nan_cells(fit):
    ...
    for c in nan_cells:
        plt.figure()
        plt.plot(fit['fit_trace_timestamps'], fit['fit_trace_arr']
                 [:, c], 'r', label='Interpolated events')
        plt.plot(fit['stimulus_interpolation']['original_timestamps'],
                 fit['stimulus_interpolation']['original_fit_arr'][:,
                                                                   c],
                 'b--',
                 label='Original Events')
        plt.plot(fit['stimulus_interpolation']['original_timestamps'],
                 fit['stimulus_interpolation']['original_dff_arr'][:,
                                                                   c],
                 'g--',
                 label='Original df/f',
                 alpha=.2)
...
def check_cv_nans(fit):
    ...
    ax[1, 1].hist(zero_VE - orig_VE, alpha=.5,
                  label='ZeroVE - Original', bins=20)
    ax[1, 1].hist(nan_VE - orig_VE, alpha=.5,
                  label='NaNVE - Original', bins=20)
    ax[1, 1].legend()
...
def reshape_rspm_by_experience(
        results_pivoted=None,
    ...
    if path is None and save_df is True:
        ...
        raise Warning(
            'Have to set save_df to True if you wish to save the file.')

    if results_pivoted is None:
        results_pivoted = build_pivoted_results_summary(
            value_to_use=model_output_type,
            results_summary=None,
            glm_version=glm_version,
            cutoff=cutoff)
    ...
    if ophys_experiment_ids_to_use is not None:
        results_pivoted = results_pivoted[results_pivoted['ophys_experiment_id'].isin(
            ophys_experiment_ids_to_use)]
    ...
    df = results_pivoted.groupby(
        ['cell_specimen_id', 'experience_level']).mean()
    ...
    if drop_duplicated_cells is True:
        if len(df) == len(np.unique(df.index.values)):
            ...
            print('found {} duplicated cells. But not removed. This needs to be fixed'.format(
                len(df) - len(np.unique(df.index.values))))
        elif len(df) < len(np.unique(df.index.values)):
...
def check_mesoscope(
    ...
    filters=[
        ...
        'depth',
        'meso']):
    results['meso'] = ['mesoscope' if x ==
                       "MESO.1" else 'scientifica' for x in results['equipment_name']]
    results['depth'] = [50 if x < 100 else 150 if x <
                        200 else 250 if x < 300 else 350 for x in results['imaging_depth']]
    summary = pd.DataFrame(results.groupby(
        filters)['Full__avg_cv_var_test'].mean())
    summary['err'] = results.groupby(
        filters)['Full__avg_cv_var_test'].sem() * 2
    summary['count'] = results.groupby(
        filters)['Full__avg_cv_var_test'].count()
    return summary
