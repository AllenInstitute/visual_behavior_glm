...
def compute_event_metrics(
    ...
    r2_events = compute_r2(
        results_events,
        groups=groups).rename(
        columns={
            'r2': 'glm_events__metrics_events'})
    evaluate_against_metrics(
        results_events.query('not passive'),
        ymetric='variance_explained_full',
        xmetric='trace_mean_over_std',
        savefig=savefig,
...
def compute_all(
    ...
    metrics_events = get_metrics(use_events=True)
    results_dff_dff = merge_cell_metrics_table(
        metrics_dff, results_pivoted_dff)
    results_events_dff = merge_cell_metrics_table(
        metrics_dff, results_pivoted_events)
    results_events_events = merge_cell_metrics_table(
        metrics_events, results_pivoted_events)
    r2_dff_dff = compute_r2(
        results_dff_dff,
        groups=groups).rename(
        columns={
            'r2': 'glm_dff__metrics_dff'})
    r2_events_dff = compute_r2(
        results_events_dff,
        groups=groups).rename(
        columns={
            'r2': 'glm_events__metrics_dff'})
    r2_events_events = compute_r2(
        results_events_events,
        groups=groups).rename(
        columns={
            'r2': 'glm_events__metrics_events'})
...
def compute_r2(
        results_metrics,
        groups=['cre_line'],
        metric1='variance_explained_full',
        metric2='trace_mean_over_std'):
    '''
    ...
    r2 = r2.drop(
        metric2, level=nlevels, axis=0).drop(
        columns=[metric1]).droplevel(nlevels)
...
def get_metrics(use_events=True, filter_events=False):
    metrics_df = cell_metrics.load_metrics_table_for_experiments(
        'all_experiments',
        'traces',
        'full_session',
        'full_session',
...
def merge_cell_metrics_table(metrics_df, results_pivoted):
    metrics_df['identifier'] = [str(x[0]) + '_' + str(x[1]) for x in zip(
        metrics_df['ophys_experiment_id'], metrics_df['cell_specimen_id'])]
    results_metrics = pd.merge(results_pivoted, metrics_df, on='identifier')
    results_metrics['equipment'] = ['Mesoscope' if x ==
                                    "MESO.1" else "Scientifica" for x in results_metrics['equipment_name']]
    results_metrics['active'] = [
        'Active' if not x else 'Passive' for x in results_metrics['passive']]
...
def plot_all(
    ...
    evaluate_against_metrics(
        ...
        title='All Cells')
    evaluate_against_metrics(
        results_metrics.query('equipment_name == "MESO.1"'),
        xmetric=metric,
        version=version,
        savefig=savefig,
        label='mesoscope_events',
        title='Mesoscope')
    evaluate_against_metrics(
        results_metrics.query('equipment_name != "MESO.!"'),
        xmetric=metric,
        version=version,
        savefig=savefig,
        label='scientifica_events',
        title='Scientifica')


def evaluate_against_metrics(
    results_metrics,
    ymetric='variance_explained_full',
    xmetric='trace_mean_over_std',
    savefig=False,
    ...
    results_metrics['cell_type'] = [mapper[x]
                                    for x in results_metrics['cre_line']]
    ...
    for cre in cell_types:
        x = results_metrics.query(
            'cell_type ==@cre')[xmetric].values.reshape((-1, 1))
