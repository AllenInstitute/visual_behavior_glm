...
def load_glm_data(glm_version):
    ...
    '''
    run_params, results, results_pivoted, weights_df, full_results = gfd.get_analysis_dfs(
        glm_version)
...
def save_data_summary(run_params, weights_df, normalize='none', other=None):
    ...
    '''
    _, _, _, _, _, _, _, _, _, _, normalization_amplitude = process_data(
        run_params, weights_df, 'image', normalize=normalize, other=other)
    _ = process_data(
        run_params,
        weights_df,
        'omissions',
...
def process_data(
        ...
        weights_df,
        kernel,
        normalize='max_by_cre',
        normalization_amplitude=None,
        other=None):
    '''
    ...
    familiar_weights = weights_df.query(
        '(ophys_session_id not in @problem_sessions) & (targeted_structure == "VISp") & (session_number in ["1","3"]) & (layer == "shallow")').copy()  # about 10k ROIs
    fpassive_weights = weights_df.query(
        '(ophys_session_id not in @problem_sessions) & (targeted_structure == "VISp") & (session_number in ["2"]) & (layer == "shallow")').copy()  # about 10k ROIs
    novel_weights = weights_df.query(
        '(ophys_session_id not in @problem_sessions) & (targeted_structure == "VISp") & (prior_exposures_to_image_set == 0) & (layer == "shallow")').copy()  # about 3.7k ROIs
    novelp_weights = weights_df.query(
        '(ophys_session_id not in @problem_sessions) & (targeted_structure == "VISp") & (session_number in ["4","6"]) & (prior_exposures_to_image_set > 0 ) & (layer == "shallow")').copy()  # about 5k
    nppassive_weights = weights_df.query(
        '(ophys_session_id not in @problem_sessions) & (targeted_structure == "VISp") & (session_number in ["5"]) & (prior_exposures_to_image_set > 0 ) & (layer == "shallow")').copy()  # about 5k

    # remove NaNs
    if kernel != 'image':
        familiar_weights = familiar_weights[familiar_weights[kernel +
                                                             '_weights'].notnull()].copy()
        if do_passive:
            fpassive_weights = fpassive_weights[fpassive_weights[kernel +
                                                                 '_weights'].notnull()].copy()
        novel_weights = novel_weights[novel_weights[kernel +
                                                    '_weights'].notnull()].copy()
        novelp_weights = novelp_weights[novelp_weights[kernel +
                                                       '_weights'].notnull()].copy()
        if do_passive:
            nppassive_weights = nppassive_weights[nppassive_weights[kernel +
                                                                    '_weights'].notnull()].copy()
    ...
    for cre in cres:
        ...
        f_kernel_dict[cre +
                      '_std'] = np.std(np.vstack(temp[kernel + '_weights']), 0)
        f_kernel_dict[cre + '_n'] = len(temp)
        f_kernel_dict[cre + '_sem'] = f_kernel_dict[cre + \
            '_std'] / np.sqrt(f_kernel_dict[cre + '_n'])

        if do_passive:
            temp = fpassive_weights.query('cre_line == @cre')
            fp_kernel_dict[cre] = np.mean(
                np.vstack(temp[kernel + '_weights']), 0)
            fp_kernel_dict[cre +
                           '_std'] = np.std(np.vstack(temp[kernel +
                                                           '_weights']), 0)
            fp_kernel_dict[cre + '_n'] = len(temp)
            fp_kernel_dict[cre + '_sem'] = fp_kernel_dict[cre + \
                '_std'] / np.sqrt(fp_kernel_dict[cre + '_n'])
        ...
        n_kernel_dict[cre + '_sem'] = n_kernel_dict[cre + \
            '_std'] / np.sqrt(n_kernel_dict[cre + '_n'])
        ...
        p_kernel_dict[cre + '_sem'] = p_kernel_dict[cre + \
            '_std'] / np.sqrt(p_kernel_dict[cre + '_n'])

        if do_passive:
            temp = nppassive_weights.query('cre_line == @cre')
            pp_kernel_dict[cre] = np.mean(
                np.vstack(temp[kernel + '_weights']), 0)
            pp_kernel_dict[cre +
                           '_std'] = np.std(np.vstack(temp[kernel +
                                                           '_weights']), 0)
            pp_kernel_dict[cre + '_n'] = len(temp)
            pp_kernel_dict[cre + '_sem'] = pp_kernel_dict[cre + \
                '_std'] / np.sqrt(pp_kernel_dict[cre + '_n'])
...
def get_time_vecs(kernel, run_params):
    ...
    meso_time = np.arange(
        ...
        10.725)
    if kernel == 'omissions':
        meso_time = np.concatenate(
            [meso_time, [meso_time[-1] + np.diff(meso_time)[0]]])
    if kernel == 'hits':
        meso_time = np.concatenate([meso_time,
                                    [meso_time[-1] + np.diff(meso_time)[0]],
                                    [meso_time[-1] + 2 * np.diff(meso_time)[0]]])
...
def get_filepath(
    ...
    if image_set == 'familiar':
        return FILEPATH + 'v_' + glm_version + '/dataset/' + kernel + '_normalized_by_' + normalize + \
            '_sessions_1_3_area_' + area + '_GLM_v' + glm_version[0:2] + '_' + data_type + '_' + other + '.pkl'
    elif image_set == 'fpassive':
        return FILEPATH + 'v_' + glm_version + '/dataset/' + kernel + '_normalized_by_' + normalize + \
            '_sessions_2_area_' + area + '_GLM_v' + glm_version[0:2] + '_' + data_type + '_' + other + '.pkl'
    elif image_set == 'novel':
        return FILEPATH + 'v_' + glm_version + '/dataset/' + kernel + '_normalized_by_' + normalize + \
            '_sessions_4_area_' + area + '_GLM_v' + glm_version[0:2] + '_' + data_type + '_' + other + '.pkl'
    elif image_set == 'nppassive':
        return FILEPATH + 'v_' + glm_version + '/dataset/' + kernel + '_normalized_by_' + normalize + \
            '_sessions_5_area_' + area + '_GLM_v' + glm_version[0:2] + '_' + data_type + '_' + other + '.pkl'
    else:
        return FILEPATH + 'v_' + glm_version + '/dataset/' + kernel + '_normalized_by_' + normalize + \
            '_sessions_6_area_' + area + '_GLM_v' + glm_version[0:2] + '_' + data_type + '_' + other + '.pkl'
...
def make_dataset(
    ...
    dataset['composite_images'] = make_composite(
        dataset, labels=define_composite())
    dataset['composite_omission'] = make_composite(
        dataset, labels=define_composite(omissions=[4]))
    if do_passive:
        dataset['composite_change'] = make_composite(
            dataset, labels=define_composite(
                num_images=12, changes=[4]))
    return dataset
    ...
    for c in cre:
        ...
        for dex, index in enumerate(image_indexes):
            if labels[dex] in ['i', 'c']:
                temp[index:index +
                     len(dataset['images']['time'])] += dataset['images'][c]
            if labels[dex] == 'o':
                temp[index:index +
                     len(dataset['omissions']['time'])] += dataset['omissions'][c]
            if labels[dex] == 'c':
                temp[index:index +
                     len(dataset['changes']['time'])] += dataset['changes'][c]
    ...
    if 'meta' in kernels:
        if kernels['meta']['kernel'] == 'composite':
            add_stimulus_bars(
                ax,
                kernels['meta']['kernel'],
                labels=kernels['meta']['composite_labels'])
        else:
