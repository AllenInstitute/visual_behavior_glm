...
def check_run_fits(VERSION):
    ...
    experiment_table = pd.read_csv(
        run_params['experiment_table_path']).reset_index(
        drop=True).set_index('ophys_experiment_id')
    ...
    for index, oeid in enumerate(experiment_table.index.values):
        ...
        filenamepbz2 = run_params['experiment_output_dir'] + \
            str(oeid) + ".pbz2"
        experiment_table.at[oeid, 'GLM_fit'] = os.path.isfile(
            filenamepkl) or os.path.isfile(filenamepbz2)
...
def check_weight_lengths(fit, design):
    ...
    num_weights_design = len(
        [x for x in design.get_X().weights.values if x.startswith('image0')])
    ...
    if ('dropouts' in fit) and ('train_weights' in fit['dropouts']['Full']):
        num_weights_fit = len([x for x in fit['dropouts']['Full'][
                              'train_weights'].weights.values if x.startswith('image0')])
...
def fit_experiment(oeid, run_params, NO_DROPOUTS=False, TESTING=False):
    ...
    fit, run_params = extract_and_annotate_ophys(
        session, run_params, TESTING=TESTING)
    ...
    if ('interpolate_to_stimulus' in run_params) and (
            run_params['interpolate_to_stimulus']):
    ...
    if (not NO_DROPOUTS) and (fit['ok_to_fit_preferred_engagement']) and (
            run_params['version_type'] == 'production'):
    ...
    filepath = os.path.join(
        run_params['experiment_output_dir'],
        str(oeid) + '.pbz2')
...
def evaluate_shuffle(fit, design, method='cells', num_shuffles=50):
    ...
    for count in tqdm(
            range(
                ...
                num_shuffles),
            total=num_shuffles,
            desc='    Shuffling by {}'.format(method)):
        if method == 'time':
            for dex in range(0, np.shape(fit_trace_shuffle)[1]):
                shuffle_count = np.random.randint(1, max_shuffle)
                fit_trace_shuffle[:, dex] = np.roll(
                    fit_trace_shuffle[:, dex], shuffle_count, axis=0)
        elif method == 'cells':
...
def evaluate_ridge(fit, design, run_params, session):
    ...
    if run_params['L2_use_fixed_value']:
        ...
        fit['cell_L2_regularization'] = np.empty(
            (fit['fit_trace_arr'].shape[1],))
        ...
        if run_params['L2_grid_type'] == 'log':
            ...
            fit['L2_grid'] = np.linspace(
                run_params['L2_grid_range'][0],
                run_params['L2_grid_range'][1],
                num=run_params['L2_grid_num'])
        train_cv = np.empty(
            (fit['fit_trace_arr'].shape[1], len(
                fit['L2_grid'])))
        test_cv = np.empty(
            (fit['fit_trace_arr'].shape[1], len(
                fit['L2_grid'])))
        ...
        for L2_index, L2_value in enumerate(fit['L2_grid']):
            cv_var_train = np.empty(
                (fit['fit_trace_arr'].shape[1], len(
                    fit['ridge_splits'])))
            cv_var_test = np.empty(
                (fit['fit_trace_arr'].shape[1], len(
                    fit['ridge_splits'])))

            # Iterate over CV splits
            for split_index, test_split in tqdm(
                enumerate(
                    fit['ridge_splits']), total=len(
                    fit['ridge_splits']), desc='    Fitting L2, {}'.format(L2_value)):
                ...
                cv_var_train[:, split_index] = variance_ratio(
                    fit_trace_train, W, X_train)
                cv_var_test[:, split_index] = variance_ratio(
                    fit_trace_test, W, X_test)

            train_cv[:, L2_index] = np.mean(cv_var_train, 1)
            test_cv[:, L2_index] = np.mean(cv_var_test, 1)

        fit['avg_L2_regularization'] = np.mean(
            [fit['L2_grid'][x] for x in np.argmax(test_cv, 1)])
        fit['cell_L2_regularization'] = [fit['L2_grid'][x]
                                         for x in np.argmax(test_cv, 1)]
        fit['L2_test_cv'] = test_cv
        fit['L2_train_cv'] = train_cv
        fit['L2_at_grid_min'] = [x == 0 for x in np.argmax(test_cv, 1)]
        fit['L2_at_grid_max'] = [x == (len(fit['L2_grid']) - 1)
                                 for x in np.argmax(test_cv, 1)]
    return fit
...
def evaluate_lasso(fit, design, run_params):
    ...
    cell_train_cv = []
    cell_test_cv = []
    for cell_index, cell_value in tqdm(
        enumerate(
            fit['fit_trace_arr']['cell_specimen_id'].values), total=len(
            fit['fit_trace_arr']['cell_specimen_id'].values), desc='   Fitting Cells'):
    ...
    fit['avg_Lasso_regularization'] = np.mean(
        [fit['Lasso_grid'][x] for x in np.argmax(cell_test_cv, 1)])
    fit['cell_Lasso_regularization'] = [fit['Lasso_grid'][x]
                                        for x in np.argmax(cell_test_cv, 1)]
    ...
    fit['Lasso_at_grid_max'] = [
        x == (len(fit['Lasso_grid']) - 1) for x in np.argmax(cell_test_cv, 1)]
    return fit
...
def evaluate_models(fit, design, run_params):
    ...
    if not fit['ok_to_fit_preferred_engagement']:
        print(
            '\tSkipping model evaluate because insufficient preferred engagement timepoints')
        ...
        fit['dropouts']['Full']['cv_weights'] = np.empty(
            (0, fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        fit['dropouts']['Full']['cv_var_train'] = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        fit['dropouts']['Full']['cv_var_test'] = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        fit['dropouts']['Full']['cv_adjvar_train'] = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        fit['dropouts']['Full']['cv_adjvar_test'] = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        fit['dropouts']['Full']['cv_adjvar_train_full_comparison'] = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        fit['dropouts']['Full']['cv_adjvar_test_full_comparison'] = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        # Needs to be xarray
        fit['dropouts']['Full']['train_weights'] = dummy_weights
        fit['dropouts']['Full']['train_variance_explained'] = np.empty(
            (fit['fit_trace_arr'].shape[1],))
        fit['dropouts']['Full']['train_adjvariance_explained'] = np.empty(
            (fit['fit_trace_arr'].shape[1],))
        fit['dropouts']['Full']['full_model_train_prediction'] = np.empty(
            (0, fit['fit_trace_arr'].shape[1]))
...
def evaluate_models_lasso(fit, design, run_params):
    ...
    for model_label in fit['dropouts'].keys():
        ...
        cv_var_test = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        cv_adjvar_train = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        cv_adjvar_test = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        cv_adjvar_train_fc = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        cv_adjvar_test_fc = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        cv_weights = np.empty(
            (np.shape(X)[1], fit['fit_trace_arr'].shape[1], len(
                fit['splits'])))
        ...
        for cell_index, cell_value in tqdm(
            ...
            Wall = fit_cell_lasso_regularized(
                fit_trace, X, fit['cell_Lasso_regularization'][cell_index])
            ...
            for index, test_split in enumerate(fit['splits']):
                ...
                # If this is the first cell, stash the design matrix and
                # covariance result
                if cell_index == 0:
                ...
                # do the fitting
                W = fit_cell_lasso_regularized(
                    fit_trace_train, X_train, fit['cell_Lasso_regularization'][cell_index])

                cv_var_train[cell_index, index] = variance_ratio(
                    fit_trace_train, W, X_train)
                cv_var_test[cell_index, index] = variance_ratio(
                    fit_trace_test, W, X_test)
                ...
                if model_label == 'Full':
                    ...
                    # Otherwise, get weights and design matrix for this
                    # cell/cv_split and compute the variance explained on this
                    # mask
                    Full_W = xr.DataArray(
...
def evaluate_models_different_ridge(fit, design, run_params):
    ...
    for model_label in fit['dropouts'].keys():
        ...
        for cell_index, cell_value in tqdm(
            ...
            fit_trace = fit['fit_trace_arr'][:, cell_index]
            Wall = fit_cell_regularized(
                X_inner, fit_trace, X, fit['cell_L2_regularization'][cell_index])
            var_explain = variance_ratio(fit_trace, Wall, X)
            adjvar_explain = masked_variance_ratio(fit_trace, Wall, X, mask)
            all_weights[:, cell_index] = Wall
            all_var_explain[cell_index] = var_explain
            ...
            for index, test_split in enumerate(fit['splits']):
                ...
                if cell_index == 0:
                    ...
                    X_train_array.append(X[train_split, :])
                    X_cov_array.append(
                        np.dot(X[train_split, :].T, X[train_split, :]))
                # Grab the stashed result
                X_test = X_test_array[index]
                X_train = X_train_array[index]
...
def evaluate_models_same_ridge(fit, design, run_params):
    ...
    for model_label in fit['dropouts'].keys():
        ...
        cv_adjvar_test_fc = np.empty(
            (fit['fit_trace_arr'].shape[1], len(fit['splits'])))
        cv_weights = np.empty(
            (np.shape(Wall)[0], np.shape(Wall)[1], len(
                fit['splits'])))

        for index, test_split in tqdm(enumerate(fit['splits']), total=len(
                fit['splits']), desc='    Fitting model, {}'.format(model_label)):
            train_split = np.sort(np.concatenate(
                [split for i, split in enumerate(fit['splits']) if i != index]))
            X_test = X[test_split, :]
            X_train = X[train_split, :]
            ...
            W = fit_regularized(
                fit_trace_train,
                X_train,
                fit['avg_L2_regularization'])
            cv_var_train[:, index] = variance_ratio(
                fit_trace_train, W, X_train)
            ...
            cv_weights[:, :, index] = W
            if model_label == 'Full':
                # If this model is Full, then the masked variance ratio is the
                # same
                ...
                # Otherwise load the weights and design matrix for this
                # cv_split, and compute VE with this support mask
                Full_W = xr.DataArray(
                    fit['dropouts']['Full']['cv_weights'][:, :, index])
                Full_X_test = Full_X[test_split, :]
                Full_X_train = Full_X[train_split, :]
                cv_adjvar_train_fc[:, index] = masked_variance_ratio(
...
def build_dataframe_from_dropouts(fit, run_params):
    ...
    # Determines whether to record VE and dropout scores without dealing with
    # -Inf on CV splits with no activity
    ...
    if len(nan_cells) > 0:
        print(
            'I found {} cells with all 0 in the fit_trace_arr'.format(
                len(nan_cells)))
    ...
    for model_label in fit['dropouts'].keys():
        ...
        results[model_label + "__avg_cv_var_train"] = np.nanmean(
            fit['dropouts'][model_label]['cv_var_train'], 1)
        if compute_with_infs:
            results[model_label + "__avg_cv_var_train_raw"] = np.mean(
                fit['dropouts'][model_label]['cv_var_train'], 1)
            results[model_label + "__avg_cv_var_test_raw"] = np.mean(
                fit['dropouts'][model_label]['cv_var_test'], 1)
            results[model_label + "__avg_cv_var_test_full_comparison_raw"] = np.mean(
                fit['dropouts']['Full']['cv_var_test'], 1)
        ...
        results[model_label + \
            "__avg_cv_var_test_sem"] = np.std(temp, 1) / np.sqrt(np.shape(temp)[1])

        # Screen for -Inf values in full model
        full_temp = fit['dropouts']['Full']['cv_var_test']
        full_temp[np.isinf(full_temp)] = 0
        results[model_label +
                "__avg_cv_var_test_full_comparison"] = np.mean(full_temp, 1)
        ...
        results[model_label + "__avg_cv_adjvar_train"] = np.nanmean(
            fit['dropouts'][model_label]['cv_adjvar_train'], 1)
        if compute_with_infs:
            results[model_label + "__avg_cv_adjvar_test_raw"] = np.nanmean(
                fit['dropouts'][model_label]['cv_adjvar_test'], 1)
            results[model_label + "__avg_cv_adjvar_test_full_comparison_raw"] = np.nanmean(
                fit['dropouts'][model_label]['cv_adjvar_test_full_comparison'], 1)
        ...
        full_temp[np.isinf(full_temp)] = 0
        results[model_label +
                "__avg_cv_adjvar_test_full_comparison"] = np.nanmean(full_temp, 1)

        # Clip the variance explained values to >= 0
        results.loc[results[model_label + "__avg_cv_var_test"]
                    < 0, model_label + "__avg_cv_var_test"] = 0
        results.loc[results[model_label +
                            "__avg_cv_var_test_full_comparison"] < 0, model_label +
                    "__avg_cv_var_test_full_comparison"] = 0
        results.loc[results[model_label + "__avg_cv_adjvar_test"]
                    < 0, model_label + "__avg_cv_adjvar_test"] = 0
        results.loc[results[model_label +
                            "__avg_cv_adjvar_test_full_comparison"] < 0, model_label +
                    "__avg_cv_adjvar_test_full_comparison"] = 0
        if compute_with_infs:
            results.loc[results[model_label + "__avg_cv_var_test_raw"]
                        < 0, model_label + "__avg_cv_var_test_raw"] = 0
            results.loc[results[model_label +
                                "__avg_cv_var_test_full_comparison_raw"] < 0, model_label +
                        "__avg_cv_var_test_full_comparison_raw"] = 0
            results.loc[results[model_label + "__avg_cv_adjvar_test_raw"]
                        < 0, model_label + "__avg_cv_adjvar_test_raw"] = 0
            results.loc[results[model_label +
                                "__avg_cv_adjvar_test_full_comparison_raw"] < 0, model_label +
                        "__avg_cv_adjvar_test_full_comparison_raw"] = 0

        # Compute the absolute change in variance
        results[model_label + "__absolute_change_from_full"] = results[model_label +
                                                                       "__avg_cv_var_test"] - results[model_label + "__avg_cv_var_test_full_comparison"]

        # Compute the dropout scores, which is dependent on whether this was a
        # single-dropout or not
        if fit['dropouts'][model_label]['is_single']:
            # Compute the dropout
            results[model_label + "__dropout"] = -results[model_label + "__avg_cv_var_test"] / \
                results[model_label + "__avg_cv_var_test_full_comparison"]
            results[model_label + "__adj_dropout"] = -results[model_label + "__avg_cv_adjvar_test"] / \
                results[model_label + "__avg_cv_adjvar_test_full_comparison"]
            if compute_with_infs:
                results[model_label + "__adj_dropout_raw"] = -results[model_label + "__avg_cv_adjvar_test_raw"] / \
                    results[model_label + "__avg_cv_adjvar_test_full_comparison_raw"]

            # Cleaning Steps, careful eye here!
            # If the single-dropout explained more variance than the
            # full_comparison, clip dropout to -1
            results.loc[results[model_label + "__avg_cv_adjvar_test_full_comparison"] <
                        results[model_label + "__avg_cv_adjvar_test"], model_label + "__adj_dropout"] = -1
            results.loc[results[model_label + "__avg_cv_var_test_full_comparison"] <
                        results[model_label + "__avg_cv_var_test"], model_label + "__dropout"] = -1
            if compute_with_infs:
                results.loc[results[model_label + "__avg_cv_adjvar_test_full_comparison_raw"] <
                            results[model_label + "__avg_cv_adjvar_test_raw"], model_label + "__adj_dropout_raw"] = -1

            # If the single-dropout explained less than THRESHOLD variance,
            # clip dropout to 0
            results.loc[results[model_label + "__avg_cv_adjvar_test"]
                        < threshold, model_label + "__adj_dropout"] = 0
            results.loc[results[model_label + "__avg_cv_var_test"]
                        < threshold, model_label + "__dropout"] = 0
            if compute_with_infs:
                results.loc[results[model_label + "__avg_cv_adjvar_test_raw"]
                            < threshold, model_label + "__adj_dropout_raw"] = 0

            # If the full_comparison model explained less than THRESHOLD
            # variance, clip the dropout to 0.
            results.loc[results[model_label + "__avg_cv_adjvar_test_full_comparison"]
                        < threshold, model_label + "__adj_dropout"] = 0
            results.loc[results[model_label + "__avg_cv_var_test_full_comparison"]
                        < threshold, model_label + "__dropout"] = 0
            ...
            # Compute the dropout
            if compute_with_infs:
                results[model_label + "__adj_dropout_raw"] = -(
                    1 - results[model_label + "__avg_cv_adjvar_test_raw"] / results[model_label + "__avg_cv_adjvar_test_full_comparison_raw"])
            results[model_label + "__adj_dropout"] = - \
                (1 - results[model_label + "__avg_cv_adjvar_test"] / results[model_label + "__avg_cv_adjvar_test_full_comparison"])
            results[model_label + "__dropout"] = - \
                (1 - results[model_label + "__avg_cv_var_test"] / results[model_label + "__avg_cv_var_test_full_comparison"])

            # Cleaning Steps, careful eye here!
            # If the dropout explained more variance than the full_comparison,
            # clip the dropout to 0
            if compute_with_infs:
                results.loc[results[model_label +
                                    "__avg_cv_adjvar_test_full_comparison_raw"] < results[model_label +
                                                                                          "__avg_cv_adjvar_test_raw"], model_label +
                            "__adj_dropout_raw"] = 0
            results.loc[results[model_label +
                                "__avg_cv_adjvar_test_full_comparison"] < results[model_label +
                                                                                  "__avg_cv_adjvar_test"], model_label +
                        "__adj_dropout"] = 0
            results.loc[results[model_label +
                                "__avg_cv_var_test_full_comparison"] < results[model_label +
                                                                               "__avg_cv_var_test"], model_label +
                        "__dropout"] = 0
    ...
    assert results['Full__avg_cv_var_test'].isnull(
    ).sum() == 0, "NaNs in variance explained"
    assert results['Full__avg_cv_var_train'].isnull(
    ).sum() == 0, "NaNs in variance explained"

    if 'var_shuffle_cells' in fit:
        # If the shuffle across cells was computed, record average VE for each
        # cell
        results['Full__shuffle_cells'] = np.nanmean(
            fit['var_shuffle_cells'], 1)

    if 'var_shuffle_time' in fit:
        # If the shuffle across time was computed, record average VE for each
        # cell
...
def load_data(oeid, run_params):
    ...
    dataset = loading.get_ophys_dataset(
        oeid, include_invalid_rois=include_invalid_rois)

    return dataset


def process_behavior_predictions(
        session,
    ...
    for column in ['lick', 'groom']:
        ...
        behavior_predictions[column][behavior_predictions[column]
                                     < cutoff_threshold] = 0
...
def process_eye_data(session, run_params, ophys_timestamps=None):
    ...
    for column in eye.keys():
        if column != 'timestamps':
            f = scipy.interpolate.interp1d(
                eye['timestamps'], eye[column], bounds_error=False)
            ophys_eye[column] = f(ophys_eye['timestamps'])
            ophys_eye[column].fillna(method='ffill', inplace=True)
            if column in z_score:
                ophys_eye[column + '_zscore'] = scipy.stats.zscore(
                    ophys_eye[column], nan_policy='omit')
...
def process_data(session, run_params, TESTING=False):
    ...
    if ('use_events' in run_params) and (run_params['use_events']):
        ...
        assert np.size(dff_trace_arr) == np.size(
            events_trace_arr), 'Events array doesnt match size of df/f array'
    ...
    assert np.sum(timestamps_to_use) == len(
        fit_trace_arr['fit_trace_timestamps'].values), 'length of `timestamps_to_use` must match length of `fit_trace_timestamps` in `fit_trace_arr`'
    assert np.sum(
        timestamps_to_use) == fit_trace_arr.values.shape[0], 'length of `timestamps_to_use` must match 0th dimension of `fit_trace_arr`'
    if ('include_invalid_rois' in run_params):
        include_invalid_rois = (run_params['include_invalid_rois'])
    else:
        include_invalid_rois = False
    ...
    if include_invalid_rois:
        assert len(
            session.cell_specimen_table) == fit_trace_arr.values.shape[1], 'number of ROIs must match 1st dimension of `fit_trace_arr`'
    else:
        assert len(session.cell_specimen_table.query('valid_roi == True')
                   ) == fit_trace_arr.values.shape[1], 'number of valid ROIs must match 1st dimension of `fit_trace_arr`'
...
def extract_and_annotate_ophys(session, run_params, TESTING=False):
    ...
    fit['fit_trace_bins'] = np.concatenate([fit['fit_trace_timestamps'], [
                                           fit['fit_trace_timestamps'][-1] + step]]) - step * .5
    ...
    # If we are splitting on engagement, then determine the engagement
    # timepoints
    if run_params['split_on_engagement']:
        print(
            'Adding Engagement labels. Preferred engagement state: ' +
...
def interpolate_to_stimulus(fit, session, run_params):
    ...
    '''
    if ('interpolate_to_stimulus' not in run_params) or (
            not run_params['interpolate_to_stimulus']):
    ...
    ##### NOTE - ADDED BY MARINA ON 3/30/24 TO HANDLE UPDATES TO ALLENSDK==2.1
    if allensdk.__version__ == '2.16.2':  # new SDK version has stimulus blocks and made the omitted column a Boolean rather than bool
        if 'stimulus_block_name' in filtered_stimulus_presentations:
            # limit to change detection block (gets rid of NaNs in omitted
            # column)
            filtered_stimulus_presentations = filtered_stimulus_presentations[
                filtered_stimulus_presentations.stimulus_block_name.str.contains('change_detection')]
            # convert to bool instead of Boolean so subsequent query works
            # properly
            filtered_stimulus_presentations['omitted'] = filtered_stimulus_presentations['omitted'].astype(
                'bool')
    ##### NOTE - ADDED BY MARINA ON 3/30/24 TO HANDLE UPDATES TO ALLENSDK==2.1

    while filtered_stimulus_presentations.iloc[0]['omitted']:
        filtered_stimulus_presentations = filtered_stimulus_presentations.iloc[1:]

    # Make new timestamps by starting with each stimulus start time, and
    # adding time points until we hit the next stimulus
    ...
    for index, start in enumerate(start_times[0:-1]):
        sets_of_stimulus_timestamps.append(np.arange(
            start_times[index], start_times[index + 1] - mean_step * .5, mean_step))

    # Check to make sure we always have the same number of timestamps per
    # stimulus
    ...
    if len(np.unique(lens)) > 1:
        ...
        for index, val in enumerate(u):
            print(
                '   Stimuli with {} timestamps: {}'.format(
                    u[index], c[index]))
        ...
        # Determine how many timestamps each stimuli most commonly has and trim
        # off the extra
        sets_of_stimulus_timestamps = [x[0:mode]
                                       for x in sets_of_stimulus_timestamps]
        ...
        if len(np.unique(lens)) > 1:
            ...
            print(
                '   I will need to check for this happening when kernels are added to the design matrix')
            u, c = np.unique(lens, return_counts=True)
            overlaps = 0
            for index, val in enumerate(u):
                print(
            ...
            if ('image_kernel_overlap_tol' in run_params) & (
                    run_params['image_kernel_overlap_tol'] > 0):
                print(
                    'checking to see if image kernel overlap is within tolerance ({})'.format(
                        run_params['image_kernel_overlap_tol']))
                print('overlapping timestamps: {}'.format(overlaps))
                if overlaps > run_params['image_kernel_overlap_tol']:
                    raise Exception(
                        'Uneven number of steps per stimulus interval')
                else:
    ...
    new_bins = np.concatenate(
        [new_timestamps, [new_timestamps[-1] + mean_step]]) - mean_step * .5
    ...
    for index in range(0, num_cells):
        # Fit array
        f = scipy.interpolate.interp1d(fit['fit_trace_timestamps'],
                                       fit['fit_trace_arr'][:,
                                                            index],
                                       bounds_error=False,
    ...
    # Use the number of timesteps per stimulus to define the image kernel
    # length so we get no overlap
    kernels_to_limit_per_image_cycle = [
        'image0',
        'image1',
        'image2',
    ...
    assert np.isnan(fit['fit_trace_arr']).sum(
    ) == 0, "Have NaNs in fit_trace_arr"
...
def check_image_kernel_alignment(design, run_params):
    ...
    if overlap > tolerance:
        raise Exception(
            'Image kernels overlap beyond tolerance: {}, {}'.format(
                overlap, tolerance))
    elif overlap > 1:
        print(
            'Image kernels overlap, but within tolerance: {}, {}'.format(
                overlap, tolerance))
    else:
...
def check_interpolation_to_stimulus(fit, session):
    ...
    if allensdk.__version__ == '2.16.2':  # new SDK version has stimulus blocks and made the omitted column a Boolean rather than bool
        if 'stimulus_block_name' in temp:
            # limit to change detection block (gets rid of NaNs in omitted
            # column)
            temp = temp[temp.stimulus_block_name.str.contains(
                'change_detection')]
            # convert to bool instead of Boolean so subsequent query works
            # properly
    ...
    temp.at[temp.index.values[-1],
            'next_start'] = temp.iloc[-1]['start_time'] + 0.75
    for index, row in temp.iterrows():
        stamps = np.sum(
            (fit['fit_trace_timestamps'] >= row.start_time) & (
                fit['fit_trace_timestamps'] < row.next_start))
        lens.append(stamps)
    return temp, lens
    if len(np.unique(lens)) > 1:
        u, c = np.unique(lens, return_counts=True)
        for index, val in enumerate(u):
            print(
                '   Stimuli with {} timestamps: {}'.format(
                    u[index], c[index]))
        raise Exception(
            'Uneven number of timestamps per stimulus presentation')
...
def plot_interpolation_debug(fit, session):
    ...
    # Stim start
    ax[0].plot(fit['stimulus_interpolation']['original_timestamps'][0:50],
               fit['stimulus_interpolation']['original_fit_arr'][0:50,
                                                                 0],
               'ko',
               markerfacecolor='None',
               label='Original')
    ax[0].plot(fit['fit_trace_timestamps'][0:50], fit['fit_trace_arr'][
               0:50, 0], 'bo', markerfacecolor='None', label='Stimulus Aligned')
    ...
    if allensdk.__version__ == '2.16.2':  # new SDK version has stimulus blocks and made the omitted column a Boolean rather than bool
        if 'stimulus_block_name' in stimulus_presentations:
            # limit to change detection block (gets rid of NaNs in omitted
            # column)
            stimulus_presentations = stimulus_presentations[stimulus_presentations.stimulus_block_name.str.contains(
                'change_detection')]
            # convert to bool instead of Boolean so subsequent query works
            # properly
            stimulus_presentations['omitted'] = stimulus_presentations['omitted'].astype(
                'bool')
    ##### NOTE - ADDED BY MARINA ON 3/30/24 TO HANDLE UPDATES TO ALLENSDK==2.1
    ...
    for dex in range(0, len(stimulus_presentations)):
        if stimulus_presentations.loc[dex].start_time > fit['stimulus_interpolation']['original_timestamps'][50]:
            break
        ax[0].axvline(
            stimulus_presentations.loc[dex].start_time,
            color='r',
            markerfacecolor='None')
    for dex, val in enumerate(
            fit['stimulus_interpolation']['original_fit_arr'][0:50, 0]):
        ax[0].plot([fit['stimulus_interpolation']['original_bins'][dex],
                    fit['stimulus_interpolation']['original_bins'][dex + 1]],
                   [val,
                    val],
                   'k-',
                   alpha=.5)
    for dex, val in enumerate(fit['fit_trace_arr'][0:50, 0]):
        ax[0].plot([fit['fit_trace_bins'][dex], fit['fit_trace_bins']
                   [dex + 1]], [val, val], 'b-', alpha=.5)
    ...
    # Stim end
    ax[1].plot(fit['stimulus_interpolation']['original_timestamps'][-50:],
               fit['stimulus_interpolation']['original_fit_arr'][-50:,
                                                                 0],
               'ko',
               markerfacecolor='None')
    ax[1].plot(fit['fit_trace_timestamps'][-50:], fit['fit_trace_arr']
               [-50:, 0], 'bo', markerfacecolor='None')
    ...
    for dex in range(0, len(stimulus_presentations)):
        if stimulus_presentations.loc[dex].start_time > fit['stimulus_interpolation']['original_timestamps'][-50]:
            ax[1].axvline(
                ...
                markerfacecolor='None')
    for dex, val in enumerate(
            fit['stimulus_interpolation']['original_fit_arr'][-50:, 0]):
        ax[1].plot([fit['stimulus_interpolation']['original_bins'][-51 + dex],
                    fit['stimulus_interpolation']['original_bins'][-50 + dex]],
                   [val,
                    val],
                   'k-',
                   alpha=.5)
    for dex, val in enumerate(fit['fit_trace_arr'][-50:, 0]):
        ax[1].plot([fit['fit_trace_bins'][-51 + dex],
                   fit['fit_trace_bins'][-50 + dex]], [val, val], 'b-', alpha=.5)
...
def add_engagement_labels(fit, session, run_params):
    ...
    # Get reward rate
    stimulus_presentations = reformat.add_rewards_each_flash(
        stimulus_presentations, session.rewards)
    stimulus_presentations['rewarded'] = [
        len(x) > 0 for x in stimulus_presentations['rewards']]
    ...
    stimulus_presentations['engaged'] = [
        x > reward_threshold for x in stimulus_presentations['reward_rate']]

    # Make dataframe with start/end of each image cycle pinned with correct
    # engagement value
    ...
    engaged_df = engaged_df.sort_values(
        by='start_time').rename(
        columns={
            'start_time': 'timestamps',
            'engaged': 'values'})

    # Interpolate onto fit timestamps
    fit['engaged'] = interpolate_to_ophys_timestamps(fit, engaged_df)[
        'values'].values
    print('\t% of session engaged:    ' +
          str(np.sum(fit['engaged']) / len(fit['engaged'])))
    print('\t% of session disengaged: ' +
          str(1 - np.sum(fit['engaged']) / len(fit['engaged'])))

    # Check min_engaged_duration:
    seconds_in_engaged = np.sum(fit['engaged']) / fit['ophys_frame_rate']
    seconds_in_disengaged = np.sum(
        ~fit['engaged'].astype(bool)) / fit['ophys_frame_rate']
...
def add_kernels(design, run_params, session, fit):
    ...
    for kernel_name in run_params['kernels']:
        ...
        if run_params['kernels'][kernel_name]['type'] == 'discrete':
            design = add_discrete_kernel_by_label(
                kernel_name, design, run_params, session, fit)
        else:
            design = add_continuous_kernel_by_label(
                kernel_name, design, run_params, session, fit)
...
def clean_failed_kernels(run_params):
    ...
    for kernel in run_params['failed_kernels']:
        ...
        for dropout in run_params['dropouts'].keys():
            # If the failed kernel is in this dropout, remove the kernel from
            # the kernel list
            if kernel in run_params['dropouts'][dropout]['kernels']:
                run_params['dropouts'][dropout]['kernels'].remove(kernel)
            # If the failed kernel is in the dropped kernel list, remove from
            # dropped kernel list
            if kernel in run_params['dropouts'][dropout]['dropped_kernels']:
                run_params['dropouts'][dropout]['dropped_kernels'].remove(
                    kernel)
...
def add_continuous_kernel_by_label(
        kernel_name,
        design,
        run_params,
        session,
    ...
    try:
        ...
        if event == 'intercept':
            ...
            timeseries = interpolate_to_ophys_timestamps(
                fit, face_motion_df)['values'].values
            timeseries = standardize_inputs(
                timeseries,
            ...
            # TODO investigate where these NaNs come from
            ...
            session.ophys_eye = process_eye_data(
                session, run_params, ophys_timestamps=fit['fit_trace_timestamps'])
            ...
            if not hasattr(session, 'lick_groom_model'):
                session.lick_groom_model = process_behavior_predictions(
                    session, ophys_timestamps=fit['fit_trace_timestamps'])
            timeseries = session.lick_groom_model[event.split('_')[0]].values
        else:
            raise Exception('Could not resolve kernel label')
    except Exception as e:
        print('\tError encountered while adding kernel for ' +
              kernel_name + '. Attemping to continue without this kernel. ')
        print(e)
        ...
        assert len(
            timeseries) == fit['fit_trace_arr'].values.shape[0], 'Length of continuous regressor must match length of fit_trace_timestamps'
...
def add_discrete_kernel_by_label(
        kernel_name,
        design,
        run_params,
        session,
    ...
    try:
        ...
        if event == 'licks':
            ...
            licks = session.licks
            licks['pre_ILI'] = licks['timestamps'] - \
                licks['timestamps'].shift(fill_value=-10)
            licks['post_ILI'] = licks['timestamps'].shift(
                periods=-1, fill_value=5000) - licks['timestamps']
            licks['bout_start'] = licks['pre_ILI'] > run_params['lick_bout_ILI']
            licks['bout_end'] = licks['post_ILI'] > run_params['lick_bout_ILI']
            assert np.sum(
                licks['bout_start']) == np.sum(
                licks['bout_end']), "Lick bout splitting failed"
            ...
            event_times = np.concatenate(
                [
                    np.arange(
                        x[0],
                        x[0] + run_params['min_time_per_bout'],
                        run_params['min_interval']) if x[2] else np.arange(
                        x[0],
                        x[0] + x[1],
                        run_params['min_interval']) for x in zip(
                        licks['timestamps'],
                        licks['post_ILI'],
                        licks['bout_end'])])
        elif event == 'rewards':
            ...
            # This method drops auto-rewarded changes
            event_times = stimulus_presentations.query(
                'is_change')['start_time'].values
            event_times = event_times[~np.isnan(event_times)]
        elif event in ['hit', 'miss', 'false_alarm', 'correct_reject']:
            # Includes auto-rewarded changes as hits, since they include a
            # reward.
            if event == 'hit':
                event_times = session.trials.query('hit or auto_rewarded')[
                    'change_time'].values
            else:
            ...
            if len(session.rewards) < 5:  # HARD CODING THIS VALUE
                raise Exception(
                    'Trial type regressors arent defined for passive sessions (sessions with less than 5 rewards)')
        ...
        events_vec, timestamps = np.histogram(
            event_times, bins=fit['fit_trace_bins'])

        if (event == 'lick_bouts') or (event == 'licks'):
            # Force this to be 0 or 1, since we purposefully over-tiled the
            # space.
            events_vec[events_vec > 1] = 1

        if np.max(events_vec) > 1:
            raise Exception(
                'Had multiple events in the same timebin, {}'.format(kernel_name))
...
def check_by_engagement_state(run_params, fit, event_times, event):
    ...
    if run_params['engagement_preference'] == 'engaged':
        preferred_engagement_state_event_times = events_vec[fit['engaged'].astype(
            bool)]
        nonpreferred_engagement_state_event_times = events_vec[~fit['engaged'].astype(
            bool)]
    else:
        preferred_engagement_state_event_times = events_vec[~fit['engaged'].astype(
            bool)]
    ...
    if np.sum(preferred_engagement_state_event_times) < 5:
        raise Exception('\tLess than minimum number of events in preferred engagement state: ' +
                        str(np.sum(preferred_engagement_state_event_times)) +
                        ' ' +
                        event +
                        ...
                        str(np.sum(nonpreferred_engagement_state_event_times)))
...
class DesignMatrix(object):
    ...
    def get_X(self, kernels=None):
        ...
        for kernel_name in kernels:
            ...
            param_labels.append(
                self.make_labels(
                    kernel_name,
                    np.shape(
                        self.kernel_dict[kernel_name]['kernel'])[0],
        ...
        assert np.shape(X)[0] == np.shape(x_labels)[
            0], 'Weight Matrix must have the same length as the weight labels'
    ...
    def add_kernel(
        ...
        if num_weights is None:
            if kernel_length == 0:
                ...
                kernel_length_samples = int(
                    np.ceil(self.ophys_frame_rate * kernel_length))
        else:
...
def split_by_engagement(design, run_params, session, fit):
    ...
    fit['engaged_trace_arr'] = fit['fit_trace_arr'][fit['engaged'].astype(
        bool), :]
    fit['disengaged_trace_arr'] = fit['fit_trace_arr'][~fit['engaged'].astype(
        bool), :]
    fit['engaged_trace_timestamps'] = fit['fit_trace_timestamps'][fit['engaged'].astype(
        bool)]
    fit['disengaged_trace_timestamps'] = fit['fit_trace_timestamps'][~fit['engaged'].astype(
        bool)]
    ...
    fit['fit_trace_timestamps'] = fit[run_params['engagement_preference'] +
                                      '_trace_timestamps']
...
def split_time(timebase, subsplits_per_split=10, output_splits=6):
    ...
    random_subsplit_inds = np.random.choice(
        np.arange(total_splits), size=total_splits, replace=False)
    subsplit_inds_per_split = np.array_split(
        random_subsplit_inds, output_splits)

    output_split_inds = []
    for ind_output in range(output_splits):
        subsplits_this_split = subsplit_inds_per_split[ind_output]
        inds_this_split = np.concatenate(
            [split_inds[sub_ind] for sub_ind in subsplits_this_split])
...
def toeplitz(events, kernel_length_samples, offset_samples):
    ...
    if offset_samples < 0:
        this_kernel = np.concatenate(
            [np.zeros((this_kernel.shape[0], np.abs(offset_samples))), this_kernel], axis=1)
        this_kernel = np.roll(this_kernel, offset_samples)[
            :, np.abs(offset_samples):]
    elif offset_samples > 0:
        this_kernel = np.concatenate([this_kernel, np.zeros(
            (this_kernel.shape[0], offset_samples))], axis=1)
...
def get_ophys_frames_to_use(session, end_buffer=0.5, stim_dur=0.25):
    ...
    '''
    # filter out omitted flashes to avoid omitted flashes at the start of the
    # session from affecting analysis range
...
def get_events_arr(session, timestamps_to_use):
    ...
    # Intentionally setting the name of the time axis to fit_trace_timestamps
    # so it matches the fit_trace_arr
...
def get_dff_arr(session, timestamps_to_use):
    ...
    #       The dataframes are built from xarrays to start with, so we are effectively converting them twice by doing this
    # But if there's no big time penalty to doing it this way, then maybe just
    # leave it be.
...
def variance_ratio(fit_trace_arr, W, X):
    ...
    '''
    Y = X.values @ W.values
    # Total variance in the ophys trace for each cell
    var_total = np.var(fit_trace_arr, axis=0)
    # Residual variance in the difference between the model and data
    var_resid = np.var(fit_trace_arr - Y, axis=0)
    # Fraction of variance explained by linear model
...
def masked_variance_ratio(fit_trace_arr, W, X, mask):
    ...
    # Total variance in the ophys trace for each cell
    var_total = my_var(fit_trace_arr, mask)
    # Residual variance in the difference between the model and data
    var_resid = my_var(fit_trace_arr - Y, mask)
    # Fraction of variance explained by linear model
    return (var_total - var_resid) / var_total
...
def error_by_time(fit, design):
    ...
    plt.figure()
    Y = design.get_X(
    ).values @ fit['dropouts']['Full']['cv_var_weights'][:, :, 0]
