...
def get_versions(vrange=[15, 20]):
    ...
    for dex, val in enumerate(np.arange(vrange[0], vrange[1])):
        out_versions = out_versions + \
            [x for x in versions if x.startswith('v_' + str(val) + '_')]
...
def make_run_json(
    ...
    experiment_table_path = os.path.join(
    ...
    python_fit_script = os.path.join(
        model_freeze_dir,
        'fit_glm_v_' + str(VERSION) + '.py')
    if src_path is None:
    ...
    shutil.copyfile(
        os.path.join(
            src_path,
            'visual_behavior_glm/GLM_fit_tools.py'),
    ...
    run_params = {
        ...
        # Min/Max L2 values for L2_optimize_by_cell, or L2_optimize_by_session
        'L2_grid_range': [.1, 500],
        # Number of L2 values for L2_optimize_by_cell, or
        # L2_optimize_by_session
        'L2_grid_num': 40,
        'L2_grid_type': 'linear',        # how to space L2 options, must be: 'log' or 'linear'
        # Fixed values to use for optimize_by_cre
        'L2_cre_values': {'Slc17a7-IRES2-Cre': 340, 'Vip-IRES-Cre': 320, 'Sst-IRES-Cre': 185},
        'ophys_experiment_ids': experiment_table.index.values.tolist(),
        'job_settings': job_settings,
        # If True, uses 'engagement_preference' to determine what engagement
        # state to use
        'split_on_engagement': False,
        # Either None, "engaged", or "disengaged". Must be None if
        # split_on_engagement is False
        'engagement_preference': None,
        # Minimum time, in seconds, the session needs to be in the preferred
        # engagement state
        'min_engaged_duration': 600,
        # The minimum duration of time between two licks to segment them into
        # separate lick bouts
        'lick_bout_ILI': 0.7,
        # length of bout event that continues after last lick in bout
        'min_time_per_bout': 0.2,
        # over-tiling value for making bout events. Must be << ophys-step-size
        'min_interval': 0.01,
        'CV_splits': 5,
        'CV_subsplits': 10,
        'eye_blink_z': 5.0,             # Threshold for excluding likely blinks
        # Threshold for excluding eye transients after blink removal
        'eye_transient_threshold': 0.5,
        ...
        # If 1, has no effect. Scales running speed to be O(1).
        'max_run_speed': 100,
        # If True, use detected events. If False, use raw deltaF/F
        'use_events': True,
        # If True, will fit to ROIs deemed invalid by the SDK. Note that the
        # SDK provides dff traces, but not events, for invalid ROISs
        'include_invalid_rois': False,
        # If True, interpolates the cell activity trace onto stimulus aligned
        # timestamps
        'interpolate_to_stimulus': True,
        # Number of timesteps image kernels are allowed to overlap during
        # entire session.
        'image_kernel_overlap_tol': 5,
        'dropout_threshold': 0.005,      # Minimum variance explained by full model
        # Should be either 'production' (run everything), 'standard' (run
        # standard dropouts), 'minimal' (just full model)
        'version_type': 'production',
        # If True, fits mesoscope 4 areas x 2 depth data
        'include_4x2_data': include_4x2_data,
    }
    ...
    if run_params['L2_use_fixed_value'] and (
            run_params['L2_fixed_lambda'] is None):
        raise Exception(
            'L2_use_fixed_value is True, but have None for L2_fixed_lambda')
    if (not run_params['L2_use_fixed_value']) and (
            run_params['L2_fixed_lambda'] is not None):
        raise Exception(
            'L2_use_fixed_value is False, but L2_fixed_lambda has been set')
    ...
    if (a or b):
        ...
        assert len(run_params['L2_grid_range']
                   ) == 2, "Must have a minimum and maximum L2 grid option"
        assert run_params['L2_grid_type'] in [
            'log', 'linear'], "L2_grid_type must be log or linear"
    ...
    if run_params['split_on_engagement']:
        assert (run_params['engagement_preference'] == 'engaged') or (run_params['engagement_preference']
                                                                      == 'disengaged'), "Splitting on engagement, preference must be 'engaged' or 'disengaged'"
    ...
    # Check version type:
    assert run_params['version_type'] in ['production', 'standard',
                                          'minimal'], "Incorrect value for version type. Define what model analyses to run."
...
def process_kernels(kernels):
    ...
    if ('each-image' in kernels) & ('any-image' in kernels):
        raise Exception(
            'Including both each-image and any-image kernels makes the model unstable')
    ...
    if 'each-image_change' in kernels:
        ...
        for index, val in enumerate(range(0, 8)):
            ...
            kernels['image_change' +
                    str(val)]['event'] = 'image_change' + str(val)
...
def define_dropouts(kernels, run_params):
    ...
    if run_params['version_type'] in ['production', 'standard']:
        ...
        for kernel in [
                kernel for kernel in kernels.keys() if kernels[kernel]['dropout']]:
            dropouts[kernel] = {
                'kernels': list(
                    kernels.keys()),
                'dropped_kernels': [],
        ...
        if 'face_motion_PC_0' in kernels:
            dropout_definitions['face_motion_energy'] = [
                kernel for kernel in list(
                    kernels.keys()) if kernel.startswith('face_motion')]
            dropout_definitions['behavioral'] = dropout_definitions['behavioral'] + \
                dropout_definitions['face_motion_energy']

        # For each nested model, move the appropriate kernels to the
        # dropped_kernel list
        for dropout_name in dropout_definitions:
            dropouts = set_up_dropouts(
                dropouts,
                kernels,
                dropout_name,
                dropout_definitions[dropout_name])
    ...
    if run_params['version_type'] == 'production':
        for drop in [drop for drop in dropouts.keys()]:
            if (drop != 'Full') & (drop != 'intercept'):
                ...
                kernels = set(dropouts['Full']['kernels']) - \
                    set(dropouts[drop]['kernels'])
                kernels.add('intercept')  # We always include the intercept
                dropped_kernels = set(dropouts['Full']['kernels']) - kernels
                dropouts['single-' + drop] = {'kernels': list(
                    kernels), 'dropped_kernels': list(dropped_kernels), 'is_single': True}

    # Check to make sure no kernels got lost in the mix
    for drop in dropouts.keys():
        assert len(
            dropouts[drop]['kernels']) + len(
            dropouts[drop]['dropped_kernels']) == len(
            dropouts['Full']['kernels']), 'bad length'
...
def load_run_json(version):
    ...
    if 'figure_dir' not in run_params:
        run_params['figure_dir'] = os.path.join(
            run_params['output_dir'], 'figures')
    if 'fig_coding_dir' not in run_params:
        run_params['fig_coding_dir'] = os.path.join(
            run_params['figure_dir'], 'coding')
        run_params['fig_kernels_dir'] = os.path.join(
            run_params['figure_dir'], 'kernels')
        run_params['fig_overfitting_dir'] = os.path.join(
            run_params['figure_dir'], 'over_fitting_figures')
        run_params['fig_clustering_dir'] = os.path.join(
            run_params['figure_dir'], 'clustering')
...
def describe_model_version(version):
    ...
    for kernel in run_params['kernels']:
        ...
        if run_params['kernels'][kernel]['type'] == 'discrete':
            ...
            print(
                kernel.ljust(18) +
                " runs the full length of the session, and is " +
                text)
    ...
    for d in run_params['dropouts']:
        ...
        if is_single:
            ...
            if len(k) > 1:
                print(
                    d.ljust(25) +
                    " contains just the kernels " +
                    ', '.join(k))
            else:
                print(
                    d.ljust(25) +
                    " contains just the kernel " +
                    ', '.join(k))
        else:
