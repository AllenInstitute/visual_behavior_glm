...
def define_levels():
    ...
    levels = {
        ...
        num_levels - 2: ['licking', 'task', 'face_motion_energy', 'pupil_and_running', 'all-images', 'beh_model',
                         'expectation'],
...
def define_kernels():
    kernels = {
        ...
        'time': {'event': 'time', 'type': 'continuous', 'length': 0, 'offset': 0, 'dropout': True,
                 'text': 'linear ramp from 0 to 1'},
        'licks': {'event': 'licks', 'type': 'discrete', 'length': 4, 'offset': -2, 'dropout': True,
                  'text': 'mouse lick'},
        # 'lick_bouts':   {'event':'lick_bouts',  'type':'discrete',      'length':4,     'offset':-2,    'dropout':True, 'text': 'lick bout'},
        'hits': {'event': 'hit', 'type': 'discrete', 'length': 5.5, 'offset': -1, 'dropout': True,
                 'text': 'lick to image change'},
        'misses': {'event': 'miss', 'type': 'discrete', 'length': 5.5, 'offset': -1, 'dropout': True,
                   'text': 'no lick to image change'},
        'passive_change': {'event': 'passive_change', 'type': 'discrete', 'length': 5.5, 'offset': -1, 'dropout': True,
                           'text': 'passive session image change'},
        'false_alarms': {'event': 'false_alarm', 'type': 'discrete', 'length': 5.5, 'offset': -1, 'dropout': True,
                         'text': 'lick on catch trials'},
        'correct_rejects': {'event': 'correct_reject', 'type': 'discrete', 'length': 5.5, 'offset': -1, 'dropout': True,
                            'text': 'no lick on catch trials'},
        'omissions': {'event': 'omissions', 'type': 'discrete', 'length': 2.5, 'offset': 0, 'dropout': True,
                      'text': 'image was omitted'},
        'each-image': {'event': 'each-image', 'type': 'discrete', 'length': 0.767, 'offset': -0.25, 'dropout': True,
    ...
    for PC in range(5):
        kernels['face_motion_PC_{}'.format(PC)] = {'event': 'face_motion_PC_{}'.format(PC), 'type': 'continuous',
                                                   'length': 2, 'offset': -1, 'dropout': True,
                                                   'text': 'PCA from face motion videos'}
    return kernels
...
def define_dropouts(kernels):
    ...
    dropout_definitions = {
        'visual': ['image0', 'image1', 'image2', 'image3', 'image4', 'image5', 'image6', 'image7', 'omissions',
                   'image_expectation'],
        'all-images': ['image0', 'image1', 'image2', 'image3', 'image4', 'image5', 'image6', 'image7'],
        'expectation': ['image_expectation', 'omissions'],
        'cognitive': ['hits', 'misses', 'false_alarms', 'correct_rejects', 'passive_change', 'change', 'rewards',
                      'model_bias', 'model_task0', 'model_timing1D', 'model_omissions1'],
    ...
    if 'face_motion_PC_0' in kernels:
        dropout_definitions['face_motion_energy'] = [kernel for kernel in list(kernels.keys()) if
                                                     kernel.startswith('face_motion')]
        dropout_definitions['behavioral'] = dropout_definitions['behavioral'] + dropout_definitions[
            'face_motion_energy']
    ...
    for drop in [drop for drop in dropouts.keys()]:
        if (drop != 'Full') & (drop != 'intercept'):
            ...
            dropouts['single-' + drop] = {'kernels': list(kernels), 'dropped_kernels': list(dropped_kernels),
                                          'is_single': True}

            # Check to make sure no kernels got lost in the mix
    for drop in dropouts.keys():
        assert len(dropouts[drop]['kernels']) + len(dropouts[drop]['dropped_kernels']) == len(
...
def describe_model_version(version):
    ...
    for kernel in run_params['kernels']:
        ...
        if run_params['kernels'][kernel]['type'] == 'discrete':
            ...
            print(kernel.ljust(18) + " is aligned from (" + str(start) + ", " + str(
                end) + ") seconds around each " + text)
        else:
