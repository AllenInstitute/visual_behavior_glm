...
def define_kernels():
    kernels = {
        ...
        'omissions_within':    {'event': 'omissions',   'type': 'discrete',      'length': .75,   'offset': 0,     'dropout': True, 'text': 'image was omitted'},
        'omissions_post':    {'event': 'omissions',   'type': 'discrete',      'length': 1.75,   'offset': .75,     'dropout': True, 'text': 'post omitted image'},
    ...
    for PC in range(5):
        kernels['face_motion_PC_{}'.format(PC)] = {'event': 'face_motion_PC_{}'.format(
            PC), 'type': 'continuous', 'length': 2, 'offset': -1, 'dropout': True, 'text': 'PCA from face motion videos'}
    return kernels
...
def make_run_json(VERSION, label='', username=None, src_path=None, TESTING=False):
    ...
    shutil.copyfile(os.path.join(
        src_path, 'visual_behavior_glm/GLM_fit_tools.py'),   python_file_full_path)
    shutil.copyfile(os.path.join(
        src_path, 'scripts/fit_glm.py'),     python_fit_script)

    # Define list of experiments to fit
    experiment_table = get_experiment_table()
    if TESTING:
        experiment_table = experiment_table.query(
            'project_code == "VisualBehavior"').tail(5)
    ...
    run_params = {
        ...
        'L2_grid_range': [.1, 500],
        # Number of L2 values for L2_optimize_by_cell, or L2_optimize_by_session
        'L2_grid_num': 40,
        ...
        # The minimum duration of time between two licks to segment them into separate lick bouts
        'lick_bout_ILI': 0.7,
    ...
    assert (a or b or c) and not ((a and b) or (b and c) or (
        a and c)), "Must select one and only on L2 option: L2_optimize_by_cell, L2_optimize_by_session, or L2_use_fixed_value"
...
def define_dropouts(kernels):
    ...
    dropouts = {'Full': {'kernels': list(
        kernels.keys()), 'dropped_kernels': [], 'is_single': False}}
    for kernel in [kernel for kernel in kernels.keys() if kernels[kernel]['dropout']]:
        dropouts[kernel] = {'kernels': list(
            kernels.keys()), 'dropped_kernels': [], 'is_single': False}
    ...
    if 'face_motion_PC_0' in kernels:
        dropout_definitions['face_motion_energy'] = [kernel for kernel in list(
    ...
    for dropout_name in dropout_definitions:
        dropouts = set_up_dropouts(
            dropouts, kernels, dropout_name, dropout_definitions[dropout_name])
    ...
    for drop in dropouts.keys():
        assert len(dropouts[drop]['kernels']) + len(dropouts[drop]
                                                    ['dropped_kernels']) == len(dropouts['Full']['kernels']), 'bad length'
...
def set_up_dropouts(dropouts, kernels, dropout_name, kernel_list):
    ...
    '''
    dropouts[dropout_name] = {'kernels': list(
        kernels.keys()), 'dropped_kernels': [], 'is_single': False}
...
def load_run_json(version):
    ...
    json_path = os.path.join(OUTPUT_DIR_BASE, 'v_' +
                             str(version), 'run_params.json')
    print(json_path)
...
def describe_model_version(version):
    ...
    for kernel in run_params['kernels']:
        ...
        if run_params['kernels'][kernel]['type'] == 'discrete':
            ...
            print(kernel.ljust(18) + " is aligned from ("+str(start) +
                  ", "+str(end)+") seconds around each "+text)
        else:
            print(kernel.ljust(18) +
                  " runs the full length of the session, and is " + text)
