...
def analysis(
    ...
    # Plot 1D summaries
    out1 = gst.strategy_kernel_comparison(
        weights_beh.query('visual_strategy_session'),
        run_params,
        kernel,
        session_filter=[experience_level])
    out2 = gst.strategy_kernel_comparison(
        weights_beh.query('not visual_strategy_session'),
        run_params,
        kernel,
        session_filter=[experience_level])
    ...
    if savefig:
        filepath = run_params['figure_dir'] + '/strategy/' + \
            kernel + '_visual_comparison_{}.svg'.format(experience_level)
        print('Figure saved to: ' + filepath)
        out1[1].savefig(filepath)
        filepath = run_params['figure_dir'] + '/strategy/' + \
            kernel + '_timing_comparison_{}.svg'.format(experience_level)
        print('Figure saved to: ' + filepath)
    ...
    for index, row in df.iterrows():
        ...
        if show_steps:
            ...
            ax.fill(points[hull.vertices, 0],
                    points[hull.vertices, 1], color=color)
...
def get_perturbation(
        ...
        kernel,
        experience_level="Familiar"):
    visual = get_kernel_averages(
        weights_df.query('visual_strategy_session'),
        run_params,
        kernel,
        session_filter=[experience_level])
    timing = get_kernel_averages(
        weights_df.query('not visual_strategy_session'),
        run_params,
        kernel,
        session_filter=[experience_level])
...
def plot_multiple(weights_df, run_params, savefig=False):
    ...
    for edex, e in enumerate(['Familiar', 'Novel 1', 'Novel >1']):
        for kdex, k in enumerate(
                ['omissions', 'hits', 'misses', 'all-images']):
...
def plot_perturbation(
    ...
    visual, timing = get_perturbation(
        weights_df, run_params, kernel, experience_level)
    time = visual['time']
    offset = 0
    multiimage = kernel in ['hits', 'misses', 'omissions']
    if multiimage:
    ...
    if ax is None:
        fig, ax = plt.subplots(
            1, 1, sharey=True, sharex=True, figsize=(
                4.5, 4))
    ...
    if savefig:
        filepath = run_params['figure_dir'] + '/strategy/' + kernel + \
            '_strategy_perturbation_{}_{}_{}.svg'.format(experience_level, x, y)
        print('Figure saved to: ' + filepath)
        plt.savefig(filepath)


def plot_perturbation_3D(
        weights_df,
        run_params,
        kernel,
        session='Familiar',
    ...
    if savefig:
        filepath = run_params['figure_dir'] + '/strategy/' + \
            kernel + '_strategy_perturbation_3D_{}.svg'.format(session)
        print('Figure saved to: ' + filepath)
        plt.savefig(filepath)
...
def get_average_kernels_inner(df):
    ...
    return df_norm.mean(axis=0), df_norm.std(
        axis=0) / np.sqrt(np.shape(df_norm)[0])


def plot_PSTH_perturbation(
        dfs,
        labels,
        condition,
        run_params=None,
