...
import visual_behavior_glm.GLM_dataset as gd
...
def get_strategy():
    data_f_visual = get_dataset(
        image_set='familiar',
        normalize='none',
        other='visual')
    data_n_visual = get_dataset(
        image_set='novel',
        normalize='none',
        other='visual')
    data_p_visual = get_dataset(
        image_set='novelp',
        normalize='none',
        other='visual')
    data_f_timing = get_dataset(
        image_set='familiar',
        normalize='none',
        other='timing')
    data_n_timing = get_dataset(
        image_set='novel',
        normalize='none',
        other='timing')
    data_p_timing = get_dataset(
        image_set='novelp',
        normalize='none',
        other='timing')
...
def save_strategy_data_summary(run_params, weights_df):
    ...
    weights_beh_visual = weights_beh.query(
        '(strategy == "visual")&(strategy_matched)').copy()
    weights_beh_timing = weights_beh.query(
        '(strategy == "timing")&(strategy_matched)').copy()
    gd.save_data_summary(
        run_params,
        weights_beh_visual,
        normalize='none',
        other='visual')
...
def plot_3D(dataset, kernels, ax=None, color='r', alpha=1, label=None):
    ...
    line = ax.plot3D(dataset[kernels][CRE[0]],
                     dataset[kernels][CRE[1]],
                     dataset[kernels][CRE[2]],
                     linewidth=2,
                     color=color,
                     alpha=alpha,
                     label=label)
    ...
    ax.scatter(dataset[kernels][CRE[0]][im],
               dataset[kernels][CRE[1]][im],
               dataset[kernels][CRE[2]][im],
               c=dataset[kernels]['time'][im],
               cmap='autumn_r',
               edgecolor='k',
               alpha=1)
    ax.set_xlabel(CRE[0])
...
def plot_proj(dataset, kernels, axes=None, cmap='autumn_r', arrow=False):
    ...
    plot_colored_time2(axes[0, 0], dataset, kernels,
                       CRE[0], CRE[1], cmap, arrow, im)
    plot_colored_time2(axes[0, 1], dataset, kernels,
                       CRE[0], CRE[2], cmap, arrow, im)
    plot_colored_time2(axes[0, 2], dataset, kernels,
                       CRE[1], CRE[2], cmap, arrow, im)

    #plot_colored_time2(axes[1,0], dataset, kernels, CRE[0], CRE[1], cmap, arrow, im)
    plot_colored_time(axes[1,
                           0],
                      dataset[kernels][CRE[0]] - dataset[kernels][CRE[1]],
                      dataset[kernels][CRE[2]],
                      dataset[kernels]['time'],
                      cmap=cmap,
                      arrow=arrow)
    axes[1,
         0].scatter(dataset[kernels][CRE[0]][im] - dataset[kernels][CRE[1]][im],
                    dataset[kernels][CRE[2]][im],
                    c=dataset[kernels]['time'][im],
                    cmap=cmap,
                    edgecolor='k')
    ...
    plot_colored_time(axes[1,
                           1],
                      dataset[kernels][CRE[0]],
                      dataset[kernels][CRE[1]] - dataset[kernels][CRE[2]],
                      dataset[kernels]['time'],
                      cmap=cmap,
                      arrow=arrow)
    axes[1,
         1].scatter(dataset[kernels][CRE[0]][im],
                    dataset[kernels][CRE[1]][im] - dataset[kernels][CRE[2]][im],
                    c=dataset[kernels]['time'][im],
                    cmap=cmap,
                    edgecolor='k')
    ...
    plot_colored_time(axes[1,
                      ...
                      arrow=False)
    axes[1,
         2].scatter(dataset[kernels]['time'][im],
                    dataset[kernels][CRE[0]][im],
                    c=dataset[kernels]['time'][im],
                    cmap=cmap,
                    edgecolor='k')
