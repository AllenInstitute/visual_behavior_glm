...
def plot_kernels_by_strategy_by_session(
        weights_beh,
        run_params,
        ym='omissions',
        cre_line='Vip-IRES-Cre',
        compare=['strategy_labels'],
        savefig=False):
    ...
    fig, ax = plt.subplots(
        2, len(sessions), figsize=(
            len(sessions) * 4, 6), sharey=True)
    for dex, session in enumerate(sessions):
        show_legend = dex == len(sessions) - 1
        out = strategy_kernel_comparison(weights_beh,
                                         run_params,
                                         ym,
                                         threshold=0,
                                         drop_threshold=0,
                                         session_filter=[session],
                                         cell_filter=cre_line,
                                         area_filter=['VISp'],
                                         compare=compare,
                                         plot_errors=True,
                                         save_kernels=False,
                                         ...
                                         filter_sessions_on=filter_sessions_on,
                                         image_set=image_set)
        out = strategy_kernel_comparison(weights_beh,
                                         run_params,
                                         ym,
...
def kernels_by_cre(
    ...
    for dex, cre in enumerate(cres):
        ...
        out = strategy_kernel_comparison(
            weights_beh,
            run_params,
            kernel,
            threshold=0,
...
def compare_cre_kernels(
    ...
    if savefig:
        filename = ym + '_by_cre_line_' + \
            '_'.join(compare) + '_' + equipment_filter
        save_figure(fig, run_params['version'], ym, filename)
    return ax
...
def strategy_kernel_comparison(
    ...
    if ax is None:
        ...
        if kernel == 'all_images':
            h = [Size.Fixed(pre_horz_offset), Size.Fixed(
                (width - pre_horz_offset - post_horz_offset) / 3 * .75)]
        else:
            h = [Size.Fixed(pre_horz_offset),
                 Size.Fixed(width - pre_horz_offset - post_horz_offset)]
        v = [
    ...
    for dex, group in enumerate(groups):
        ...
        k = strategy_kernel_comparison_inner(
            ax,
            weights_dfiltered,
            group,
            color,
...
def strategy_kernel_evaluation(
        weights_df,
        run_params,
        kernel,
        save_results=False,
    ...
    sst_table = weights.query(
        'cre_line=="Sst-IRES-Cre"')[[kernel + '_weights', kernel, 'strategy_dropout_index']]
    vip_table = weights.query(
        'cre_line=="Vip-IRES-Cre"')[[kernel + '_weights', kernel, 'strategy_dropout_index']]
    slc_table = weights.query(
        'cre_line=="Slc17a7-IRES2-Cre"')[[kernel + '_weights', kernel, 'strategy_dropout_index']]
    ncells = {
        'vip': len(vip_table),
        'sst': len(sst_table),
        'exc': len(slc_table),
    ...
    zlims_test = strategy_kernel_heatmap_with_dropout(
        vip_table,
        sst_table,
        slc_table,
        time_vec,
...
def strategy_kernel_heatmap_with_dropout(
        vip_table,
        sst_table,
        slc_table,
        time_vec,
    ...
    cbar2 = ax2.imshow(sst_weights_sorted.T, aspect='auto', extent=[
                       time_vec[0], time_vec[-1], 0, np.shape(sst_weights_sorted)[1]], cmap='bwr')
    cbar2.set_clim(zlims[0], zlims[1])
    cbar3 = ax3.imshow(slc_weights_sorted.T, aspect='auto', extent=[
                       time_vec[0], time_vec[-1], 0, np.shape(vip_weights_sorted)[1]], cmap='bwr')
...
def scatter_dataset(
        ...
        threshold=0,
        ymetric_threshold=0,
        xmetric='strategy_dropout_index',
        ymetric='omissions',
        depth=[
            0,
            1000],
    ...
    for dex, cre in enumerate(cres):
        col_start = dex == 0
        fits[cre] = scatter_by_experience(results_beh,
                                          run_params,
                                          cre_line=cre,
                                          threshold=threshold,
                                          ymetric_threshold=ymetric_threshold,
                                          xmetric=xmetric,
                                          ymetric=ymetric,
                                          ax=ax[dex,
                                                :],
                                          ...
                                          area=area,
                                          use_prior_image_set=use_prior_image_set)
...
def scatter_by_experience(
    ...
    for dex, s in enumerate(sessions):
        row_start = dex == 0
        fits[str(s)] = scatter_by_cell(results_beh,
                                       run_params,
                                       cre_line=cre_line,
                                       threshold=threshold,
                                       ymetric_threshold=ymetric_threshold,
...
def scatter_by_cell(
    ...
    ax.plot(np.array([xpoints, xpoints]), np.array(
        [ypoints - y_sem, ypoints + y_sem]), 'r-')

    # Plot Linear regression
    x = linregress(g[xmetric], g[ymetric])
    label = 'r = ' + str(np.round(x.rvalue, 4)) + '\np = ' + \
        str(np.round(x.pvalue, decimals=4))
...
def plot_dropout_summary_population(
    ...
    for index, cre in enumerate(cre_lines):
        ...
        if dropout_cleaning_threshold is not None:
            ...
            extra = ''
        if savefig:
            filename = run_params['figure_dir'] + '/strategy/' + \
                cre + ' dropout_summary_boxplot' + extra + '.svg'
            print('Figure saved to: ' + filename)
            plt.savefig(filename)
...
def plot_fraction_summary_population(
    ...
    cre_lines = ['Vip-IRES-Cre', 'Sst-IRES-Cre', 'Slc17a7-IRES2-Cre']
    results_pivoted['cre_line_strat'] = [
        x[0] +
        ' ' +
        x[1] for x in zip(
            results_pivoted['cre_line'],
            results_pivoted['strategy_labels'])]
    ...
    if kernel_excitation:
        ...
        summary_df = results_pivoted.groupby(['cre_line_strat',
                                              'experience_level'])[['code_anything',
                                                                    'code_' + kernel,
                                                                    'code_' + kernel + '_excited',
                                                                    'code_' + kernel + '_inhibited']].mean()
        summary_df['n'] = results_pivoted.groupby(
            [
                'cre_line_strat',
                'experience_level'])[
            [
                ...
                'code_' +
                kernel +
                '_inhibited']].count()['code_anything']
    ...
    if kernel_excitation:
        ...
        fig, ax = plt.subplots(
            3, len(coding_groups), figsize=(
                10.8, 8), sharey=sharey)
    for index, feature in enumerate(coding_groups):
        ...
        ax[0,
           index].plot([0,
                        1,
                        2],
                       summary_df.loc['Vip-IRES-Cre visual'][feature],
                       '-',
                       color=colors['visual'],
                       label='Vip visual',
                       linewidth=3)
        ax[0,
           index].plot([0,
                        1,
                        2],
                       summary_df.loc['Vip-IRES-Cre timing'][feature],
                       '-',
                       color=colors['timing'],
                       label='Vip timing',
                       linewidth=3)
        ax[1,
           index].plot([0,
                        1,
                        2],
                       summary_df.loc['Sst-IRES-Cre visual'][feature],
                       '-',
                       color=colors['visual'],
                       label='Sst visual',
                       linewidth=3)
        ax[1,
           index].plot([0,
                        1,
                        2],
                       summary_df.loc['Sst-IRES-Cre timing'][feature],
                       '-',
                       color=colors['timing'],
                       label='Sst timing',
                       linewidth=3)
        ax[2,
           index].plot([0,
                        1,
                        2],
                       summary_df.loc['Slc17a7-IRES2-Cre visual'][feature],
                       '-',
                       color=colors['visual'],
                       label='Exc visual',
                       linewidth=3)
        ax[2,
           index].plot([0,
                        1,
                        2],
                       summary_df.loc['Slc17a7-IRES2-Cre timing'][feature],
                       '-',
                       color=colors['timing'],
                       label='Exc timing',
                       linewidth=3)
    ...
    if savefig:
        if kernel_excitation:
            filename = run_params['figure_dir'] + \
                '/strategy/coding_fraction_' + kernel + '_summary.svg'
            plt.savefig(filename)
            print('Figure saved to: ' + filename)
        else:
            filename = run_params['figure_dir'] + \
                '/strategy/coding_fraction_summary.svg'
            plt.savefig(filename)
            print('Figure saved to: ' + filename)
    return summary_df
            ...
            for cindex, cell_type in enumerate(cell_types):
                for eindex, exp in enumerate(experience_levels):
                    ...
                    if ttest.pvalue < 0.001:
                        ...
                        ax[cindex].plot(
                            eindex + .1, y1h, '*', color=stats_color)
                        ax[cindex].plot(
                            eindex - .1, y1h, '*', color=stats_color)
