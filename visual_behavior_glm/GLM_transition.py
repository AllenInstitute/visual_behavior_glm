...
def clean_table(df):
    ...
    '''
    df['identifier'] = [str(x[0]) + '_' + str(x[1])
                        for x in zip(df['cell_specimen_id'], df['session_number'])]
...
def build_pivot_table(cre='all'):
    ...
    df_pivot['never_tracked'] = df_pivot[[
        '1', '2', '3', '4', '5', '6']].isnull().sum(axis=1) == 5
    df_pivot['full_tracked'] = df_pivot[[
        '1', '2', '3', '4', '5', '6']].isnull().sum(axis=1) == 0
    df_pivot['full_novel_tracked'] = (df_pivot[['4', '5', '6']].isnull().sum(
        axis=1) == 0) & (df_pivot[['1', '2', '3']].isnull().sum(axis=1) == 3)
    df_pivot['full_familiar_tracked'] = (df_pivot[['1', '2', '3']].isnull().sum(
        axis=1) == 0) & (df_pivot[['4', '5', '6']].isnull().sum(axis=1) == 3)
    df_pivot['partial_novel_tracked'] = (df_pivot[['4', '5', '6']].isnull().sum(
        axis=1) < 3) & (df_pivot[['1', '2', '3']].isnull().sum(axis=1) == 3)
    df_pivot['partial_familiar_tracked'] = (df_pivot[['1', '2', '3']].isnull().sum(
        axis=1) < 3) & (df_pivot[['4', '5', '6']].isnull().sum(axis=1) == 3)
    return df_pivot
...
def compute_distribution(df_pivot, session, session_dists={}, dropna=False):
    ...
    '''
    # do the count
    session_dists[str(session)] = df_pivot[str(
        session)].value_counts(dropna=dropna)
    ...
    session_dists[str(session)] = session_dists[str(
        session)].sort_index(na_position='first')
    session_dists['normalized_' + str(session)] = session_dists[str(
        session)] / sum(session_dists[str(session)])
...
def compute_all_distributions(df_pivot):
    ...
    for i in range(1, 7):
        session_dists = compute_distribution(
            df_pivot, i, session_dists=session_dists)
        session_dists_no_nans = compute_distribution(
            df_pivot, i, session_dists=session_dists_no_nans, dropna=True)
...
def build_transition_matrix(df_pivot, sessions):
    ...
    # find tracked cells
    df_pivot_sessions = df_pivot[~df_pivot[sessions[0]
                                           ].isnull() & ~df_pivot[sessions[1]].isnull()]
...
def transitions_by_cre_inner(df_pivot, ax, cmap):
    ...
    for sdex, s in enumerate(sessions):
        ...
        plot_transition_matrix(
            T, [str(s), str(s + 1)], ax=ax[s - 1], compact=True, cmap=cmap)
...
def build_estimate(df_pivot, sessions, session_dists):
    ...
    session_dists['estimate_' + str(sessions[0]) + '_to_' + str(sessions[1])] = np.dot(
        normalize_matrix(T), session_dists['normalized_' + str(sessions[0])])
    session_dists['estimate_ci_' +
                  str(sessions[0]) +
                  '_to_' +
                  str(sessions[1])] = np.dot(normalize_matrix(T), session_dists['ci_' +
                                                                                str(sessions[0])])
    return session_dists
...
def build_all_estimates(session_dists, df_pivot):
    ...
    for sdex, s in enumerate(sessions):
        for edex, e in enumerate(sessions[:sdex]):
            session_dists = build_estimate(
                df_pivot, [str(e), str(s)], session_dists)
    return session_dists
...
def plot_estimate(
    ...
    if transition_query == '':
        ...
        labels[1] += ' \nTransition  = all cells tracked ' + \
            str(sessions[0]) + ',' + str(sessions[1])
    else:
        df_pivot_transition = build_pivot_table(
            cre=cre).query(transition_query)
    ...
    # Build distributions
    session_dists_with_nans, in_session_dists = compute_all_distributions(
        df_pivot_in_dist)
    est_session_dists = build_all_estimates(
        in_session_dists, df_pivot_transition)
    out_session_dists_with_nans, out_session_dists = compute_all_distributions(
        df_pivot_out_dist)

    # plot
    title = 'Estimating Session ' + \
        str(sessions[1]) + ' from Session ' + str(sessions[0])
    savefile = 'estimate_session_' + str(sessions[1]) + '_from_session_' + str(sessions[0]) + '_' + cre + \
        '_transition_' + transition_query + '_input_' + in_dist_query + '_output_' + out_dist_query + '.png'
    plot_distribution_estimate(
        out_session_dists,
        est_session_dists,
        sessions,
        title=title,
...
def check_estimates(cre='all'):
    ...
    plot_estimate(
        cre=cre,
        transition_query='full_tracked',
        in_dist_query='full_familiar_tracked',
        out_dist_query='full_novel_tracked',
        sessions=[
            '3',
            '4'])
    plot_estimate(
        cre=cre,
        transition_query='full_tracked',
        in_dist_query='partial_familiar_tracked',
        out_dist_query='partial_novel_tracked',
        sessions=[
            '3',
            '4'])
    ...
    plot_estimate(
        cre=cre,
        in_dist_query='not tracked_34',
        out_dist_query='not tracked_34',
        sessions=[
            '3',
            '4'])
    ...
    plot_estimate(
        cre=cre,
        in_dist_query='not tracked_12',
        out_dist_query='not tracked_12',
        sessions=[
            '1',
            '2'])
    plot_estimate(
        cre=cre,
        in_dist_query='not tracked_23',
        out_dist_query='not tracked_23',
        sessions=[
            '2',
            '3'])
    plot_estimate(
        cre=cre,
        in_dist_query='not tracked_45',
        out_dist_query='not tracked_45',
        sessions=[
            '4',
            '5'])
    plot_estimate(
        cre=cre,
        in_dist_query='not tracked_56',
        out_dist_query='not tracked_56',
        sessions=[
            '5',
            '6'])
...
def check_transitions(cre='all'):
    ...
    for sdex, s in enumerate(sessions):
        ...
        for edex, e in enumerate(sessions[sdex + 1:]):
            ...
            plot_transition_matrix(
                T, [str(s), str(e)], ax=ax[s - 1, e - 2], compact=True)
...
def check_distributions(cre='all'):
    ...
    session_dists_13, session_dists_no_nans_13 = compute_all_distributions(
        df_pivot.query('tracked_13'))
    session_dists_12, session_dists_no_nans_12 = compute_all_distributions(
        df_pivot.query('tracked_12'))
    session_dists_34, session_dists_no_nans_34 = compute_all_distributions(
        df_pivot.query('tracked_34'))
    session_dists_n13, session_dists_no_nans_n13 = compute_all_distributions(
        df_pivot.query('not tracked_13'))
    session_dists_n12, session_dists_no_nans_n12 = compute_all_distributions(
        df_pivot.query('not tracked_12'))
    session_dists_n34, session_dists_no_nans_n34 = compute_all_distributions(
        df_pivot.query('not tracked_34'))

    session_dists_never, session_dists_no_nans_never = compute_all_distributions(
        df_pivot.query('never_tracked'))
    session_dists_full, session_dists_no_nans_full = compute_all_distributions(
        df_pivot.query('full_tracked'))

    session_dists_partial_novel, session_dists_no_nans_partial_novel = compute_all_distributions(
        df_pivot.query('partial_novel_tracked'))
    session_dists_partial_familiar, session_dists_no_nans_partial_familiar = compute_all_distributions(
        df_pivot.query('partial_familiar_tracked'))

    session_dists_full_novel, session_dists_no_nans_full_novel = compute_all_distributions(
        df_pivot.query('full_novel_tracked'))
    session_dists_full_familiar, session_dists_no_nans_full_familiar = compute_all_distributions(
        df_pivot.query('full_familiar_tracked'))

    # Result 1, are clusters different across sessions?
    plot_distribution_by_session(
        session_dists, [
    ...
    plot_distribution_by_session(
        session_dists_no_nans, [
            str(x) for x in range(
                1, 7)], dropna=True, title=cre)

    #plot_distribution_by_group([session_dists, session_dists_13],1,labels=['all','tracked 1,3'])
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_13,
                                session_dists_no_nans_n13],
                               1,
                               labels=['all',
                                       'tracked 1,3',
                                       'not tracked 1,3'],
    ...
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_12,
                                session_dists_no_nans_n12],
                               1,
                               labels=['all',
                                       'tracked 1,2',
                                       'not tracked 1,2'],
    ...
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_34,
                                session_dists_no_nans_n34],
                               3,
                               labels=['all',
                                       'tracked 3,4',
                                       'not tracked 3,4'],
    ...
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_never,
                                session_dists_no_nans_full],
                               1,
                               labels=['all',
                                       'never tracked',
                                       'full tracked'],
                               dropna=True,
                               title=cre + ' never full' + ' ' + str(1))
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_never,
                                session_dists_no_nans_full],
                               2,
    ...
    plot_distribution_by_group([session_dists_no_nans,
                               ...
                               labels=['all',
                                       ...
                                       'full tracked'],
                               dropna=True,
                               title=cre + ' never full' + ' ' + str(6))

    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_full_novel,
                                session_dists_no_nans_full_familiar],
                               1,
                               labels=['all',
                                       'full_novel tracked',
                                       'full_familiar tracked'],
                               dropna=True,
                               title=cre + ' full_novel full_familiar' + ' ' + str(1))
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_full_novel,
                                session_dists_no_nans_full_familiar],
                               2,
                               labels=['all',
                                       ...
                                       'full_familiar tracked'],
                               dropna=True,
                               title=cre + ' full_novel full_familiar' + ' ' + str(2))
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_full_novel,
                                session_dists_no_nans_full_familiar],
                               3,
                               labels=['all',
                                       ...
                                       'full_familiar tracked'],
                               dropna=True,
                               title=cre + ' full_novel full_familiar' + ' ' + str(3))
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_full_novel,
                                session_dists_no_nans_full_familiar],
                               4,
                               labels=['all',
                                       ...
                                       'full_familiar tracked'],
                               dropna=True,
                               title=cre + ' full_novel full_familiar' + ' ' + str(4))
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_full_novel,
                                session_dists_no_nans_full_familiar],
                               5,
                               labels=['all',
                                       ...
                                       'full_familiar tracked'],
                               dropna=True,
                               title=cre + ' full_novel full_familiar' + ' ' + str(5))
    plot_distribution_by_group([session_dists_no_nans,
                                session_dists_no_nans_full_novel,
                                session_dists_no_nans_full_familiar],
                               6,
                               labels=['all',
                                       ...
                                       'full_familiar tracked'],
                               dropna=True,
                               title=cre + ' full_novel full_familiar' + ' ' + str(6))

    plot_distribution_by_group(
        [
            session_dists_no_nans,
            session_dists_no_nans_partial_novel,
            session_dists_no_nans_partial_familiar],
        1,
        labels=[
            'all',
            'partial_novel tracked',
            'partial_familiar tracked'],
        dropna=True,
        title=cre +
        ' partial_novel partial_familiar' +
        ' ' +
        str(1))
    plot_distribution_by_group(
        [
...
def plot_distribution_by_session(
        session_dists,
        sessions,
        dropna=False,
        title=''):
...
def plot_transition_matrix(
    ...
    if compact:
        ...
        if normalize:
            cbar.set_label(
                'Prob(' + str(sessions[1]) + '|' + str(sessions[0]) + ')', fontsize=14)
        else:
...
def plot_distribution_estimate(
        measured_session_dists,
        estimate_session_dists,
        sessions,
        title='',
        labels=[],
        savefile=None):
    ...
    xvec = x + width * (1 + 1) - .35
    ax.bar(xvec, es['estimate_' + str(sessions[0]) +
           '_to_' + str(sessions[1])], width, label=labels[1])
    ax.plot(np.array([xvec, xvec]), es['estimate_ci_' +
            str(sessions[0]) + '_to_' + str(sessions[1])].T, 'k-')
