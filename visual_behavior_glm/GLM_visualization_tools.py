...
def plot_kernel_support(
    ...
    '''
    discrete = [
        x for x in glm.run_params['kernels'] if (
            glm.run_params['kernels'][x]['type'] == 'discrete') or (
            x == 'lick_model') or (
                x == 'groom_model')]
    continuous = [x for x in glm.run_params['kernels']
                  if glm.run_params['kernels'][x]['type'] == 'continuous']
    if include_cont:
    ...
    if 'rewards' in glm.run_params['kernels']:
        reward_dex = stim_points['rewards'][0] + dt * np.ceil(
            np.abs(glm.run_params['kernels']['rewards']['offset']) * frame_rate)
    elif 'hits' in glm.run_params['kernels']:
        reward_dex = stim_points['hits'][0] + dt * np.ceil(
            np.abs(glm.run_params['kernels']['hits']['offset']) * frame_rate)
    else:
        reward_dex = 0
    if plot_ticks:
        rewards = glm.session.rewards.query(
            'timestamps < @end_t & timestamps > @start_t')['timestamps']
        #plt.plot(rewards, reward_dex*np.ones(np.shape(rewards)),'ro')

    # Stimulus Presentations
    stim = glm.session.stimulus_presentations.query(
        'start_time > @start_t & start_time < @end_t & not omitted & not is_change')
    for index, time in enumerate(stim['start_time'].values):
        plt.axvspan(time, time + 0.25, color='k', alpha=.1)
    if plot_ticks:
        for index in range(0, 8):
            image = glm.session.stimulus_presentations.query(
                'start_time >@start_t & start_time < @end_t & image_index == @index')['start_time']
            image_dex = stim_points['image' + str(index)][0] + dt * np.ceil(
                np.abs(glm.run_params['kernels']['image' + str(index)]['offset']) * frame_rate)
            plt.plot(image, image_dex * np.ones(np.shape(image)), 'k|')

    # Stimulus Changes
    change = glm.session.stimulus_presentations.query(
        'start_time > @start_t & start_time < @end_t & is_change')
    if plot_ticks:
        if 'change' in glm.run_params['kernels']:
            change_dex = stim_points['change'][0] + dt * np.ceil(
                np.abs(glm.run_params['kernels']['change']['offset']) * frame_rate)
            plt.plot(
                change['start_time'],
                change_dex *
                np.ones(
    ...
    for index, time in enumerate(change['start_time'].values):
        plt.axvspan(time, time + 0.25, color=project_colors()
                    ['schematic_change'], alpha=.5, edgecolor=None)

    # Stimulus Omissions
    if plot_ticks:
        if 'omissions' in glm.run_params['kernels']:
            omitted = glm.session.stimulus_presentations.query(
                'start_time >@start_t & start_time < @end_t & omitted')['start_time']
            omitted_dex = stim_points['omissions'][0] + dt * np.ceil(
                np.abs(glm.run_params['kernels']['omissions']['offset']) * frame_rate)
            plt.plot(omitted, omitted_dex * np.ones(np.shape(omitted)), 'k|')
            for index, time in enumerate(omitted):
                plt.axvline(
                    time,
                    color=project_colors()['schematic_omission'],
                    linestyle='--',
                    zorder=-np.inf,
                    linewidth=1.5)
    ...
    if plot_ticks & ('image_expectation' in glm.run_params['kernels']):
        expectation = glm.session.stimulus_presentations.query(
            'start_time >@start_t & start_time < @end_t & not omitted')['start_time']
        expectation_dex = stim_points['image_expectation'][0] + dt * np.ceil(
            np.abs(glm.run_params['kernels']['image_expectation']['offset']) * frame_rate)
        plt.plot(
            expectation,
            expectation_dex *
            np.ones(
                np.shape(expectation)),
            'k|')

    # Licks
    if plot_ticks:
        licks = glm.session.licks.query(
            'timestamps < @end_t & timestamps > @start_t')['timestamps']

        if 'pre_lick_bouts' in glm.run_params['kernels']:
            bouts = glm.session.licks.query(
                'timestamps < @end_t & timestamps > @start_t & bout_start')['timestamps']
            pre_dex = stim_points['pre_lick_bouts'][0] + dt * np.ceil(
                np.abs(glm.run_params['kernels']['pre_lick_bouts']['offset']) * frame_rate)
            post_dex = stim_points['post_lick_bouts'][0] + dt * np.ceil(
                np.abs(glm.run_params['kernels']['post_lick_bouts']['offset']) * frame_rate)
        ...
        if 'lick_bouts' in glm.run_params['kernels']:
            bouts = glm.session.licks.query(
                'timestamps < @end_t & timestamps > @start_t & bout_start')['timestamps']
            dex = stim_points['lick_bouts'][0] + dt * np.ceil(
                np.abs(glm.run_params['kernels']['lick_bouts']['offset']) * frame_rate)
            plt.plot(bouts, dex * np.ones(np.shape(bouts)), 'k|')

        if 'pre_licks' in glm.run_params['kernels']:
            pre_dex = stim_points['pre_licks'][0] + dt * np.ceil(
                np.abs(glm.run_params['kernels']['pre_licks']['offset']) * frame_rate)
            post_dex = stim_points['post_licks'][0] + dt * np.ceil(
                np.abs(glm.run_params['kernels']['post_licks']['offset']) * frame_rate)
        ...
        if 'licks' in glm.run_params['kernels']:
            dex = stim_points['licks'][0] + dt * np.ceil(
                np.abs(glm.run_params['kernels']['licks']['offset']) * frame_rate)
    ...
    if plot_ticks:
        ...
        trials = glm.session.trials.query(
            'change_time < @end_t & change_time > @start_t')
        for index, t in enumerate(types):
            if ks[index] in glm.run_params['kernels']:
                try:
                    change_time = trials[trials[t]]['change_time']
                    trial_dex = stim_points[ks[index]][0] + dt * np.ceil(
                        np.abs(glm.run_params['kernels'][ks[index]]['offset']) * frame_rate)
                    plt.plot(
                        change_time,
                        trial_dex *
                        np.ones(
...
def plot_glm_version_comparison_histogram(
        comparison_table=None,
        results=None,
        versions_to_compare=None,
        savefig=True):
    ...
    if comparison_table is None:
        comparison_table = gat.get_glm_version_comparison_table(
            versions_to_compare=versions_to_compare, results=results)

    cre_lines = np.sort(comparison_table['cre_line'].dropna().unique())

    comparison_table['Diff'] = comparison_table[versions_to_compare[0]
                                                ] - comparison_table[versions_to_compare[1]]
    plt.figure()
    ...
    if savefig and (versions_to_compare is not None):
        version_strings = '_'.join([x.split('_')[0]
                                   for x in versions_to_compare])
        for version in versions_to_compare:
            run_params = glm_params.load_run_json(version)
            filepath = os.path.join(
                run_params['figure_dir'],
                'version_comparison_histogram_' +
                version_strings +
                '.png')
...
def plot_glm_version_comparison(
        comparison_table=None,
        results=None,
        versions_to_compare=None,
        savefig=True):
...
def plot_significant_cells(
    ...
    for i, c in enumerate(cre):
        ...
        num_cells = len(cells)
        cell_count = np.array([np.sum(cells.query(
            'session_number == @x')[dropout] < dropout_threshold) for x in sessions])
...
def plot_regressor_correlation(glm, add_lines=True, save_plot=False):
    ...
    discrete = [x for x in glm.run_params['kernels']
                if glm.run_params['kernels'][x]['type'] == 'discrete']
    ...
    if save_plot:
        filename = os.path.join(
            glm.run_params['figure_dir'],
            'discrete_regressor_correlation.png')
        plt.savefig(filename)

    # Look at the continuous kernels
    cont = [x for x in glm.run_params['kernels']
            if glm.run_params['kernels'][x]['type'] == 'continuous']
    ...
    if save_plot:
        filename = os.path.join(
            glm.run_params['figure_dir'],
            'continuous_regressor_correlation.png')
        plt.savefig(filename)

    # Plot the correlations between the timeseries with no delay for the
    # continuous kernels
    ...
    if save_plot:
        filename = os.path.join(
            glm.run_params['figure_dir'],
            'continuous_events_correlation.png')
        plt.savefig(filename)
...
def var_explained_matched(results_pivoted, run_params):
    ...
    results_pivoted['cell_type'] = [mapper[x]
                                    for x in results_pivoted['cre_line']]
    ...
    results_pivoted['matched'] = ['Matched' if x in np.array(
        matched_cells) else 'Non-matched' for x in results_pivoted['cell_specimen_id']]
    ...
    plt.savefig(filename)
    return results_pivoted.groupby(['cell_type', 'experience_level', 'matched'])[
        'variance_explained_percent'].describe()


def var_explained_by_experience(
        results_pivoted,
        run_params,
        threshold=0,
        savefig=False):
    ...
    if threshold != 0:
        results_pivoted = results_pivoted.query(
            '(not passive) & (variance_explained_full > @threshold)').copy()
    else:
        results_pivoted = results_pivoted.query('not passive').copy()
    ...
    if savefig:
        if threshold != 0:
            filename = run_params['figure_dir'] + \
                '/variance_explained_by_experience_filtered.svg'
            plt.savefig(
                run_params['figure_dir'] +
                '/variance_explained_by_experience_filtered.png')
            print('Figure saved to: ' + filename)
            plt.savefig(filename)
        else:
            filename = run_params['figure_dir'] + \
                '/variance_explained_by_experience.svg'
            plt.savefig(
                run_params['figure_dir'] +
                '/variance_explained_by_experience.png')
            print('Figure saved to: ' + filename)
            plt.savefig(filename)
    return results_pivoted.groupby(['cell_type', 'experience_level'])[
        'variance_explained_percent'].describe()


def compare_var_explained_by_version(
        results=None,
        fig=None,
        ax=None,
        test_data=True,
    ...
    if cre is not None:
        ...
        if show_equipment:
            hue = 'meso'
            results['meso'] = [
                'Mesoscope' if "MESO" in x else 'Scientifica' for x in results['equipment_name']]
    ...
    if sort_by_signal:
        ...
        if num_versions > 2:
            glm_version_order = np.concatenate([np.sort(results.query('dff')['glm_version'].unique()), [
                                               ''], np.sort(results.query('not dff')['glm_version'].unique())])
        else:
            glm_version_order = np.concatenate(
                [
                    np.sort(
                        results.query('dff')['glm_version'].unique()), np.sort(
                        results.query('not dff')['glm_version'].unique())])
    else:
    ...
    ax.set_xlabel(
        metric +
        ' Model Variance Explained on {} set'.format(dataset),
        fontsize=16)
...
def get_movie_filepath(
        session_id,
        session_type='OphysSession',
        movie_type='RawBehaviorTrackingVideo'):
...
def plot_kernels(
    ...
    # kernels_to_exclude_from_plot =
    # ['intercept','time',]#['intercept','time','model_task0','model_timing1D','model_bias','model_omissions1']
    kernels_to_exclude_from_plot = [
        'intercept',
        'time',
        'model_task0',
        ...
        'model_bias',
        'model_omissions1']
    kernels_to_include_in_plot = [k for k in kernel_df['kernel_name'].unique(
    ) if k not in kernels_to_exclude_from_plot]
    palette = palette_df.query(
        'kernel_name in @kernels_to_include_in_plot')['kernel_color'].to_list()

    if t_span:
        t0, t1 = t_span
        data_to_plot = kernel_df.query(
            'timestamps >= @t0 and timestamps <= @t1 and kernel_name not in @kernels_to_exclude_from_plot')
    else:
        data_to_plot = kernel_df.query(
            'kernel_name not in @kernels_to_exclude_from_plot')
...
def plot_session_summary(glm):
    plt.figure()
    plt.plot(glm.dropout_summary.query('dropout=="Full"')
             ['variance_explained'].sort_values().values)
...
def plot_dropout_summary(
    ...
    data_to_plot = data_to_plot.query(
        'dropout not in @all_identified_dropouts_to_exclude')
    ...
    for idx, row in data_to_plot.iterrows():
        if row['dropout'] in single_dropouts:
            ...
            data_to_plot.at[idx, 'dropout_simple'] = row['dropout'].split(
                'single-')[1]
    ...
    yorder = (data_to_plot .query('dropout_type == @sort_by') .sort_values(
        by='adj_fraction_change_from_full', ascending=False)['dropout_simple'] .values)

    bp = sns.barplot(
        data=data_to_plot.sort_values(
            by='adj_fraction_change_from_full',
            ascending=False),
        ...
        hue_order=[
            'standard',
            'single'] if dropouts_to_plot == 'both' else None,
        order=yorder,
        palette=[
            'magenta',
            'cyan'] if dropouts_to_plot == 'both' else ['cyan'])
    ...
    plt.setp(ax.get_legend().get_texts(),
             fontsize=legend_fontsize)  # for legend text
    plt.setp(ax.get_legend().get_title(),
             fontsize=legend_fontsize)  # for legend title
...
def plot_filters(glm, cell_specimen_id, n_cols=5):
    ...
    fig, ax = plt.subplots(
        int(n_rows), int(n_cols), figsize=(
            2.5 * n_cols, 2.5 * n_rows), sharey=True)
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            if ii <= len(kernel_list) - 1:
                ...
                t_kernel, w_kernel = get_kernel_weights(
                    glm, kernel_name, cell_specimen_id)
...
def get_title(ophys_experiment_id, cell_specimen_id, glm_version):
    ...
    row = experiments_table.query(
        'ophys_experiment_id == @ophys_experiment_id').iloc[0].to_dict()
...
def get_max_locs_df(df_in):
    ...
    max_df = df_in.groupby('kernel_name')[['kernel_outputs', 'kernel_outputs_abs']].max(
    ).sort_values(by='kernel_outputs', ascending=False)
    max_locs = []
...
class GLM_Movie(object):
    ...
    def __init__(
        ...
        try:
            ...
            # GLM versions 10 and earlier lack the 'use_events' key and always
            # fit dff
        ...
        if destination_folder is None:
            # if destination_folder is not specified, set it to
            # {run_params['output_dir']}/output_files
        ...
        for ii, movie_name in enumerate(['Behavior', 'Eye']):
            try:
                sync_timestamps = self.sync_data['cam{}_exposure_rising'.format(
                    ii + 1)]
            except KeyError:
                if movie_name == 'Eye':
        ...
        self.results_summary = glm.dropout_summary
        self.dropout_summary_plotted = False
        self.cell_roi_plotted = False

        self.this_cell = self.glm.cell_results_df.query(
    ...
    def make_cell_movie_frame(
        ...
        if self.verbose:
            print(
                'done getting cell info at {:0.2f} seconds'.format(
                    time.time() - ti))

        cell_index = np.where(
            glm.W['cell_specimen_id'] == cell_specimen_id)[0][0]

        if self.verbose:
            print(
                'done getting cell index at {:0.2f} seconds'.format(
                    time.time() - ti))
        ...
        if self.verbose:
            print(
                'done setting up tspan at {:0.2f} seconds'.format(
                    time.time() - ti))
        ...
        if not self.dropout_summary_plotted and len(
                self.glm.dropout_summary['dropout'].unique()) > 1:
            try:
        ...
        if self.verbose:
            print(
                'done plotting dropout summary at {:0.2f} seconds'.format(
                    time.time() - ti))
        ...
        if self.verbose:
            print(
                'done clearing axes at {:0.2f} seconds'.format(
                    time.time() - ti))
        ...
        if not self.cell_roi_plotted:
            ...
            if cell_specimen_id in glm.session.cell_specimen_table.index.tolist():
                ...
                self.com = ndimage.measurements.center_of_mass(
                    glm.session.cell_specimen_table.loc[cell_specimen_id]['roi_mask'])
            elif cell_roi_id in glm.session.cell_specimen_table['cell_roi_id'].tolist():
                print('FOUND CELL ROI ID')
                correct_csid = self.glm.session.cell_specimen_table.query(
                    'cell_roi_id == @cell_roi_id').index[0]
        ...
        if self.verbose:
            print(
                'done getting roi info at {:0.2f} seconds'.format(
                    time.time() - ti))

        for movie_name in ['behavior', 'eye']:
            frame = self.tracking_movies[movie_name].get_frame(time=t_now)[
                :, :, 0]
            ...
            ax['{}_movie'.format(movie_name)].set_title(
                '{} tracking movie'.format(movie_name), fontsize=22)

        if self.verbose:
            print(
                'done plotting behavior videos at {:0.2f} seconds'.format(
                    time.time() - ti))

        # make a crude approximation of pixelwise df/f
        frame_2p = self.this_cell.query(
            'fit_trace_timestamps >= @t_now')['frame_index'].iloc[0]
        f0 = self.real_2p_movie[frame_2p -
                                100:frame_2p + 100, :, :].mean(axis=0)
        f = self.real_2p_movie[frame_2p - 3:frame_2p + 3, :, :].mean(axis=0)
        dff = (f - f0) / f0
        dff[pd.isnull(dff)] = 0
        # set cmax to 95th percentile of this image
        cmax = np.percentile(dff, 95)
        ax['real_fov'].imshow(dff[10:-10, 10:-10], cmap='gray', clim=[0, cmax])

        ax['real_fov'].set_title('2P Field of View', fontsize=22)

        # ,'reconstructed_fov','simulated_fov']:
        for axis_name in ['real_fov']:
        ...
        if self.verbose:
            print(
                'done plotting 2P FOV at {:0.2f} seconds'.format(
                    time.time() - ti))
        ...
        qs = 'fit_trace_timestamps >= {} and fit_trace_timestamps <= {}'.format(
            self.initial_time - t_before, self.final_time + t_after)
        ax['dff'].set_ylim(
        ...
        if self.verbose:
            print(
                'done plotting Fluorscence timeseries at {:0.2f} seconds'.format(
                    time.time() - ti))
        ...
        if self.verbose:
            print(
                'done plotting rewards and licks at {:0.2f} seconds'.format(
                    time.time() - ti))
        ...
        if self.verbose:
            print(
                'done plotting pupil and running at {:0.2f} seconds'.format(
                    time.time() - ti))

        plot_kernels(
            self.kernel_df,
            ax['kernel_contributions'],
            self.palette_df,
            t_span)
        ...
        if self.verbose:
            print(
                'done plotting kernels at {:0.2f} seconds'.format(
                    time.time() - ti))
        ...
        if self.verbose:
            print(
                'done plotting stimulus spans at {:0.2f} seconds'.format(
                    time.time() - ti))

        ax['dff'].set_title(
            'Time series plots for cell {}'.format(cell_specimen_id),
            fontsize=22)
        ...
        ax['dff'].set_ylabel(
            ...
            ha='right',
            va='center')
        ax['events'].set_ylabel(
            'event\nmagnitude',
            rotation=0,
            ha='left',
            va='center')
        ax['running'].set_ylabel(
            'Running\nSpeed\n(cm/s)',
            rotation=0,
            ha='right',
            va='center')
        ax['pupil'].set_ylabel(
            ...
            ha='left',
            va='center')
        ax['kernel_contributions'].set_ylabel(
            'kernel\ncontributions\nto predicted\nsignal\n($\\Delta$F/F)',
            rotation=0,
            ha='right',
            va='center')
        ...
        if self.verbose:
            print(
                'done axis formatting at {:0.2f} seconds'.format(
                    time.time() - ti))
    ...
    def set_up_axes(self):
        ...
        ax['kernel_contributions'].get_shared_x_axes().join(
            ax['kernel_contributions'], ax['dff'])

        variance_explained_string = 'Variance explained (full model) = {:0.1f}%'.format(
            100 * self.glm.results.loc[self.cell_specimen_id]['Full__avg_cv_var_test'])
        fig.suptitle(
            self.title +
            '\n' +
    ...
    def make_movie(self):
        ...
        filename = self.title + \
            '_frame_{}_to_{}.mp4'.format(self.start_frame, self.end_frame)
...
# TODO update
def plot_all_kernel_comparison(
    weights_df, run_params, drop_threshold=0, session_filter=[
        1, 2, 3, 4, 5, 6], equipment_filter="all", depth_filter=[
            0, 1000], cell_filter="all", area_filter=[
                'VISp', 'VISl'], compare=['cre_line'], plot_errors=False):
    '''
...
# TODO
def plot_compare_across_kernels(
        weights_df,
        run_params,
        kernels,
        session_filter=[
    ...
    if session_filter != [1, 2, 3, 4, 5, 6]:
        filter_string += '_sessions_' + \
            '_'.join([str(x) for x in session_filter])
    if area_filter != ['VISp', 'VISl']:
        filter_string += '_area_' + '_'.join(area_filter)
    if title is None:
        title = filter_string

    weights = weights_df.query(
        '(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&(session_number in @session_filter) & (ophys_session_id not in @problem_sessions) & (variance_explained_full > @threshold)').copy()

    # Set up time vectors.
    time_vec = np.arange(run_params['kernels'][kernels[0]]['offset'], run_params['kernels'][
                         kernels[0]]['offset'] + run_params['kernels'][kernels[0]]['length'], 1 / 31)
    ...
    if len(compare) == 0:
        ...
        plot_compare_across_kernels_inner(
            ax, weights_dfiltered, kernel_weights, '', 'k', lines, time_vec)
    else:
        ...
        for dex, group in enumerate(groups):
            ...
            plot_compare_across_kernels_inner(
                ax, weights_dfiltered, kernel_weights, group, color, lines, time_vec)
...
def plot_compare_across_kernels_inner(
        ax,
        df,
        kernels,
        group,
...
def plot_perturbation(
    weights_df, run_params, kernel, drop_threshold=0, session_filter=[
        1, 2, 3, 4, 5, 6], equipment_filter="all", depth_filter=[
            0, 1000], cell_filter="all", area_filter=[
                'VISp', 'VISl'], normalize=True, CMAP='Blues', in_ax=None):
    ...
    if depth_filter != [0, 1000]:
        filter_string += '_depth_' + \
            str(depth_filter[0]) + '_' + str(depth_filter[1])
    if area_filter != ['VISp', 'VISl']:
        filter_string += '_area_' + '_'.join(area_filter)
    filename2 = os.path.join(
        run_params['fig_kernels_dir'],
        kernel +
        filter_string +
        '_perturbation_validation.png')
    ...
    weights = weights_df.query(
        '(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&(session_number in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0])& (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))
    ...
    for dex, group in enumerate(groups):
        ...
        df_norm = [
            x if len(x) == len(time_vec) else scipy.interpolate.interp1d(
                meso_time_vec,
                x,
                fill_value="extrapolate",
                bounds_error=False)(time_vec) for x in df_norm]
        df_norm = np.vstack(df_norm)
        kernel_means[group] = df_norm.mean(axis=0)

    mids_range = range(
        int(np.floor(time_vec[0] / 0.75)), int(np.floor(time_vec[-1] / 0.75)) + 1)
    mids = [x * 0.75 + 0.75 / 2 for x in mids_range]
    print(mids)
    times = {}
    for flash_num in mids_range:
        times[flash_num] = (time_vec >= 0.75 *
                            flash_num) & (time_vec < 0.75 * (flash_num + 1))

    avg_vals = pd.DataFrame(
        {'Slc17a7-IRES2-Cre': [], 'Sst-IRES-Cre': [], 'Vip-IRES-Cre': []})
    for dex, group in enumerate(groups):
        avg_vals[group] = [np.mean(kernel_means[group][times[x]])
                           for x in mids_range]
    ...
    for dex, flash_num in enumerate(mids_range):
        ...
        ax.plot(
            avg_vals.loc[dex]['vip-sst'],
            avg_vals.loc[dex]['Slc17a7-IRES2-Cre'],
            'o',
            color=cmap[dex],
            markersize=10,
            label=label)
    ...
    for dex, flash_num in enumerate(mids_range):
        if dex == 0:
            ...
            startxy = [avg_vals.loc[dex - 1]['vip-sst'],
                       avg_vals.loc[dex - 1]['Slc17a7-IRES2-Cre']]

        endxy = [avg_vals.loc[dex]['vip-sst'],
                 avg_vals.loc[dex]['Slc17a7-IRES2-Cre']]
    ...
    plt.figure()
    plt.plot(
        kernel_means['Vip-IRES-Cre'] -
        kernel_means['Sst-IRES-Cre'],
        kernel_means['Slc17a7-IRES2-Cre'])
...
def plot_kernel_comparison_by_experience(
        weights_df,
        run_params,
        kernel,
        threshold=0,
    ...
    k, fig_f, ax_f = plot_kernel_comparison(weights_df, run_params, kernel, save_results=False, session_filter=[
                                            'Familiar'], threshold=threshold, drop_threshold=drop_threshold)
    k, fig_n, ax_n = plot_kernel_comparison(weights_df, run_params, kernel, save_results=False, session_filter=[
                                            'Novel 1'], threshold=threshold, drop_threshold=drop_threshold)
    k, fig_np, ax_np = plot_kernel_comparison(weights_df, run_params, kernel, save_results=False, session_filter=[
                                              'Novel >1'], threshold=threshold, drop_threshold=drop_threshold)

    ylims = list(ax_f.get_ylim()) + list(ax_n.get_ylim()) + \
        list(ax_np.get_ylim())
    ...
    k, fig_v, ax_v = plot_kernel_comparison(
        weights_df, run_params, kernel, save_results=False, session_filter=[
            'Familiar', 'Novel 1', 'Novel >1'], cell_filter='Vip-IRES-Cre', compare=['experience_level'], threshold=threshold, drop_threshold=drop_threshold)
    k, fig_s, ax_s = plot_kernel_comparison(
        weights_df, run_params, kernel, save_results=False, session_filter=[
            'Familiar', 'Novel 1', 'Novel >1'], cell_filter='Sst-IRES-Cre', compare=['experience_level'], threshold=threshold, drop_threshold=drop_threshold)
    k, fig_e, ax_e = plot_kernel_comparison(
        weights_df, run_params, kernel, save_results=False, session_filter=[
            'Familiar', 'Novel 1', 'Novel >1'], cell_filter='Slc17a7-IRES2-Cre', compare=['experience_level'], threshold=threshold, drop_threshold=drop_threshold)
    if savefig:
        fig_v.savefig(
            run_params['fig_kernels_dir'] +
            '/' +
...
def plot_kernel_comparison_by_kernel_excitation(
        weights_df, run_params, kernel, savefig=False):
    mapper = {
        'Slc17a7-IRES2-Cre': 'Excitatory',
        'Sst-IRES-Cre': 'Sst Inhibitory',
        'Vip-IRES-Cre': 'Vip Inhibitory'
    ...
    plot_kernel_comparison(
        ...
        cell_filter='Slc17a7-IRES2-Cre',
        compare=[
            kernel + '_excited'],
        set_title='Excitatory, Familiar, ' + nk,
        save_results=savefig)
    plot_kernel_comparison(
        weights_df,
        run_params,
        kernel,
        session_filter=['Novel 1'],
        cell_filter='Slc17a7-IRES2-Cre',
        compare=[
            kernel + '_excited'],
        set_title='Excitatory, Novel 1, ' + nk,
        save_results=savefig)
    plot_kernel_comparison(
        weights_df,
        run_params,
    ...
    plot_kernel_comparison(
        ...
        kernel,
        session_filter=['Familiar'],
        cell_filter='Sst-IRES-Cre',
        compare=[
            kernel + '_excited'],
        set_title='Sst Inhibitory, Familiar, ' + nk,
        save_results=savefig)
    plot_kernel_comparison(
        weights_df,
        run_params,
        ...
        compare=[
            kernel + '_excited'],
        set_title='Sst Inhibitory, Novel 1, ' + nk,
        save_results=savefig)
    plot_kernel_comparison(
        weights_df,
        run_params,
    ...
    plot_kernel_comparison(
        ...
        kernel,
        session_filter=['Familiar'],
        cell_filter='Vip-IRES-Cre',
        compare=[
            kernel + '_excited'],
        set_title='Vip Inhibitory, Familiar, ' + nk,
        save_results=savefig)
    plot_kernel_comparison(
        weights_df,
        run_params,
        ...
        compare=[
            kernel + '_excited'],
        set_title='Vip Inhibitory, Novel 1, ' + nk,
        save_results=savefig)
    plot_kernel_comparison(
        weights_df,
        run_params,
...
def plot_kernel_comparison(
    ...
    if kernel == 'all-passive_change':
        ...
        run_params['kernels'][kernel]['length'] = run_params['kernels']['passive_change']['length'] + \
    ...
    if ('misses' == kernel) & ('post-misses' in run_params['kernels']):
        time_vec = time_vec[:-1]
    if ('passive_change' == kernel) & (
            'post-passive_change' in run_params['kernels']):
        time_vec = time_vec[:-1]
    ...
    if kernel in weights_df:
        weights = weights_df.query(
            '(not passive)&(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&({0} in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0])& (variance_explained_full > @threshold) & ({1} <= @drop_threshold)'.format(
                filter_sessions_on,
                kernel))
        use_dropouts = True
    else:
        weights = weights_df.query(
            '(not passive)&(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&({0} in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0])& (variance_explained_full > @threshold)'.format(filter_sessions_on))
    ...
    if show_legend:
        ax.legend(
            loc='upper left', bbox_to_anchor=(
                1.05, 1), title=' & '.join(compare).replace(
                '_', ' '), handlelength=4)
...
def plot_kernel_comparison_inner(
        ax,
        df,
        label,
        color,
...
def kernel_evaluation_by_experience(
        weights_df,
        run_params,
        kernel,
        save_results=False,
    ...
    if drop_threshold > 0:
        print(
            'Are you sure you dont want to use -' +
            str(drop_threshold) +
            ' ?')
    if drop_threshold <= -1:
    ...
    filename_svg = os.path.join(
        run_params['fig_kernels_dir'],
        kernel + '_evaluation_' + filter_string + '.svg')
    problem_sessions = get_problem_sessions()
    ...
    # Make dropout list
    drop_list = [
        d for d in run_params['dropouts'].keys() if (
            (run_params['dropouts'][d]['is_single']) & (
                kernel in run_params['dropouts'][d]['kernels'])) or (
            (not run_params['dropouts'][d]['is_single']) & (
                kernel in run_params['dropouts'][d]['dropped_kernels']))]
    ...
    if kernel in weights_df:
        weights = weights_df.query(
            '(not passive)&(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&({0} in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0])& (variance_explained_full > 0) & ({1} <= 0)'.format(
                filter_sessions_on,
                kernel))
        use_dropouts = True
    else:
        weights = weights_df.query(
            '(not passive)&(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&({0} in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0])& (variance_explained_full > 0)'.format(filter_sessions_on))
        print('Dropouts not included, cannot use drop filter')
        use_dropouts = False
    ...
    sst_table = weights.query(
        'cre_line == "Sst-IRES-Cre"')[[kernel + '_weights', kernel]]
    vip_table = weights.query(
        'cre_line == "Vip-IRES-Cre"')[[kernel + '_weights', kernel]]
    slc_table = weights.query(
        'cre_line == "Slc17a7-IRES2-Cre"')[[kernel + '_weights', kernel]]
    ...
    # kernel,
    # run_params,ncells,session_filter=session_filter,zlims=None,savefig=save_results)
    zlims = plot_kernel_heatmap_with_dropout_by_experience(
        vip_table_F,
        vip_table_N,
        vip_table_Np,
        time_vec,
        ...
        ncells,
        cell_filter='Vip',
        zlims=zlim_table['vip'],
        savefig=save_results,
        sort_by_dropout=plot_dropout_sorted,
        limited=limited)
    zlims = plot_kernel_heatmap_with_dropout_by_experience(
        sst_table_F,
        sst_table_N,
...
def kernel_evaluation(
    ...
    # Plot
    ax[0, 0].plot(time_vec, sst.mean(axis=0), label='SST (n=' +
                  str(n_sst) + ')', color=colors['sst'], linewidth=2)
    ax[0, 0].plot(time_vec, vip.mean(axis=0), label='VIP (n=' +
                  str(n_vip) + ')', color=colors['vip'], linewidth=2)
    ax[0, 0].plot(time_vec, slc.mean(axis=0), label='SLC (n=' +
                  str(n_slc) + ')', color=colors['slc'], linewidth=2)
    ...
    sst_f = weights.query(
        'cre_line == "Sst-IRES-Cre" & variance_explained_full > @threshold')[kernel + '_weights']
    vip_f = weights.query(
        'cre_line == "Vip-IRES-Cre" & variance_explained_full > @threshold')[kernel + '_weights']
    slc_f = weights.query(
        'cre_line == "Slc17a7-IRES2-Cre" & variance_explained_full > @threshold')[kernel + '_weights']
    if plot_dropout_sorted:
        sst_drop_f = weights.query(
            'cre_line == "Sst-IRES-Cre" & variance_explained_full > @threshold')[kernel]
        vip_drop_f = weights.query(
            'cre_line == "Vip-IRES-Cre" & variance_explained_full > @threshold')[kernel]
        slc_drop_f = weights.query(
            'cre_line == "Slc17a7-IRES2-Cre" & variance_explained_full > @threshold')[kernel]
    n_sst = len(sst_f)
    n_vip = len(vip_f)
    n_slc = len(slc_f)
    if len(sst_f) > 0:
    ...
    if len(slc_f) > 0:
        ...
        slc_f[:] = np.nan
    ax[1, 0].plot(time_vec, sst_f.mean(axis=0), label='SST (n=' +
                  str(n_sst) + ')', color=colors['sst'], linewidth=2)
    ax[1, 0].plot(time_vec, vip_f.mean(axis=0), label='VIP (n=' +
                  str(n_vip) + ')', color=colors['vip'], linewidth=2)
    ax[1, 0].plot(time_vec, slc_f.mean(axis=0), label='SLC (n=' +
                  str(n_slc) + ')', color=colors['slc'], linewidth=2)
    ...
    if use_dropouts:
        sst_df = weights.query(
            '(cre_line == "Sst-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[
            kernel + '_weights']
        vip_df = weights.query(
            '(cre_line == "Vip-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[
            kernel + '_weights']
        slc_df = weights.query(
            '(cre_line == "Slc17a7-IRES2-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[
            kernel + '_weights']
        if plot_dropout_sorted:
            sst_drop_df = weights.query(
                '(cre_line == "Sst-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[kernel]
            vip_drop_df = weights.query(
                '(cre_line == "Vip-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[kernel]
            slc_drop_df = weights.query(
                '(cre_line == "Slc17a7-IRES2-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[kernel]

            sst_table_df = weights.query(
                '(cre_line == "Sst-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[[kernel + '_weights', kernel]]
            vip_table_df = weights.query(
                '(cre_line == "Vip-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[[kernel + '_weights', kernel]]
            slc_table_df = weights.query(
                '(cre_line == "Slc17a7-IRES2-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[[kernel + '_weights', kernel]]
        ...
        ax[2,
           0].plot(time_vec,
                   sst_df.mean(axis=0),
                   label='SST (n=' + str(n_sst) + ')',
                   color=colors['sst'],
                   linewidth=2)
        ax[2,
           0].plot(time_vec,
                   vip_df.mean(axis=0),
                   label='VIP (n=' + str(n_vip) + ')',
                   color=colors['vip'],
                   linewidth=2)
        ax[2,
           0].plot(time_vec,
                   slc_df.mean(axis=0),
                   label='SLC (n=' + str(n_slc) + ')',
                   color=colors['slc'],
                   linewidth=2)
        ...
        ax[2, 0].legend()
        ax[2, 0].set_title(
            'Filtered on Dropout Score < ' + str(drop_threshold))
    ...
    cbar = ax[0, 1].imshow(weights_sorted.T, aspect='auto', extent=[
                           time_vec[0], time_vec[-1], 0, np.shape(weights_sorted)[1]], cmap='PRGn')
    cbar.set_clim(-np.nanpercentile(np.abs(weights_sorted), 95),
                  np.nanpercentile(np.abs(weights_sorted), 95))
    color_bar = fig.colorbar(cbar, ax=ax[0, 1])
    color_bar.ax.set_ylabel('Weights')
    ax[0, 1].set_ylabel('{0} Cells\n sorted by peak'.format(
        np.shape(weights_sorted)[1]))
    ax[0, 1].set_xlabel('Time (s)')
    ax[0, 1].axhline(np.shape(vip)[1], color='k', linewidth='1')
    ax[0, 1].axhline(np.shape(vip)[1] + np.shape(sst)
                     [1], color='k', linewidth='1')
    ax[0, 1].set_yticks([np.shape(vip)[1] /
                         2, np.shape(vip)[1] +
                         np.shape(sst)[1] /
                         2, np.shape(vip)[1] +
                         np.shape(sst)[1] +
    ...
    cbar = ax[1, 1].imshow(weights_sorted_f.T, aspect='auto', extent=[
                           time_vec[0], time_vec[-1], 0, np.shape(weights_sorted_f)[1]], cmap='PRGn')
    cbar.set_clim(-np.nanpercentile(np.abs(weights_sorted_f), 95),
                  np.nanpercentile(np.abs(weights_sorted_f), 95))
    color_bar = fig.colorbar(cbar, ax=ax[1, 1])
    color_bar.ax.set_ylabel('Weights')
    ax[1, 1].set_ylabel('{0} Cells\n sorted by peak'.format(
        np.shape(weights_sorted_f)[1]))
    ax[1, 1].set_xlabel('Time (s)')
    ax[1, 1].axhline(np.shape(vip_f)[1], color='k', linewidth='1')
    ax[1, 1].axhline(np.shape(vip_f)[1] + np.shape(sst_f)
                     [1], color='k', linewidth='1')
    ax[1, 1].set_yticks([np.shape(vip_f)[1] /
                         2, np.shape(vip_f)[1] +
                         np.shape(sst_f)[1] /
                         2, np.shape(vip_f)[1] +
                         np.shape(sst_f)[1] +
    ...
    if use_dropouts:
        ...
        weights_sorted_df = np.hstack(
            [slc_sorted_df, sst_sorted_df, vip_sorted_df])
        cbar = ax[2, 1].imshow(weights_sorted_df.T, aspect='auto', extent=[
                               time_vec[0], time_vec[-1], 0, np.shape(weights_sorted_df)[1]], cmap='PRGn')
        cbar.set_clim(-np.nanpercentile(np.abs(weights_sorted_df), 95),
                      np.nanpercentile(np.abs(weights_sorted_df), 95))
        color_bar = fig.colorbar(cbar, ax=ax[2, 1])
        color_bar.ax.set_ylabel('Weights')
        ax[2, 1].set_ylabel('{0} Cells\n sorted by peak'.format(
            np.shape(weights_sorted_df)[1]))
        ax[2, 1].set_xlabel('Time (s)')
        ax[2, 1].axhline(np.shape(vip_df)[1], color='k', linewidth='1')
        ax[2, 1].axhline(np.shape(vip_df)[1] +
                         np.shape(sst_df)[1], color='k', linewidth='1')
        ax[2, 1].set_yticks([np.shape(vip_df)[1] /
                             2, np.shape(vip_df)[1] +
                             np.shape(sst_df)[1] /
                             2, np.shape(vip_df)[1] +
                             np.shape(sst_df)[1] +
    ...
    if plot_dropout_sorted:
        weights_sorted = np.hstack(
            [slc_drop_sorted, sst_drop_sorted, vip_drop_sorted])
        cbar = ax[0, 2].imshow(weights_sorted.T, aspect='auto', extent=[
                               time_vec[0], time_vec[-1], 0, np.shape(weights_sorted)[1]], cmap='PRGn')
        cbar.set_clim(-np.nanpercentile(np.abs(weights_sorted), 95),
                      np.nanpercentile(np.abs(weights_sorted), 95))
        color_bar = fig.colorbar(cbar, ax=ax[0, 2])
        color_bar.ax.set_ylabel('Weights')
        ax[0, 2].set_ylabel('{0} Cells\n sorted by dropout'.format(
            np.shape(weights_sorted)[1]))
        if kernel == 'omissions':
        ...
        ax[0, 2].axhline(np.shape(vip)[1] + np.shape(sst)
                         [1], color='k', linewidth='1')
        ax[0, 2].set_yticks([np.shape(vip)[1] /
                             2, np.shape(vip)[1] +
                             np.shape(sst)[1] /
                             2, np.shape(vip)[1] +
                             np.shape(sst)[1] +
        ...
        zlims = plot_kernel_heatmap(
            ...
            session_filter=session_filter,
            savefig=save_results)
        zlims_test = plot_kernel_heatmap_with_dropout(
            vip_table,
            sst_table,
            slc_table,
            time_vec,
        ...
        for index, dropout in enumerate(drop_list):
            ...
            for patch, color in zip(
                drops['boxes'], [
                    colors['sst'], colors['vip'], colors['slc']]):
        ...
        ax[0, 2].set_xticklabels(
            drop_list, rotation=60, fontsize=8, ha='right')
    ...
    if plot_dropout_sorted:
        weights_sorted_f = np.hstack(
            [slc_drop_sorted_f, sst_drop_sorted_f, vip_drop_sorted_f])
        cbar = ax[1, 2].imshow(weights_sorted_f.T, aspect='auto', extent=[
                               time_vec[0], time_vec[-1], 0, np.shape(weights_sorted_f)[1]], cmap='PRGn')
        cbar.set_clim(-np.nanpercentile(np.abs(weights_sorted_f), 95),
                      np.nanpercentile(np.abs(weights_sorted_f), 95))
        ...
        ax[1, 2].axhline(np.shape(vip_f)[1] +
                         np.shape(sst_f)[1], color='k', linewidth='1')
        ax[1, 2].set_yticks([np.shape(vip_f)[1] /
                             2, np.shape(vip_f)[1] +
                             np.shape(sst_f)[1] /
                             2, np.shape(vip_f)[1] +
                             np.shape(sst_f)[1] +
        ...
        for index, dropout in enumerate(drop_list):
            drop_sst = weights.query(
                'cre_line=="Sst-IRES-Cre" & variance_explained_full > @threshold')[dropout]
            drop_vip = weights.query(
                'cre_line=="Vip-IRES-Cre" & variance_explained_full > @threshold')[dropout]
            drop_slc = weights.query(
                'cre_line=="Slc17a7-IRES2-Cre" & variance_explained_full > @threshold')[dropout]
            drop_sst = drop_sst[~drop_sst.isnull()].values
            drop_vip = drop_vip[~drop_vip.isnull()].values
            drop_slc = drop_slc[~drop_slc.isnull()].values
    ...
    if plot_dropout_sorted:
        weights_sorted_df = np.hstack(
            [slc_drop_sorted_df, sst_drop_sorted_df, vip_drop_sorted_df])
        cbar = ax[2, 2].imshow(weights_sorted_df.T, aspect='auto', extent=[
                               time_vec[0], time_vec[-1], 0, np.shape(weights_sorted_df)[1]], cmap='PRGn')
        cbar.set_clim(-np.nanpercentile(np.abs(weights_sorted_df), 95),
                      np.nanpercentile(np.abs(weights_sorted_df), 95))
        ...
        ax[2, 2].axhline(np.shape(vip_df)[1] +
                         np.shape(sst_df)[1], color='k', linewidth='1')
        ax[2, 2].set_yticks([np.shape(vip_df)[1] /
                             2, np.shape(vip_df)[1] +
                             np.shape(sst_df)[1] /
                             2, np.shape(vip_df)[1] +
                             np.shape(sst_df)[1] +
        ...
        for index, dropout in enumerate(drop_list):
            drop_sst = weights.query(
                '(cre_line == "Sst-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[dropout].values
            drop_vip = weights.query(
                '(cre_line == "Vip-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[dropout].values
            drop_slc = weights.query(
                '(cre_line == "Slc17a7-IRES2-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(kernel))[dropout].values
        ...
        ax[2, 2].axhline(drop_threshold, color='r',
...
def plot_kernel_heatmap(
    ...
    cbar = ax.imshow(weights_sorted.T, aspect='auto', extent=[
                     time_vec[0], time_vec[-1], 0, np.shape(weights_sorted)[1]], cmap='PRGn')
    if zlims is None:
        zlims = [-np.nanpercentile(np.abs(weights_sorted), 95),
                 np.nanpercentile(np.abs(weights_sorted), 95)]
    ...
    ax.set_yticks([ncells['vip'] / 2,
                   ncells['vip'] + ncells['sst'] / 2,
                   ncells['vip'] + ncells['sst'] + ncells['exc'] / 2])
    ...
    if len(session_filter) == 1:
        ...
        extra = extra + '_' + \
            session_filter[0].replace(' ', '_').replace('>', 'p')
        title = title + ', ' + mapper[session_filter[0]]
    ax.set_title(title, fontsize=20)
    filename = os.path.join(
        run_params['fig_kernels_dir'],
        kernel + '_heatmap_' + extra + '.svg')
    if savefig:
        plt.savefig(filename)
    return zlims
    # plt.tight_layout()
...
def plot_kernel_heatmap_with_dropout_by_experience(
        vip_table,
        sst_table,
        slc_table,
        time_vec,
    ...
    if ax is None:
        ...
        vertical_offset = .75
        fig = plt.figure(figsize=(width, height))
        h = [Size.Fixed(pre_horz_offset), Size.Fixed(
            width - pre_horz_offset - post_horz_offset - .25)]
        v = [Size.Fixed(vertical_offset), Size.Fixed(
            (height - vertical_offset - .5) / 3)]
        divider = Divider(fig, (0, 0, 1, 1), h, v, aspect=False)
        ax3 = fig.add_axes(
            divider.get_position(),
            axes_locator=divider.new_locator(
        ...
        h = [Size.Fixed(width - post_horz_offset + .25), Size.Fixed(.25)]
        v = [Size.Fixed(vertical_offset + (height - vertical_offset - .5) /
                        2) + .125, Size.Fixed((height - vertical_offset - .5) / 2 - .125)]
        divider = Divider(fig, (0, 0, 1, 1), h, v, aspect=False)
        cax1 = fig.add_axes(
            divider.get_position(),
            axes_locator=divider.new_locator(
        ...
        v = [
            Size.Fixed(
                vertical_offset / 4),
            Size.Fixed(
                (height - vertical_offset - .5) / 2 - .125)]
        divider = Divider(fig, (0, 0, 1, 1), h, v, aspect=False)
        cax2 = fig.add_axes(
            divider.get_position(),
            axes_locator=divider.new_locator(
    ...
    vip_df = pd.DataFrame(vip_table[kernel +
                                    '_weights'].to_list(), columns=['w' +
                                                                    str(x) for x in range(0, ncols)])
    ...
    sst_df = pd.DataFrame(sst_table[kernel +
                                    '_weights'].to_list(), columns=['w' +
                                                                    str(x) for x in range(0, ncols)])
    ...
    slc_df = pd.DataFrame(slc_table[kernel +
                                    '_weights'].to_list(), columns=['w' +
                                                                    str(x) for x in range(0, ncols)])
    ...
    weights_sorted = np.concatenate(
        [slc_df.to_numpy(), sst_df.to_numpy(), vip_df.to_numpy()])[:, 0:-1].T
    drop_sorted = np.concatenate(
        [slc_df.to_numpy(), sst_df.to_numpy(), vip_df.to_numpy()])[:, -1].T
    ...
    cbar1 = ax1.imshow(vip_weights_sorted.T, aspect='auto', extent=[
                       time_vec[0], time_vec[-1], 0, np.shape(slc_weights_sorted)[1]], cmap='PRGn')
    if zlims is None:
        zlims = [-np.nanpercentile(np.abs(weights_sorted), 95),
                 np.nanpercentile(np.abs(weights_sorted), 95)]
    cbar1.set_clim(zlims[0], zlims[1])
    cbar2 = ax2.imshow(sst_weights_sorted.T, aspect='auto', extent=[
                       time_vec[0], time_vec[-1], 0, np.shape(sst_weights_sorted)[1]], cmap='PRGn')
    cbar2.set_clim(zlims[0], zlims[1])
    cbar3 = ax3.imshow(slc_weights_sorted.T, aspect='auto', extent=[
                       time_vec[0], time_vec[-1], 0, np.shape(vip_weights_sorted)[1]], cmap='PRGn')
    ...
    dax2.imshow(np.sqrt(sst_drop_sorted[:, np.newaxis]),
                aspect='auto', cmap=cmap, vmin=1e-10, vmax=1)
    dax2.set_yticks([])
    dax2.set_xticks([])
    dax3.imshow(np.sqrt(slc_drop_sorted[:, np.newaxis]),
                aspect='auto', cmap=cmap, vmin=1e-10, vmax=1)
    dax3.set_yticks([])
    dax3.set_xticks([])
    ...
    if savefig:
        filename = os.path.join(
            run_params['fig_kernels_dir'],
            kernel +
            '_heatmap_with_dropout_by_experience' +
            extra +
            cell_filter +
            '.svg')
        plt.savefig(filename)
...
def plot_kernel_heatmap_with_dropout(
        vip_table,
        sst_table,
        slc_table,
        time_vec,
    ...
    if ax is None:
        ...
        v = [Size.Fixed(vertical_offset + (height - vertical_offset - .5) /
                        2) + .35, Size.Fixed((height - vertical_offset - .5) / 2.5 - .125)]
        divider = Divider(fig, (0, 0, 1, 1), h, v, aspect=False)
        cax1 = fig.add_axes(
            divider.get_position(),
            axes_locator=divider.new_locator(
        ...
        v = [
            Size.Fixed(
                vertical_offset / 1),
            Size.Fixed(
                (height - vertical_offset - .5) / 2.5 - .125)]
    ...
    cmap = copy.copy(plt.cm.get_cmap('Blues'))
    cmap.set_under('black')
    cbar2 = dax1.imshow(np.sqrt(
        vip_drop_sorted[:, np.newaxis]), aspect='auto', cmap=cmap, vmin=1e-10, vmax=1)
    dax1.set_yticks([])
    dax1.set_xticks([])
    color_bar = fig.colorbar(cbar2, cax=cax2, extend='min')
    color_bar.ax.set_title('Coding \nScore', fontsize=16, loc='left')
...
def add_stimulus_bars(ax, kernel, alpha=0.1):
    ...
    if kernel in [
        ...
        times = set(np.concatenate(
            [np.arange(0, lims[1], 0.75), np.arange(-0.75, lims[0] - 0.001, -0.75)]))
        if kernel == 'omissions':
            ...
            for flash_start in times:
                if flash_start == 0:
                    ax.axvspan(
                        flash_start,
                        flash_start + 0.25,
                        color=project_colors()['schematic_change'],
                        alpha=0.2,
                        zorder=-np.inf)
                else:
                    ax.axvspan(
...
def plot_over_fitting(full_results, dropout, save_file=""):
    ...
    # First panel, relationship between variance explained and overfitting
    # proportion
    ax[0].plot(full_results[dropout + '__avg_cv_var_test'] * 100,
               full_results[dropout + '__over_fit'], 'ko', alpha=.1)
    ax[0].set_xlim(0, 100)
    ax[0].set_ylim(0, 1)
    ...
    if dropout == "Full":
        full_results = full_results.query(
            '(Full__over_fit < 1) and (Full__over_fit >=0)').copy()

    # Second panel, histogram of overfitting proportion, with mean/median
    # marked
    hist_output = ax[1].hist(
        full_results[dropout + '__over_fit'], 50, density=True)
    ax[1].set_xlim(0, 1)
    ax[1].set_ylim(0, 1.25 * np.max(hist_output[0][:-1]))
    ax[1].plot(np.mean(full_results[dropout + '__over_fit']),
               1.1 * np.max(hist_output[0][:-1]), 'rv', label='Mean')
    ...
    # Third panel, distribution of dropout_overfitting_proportion compared to
    # full model
    if dropout != "Full":
        ax[2].hist(full_results[dropout +
                                '__dropout_overfit_proportion'].where(lambda x: (x < 1) & (x > -
                                                                                           1)), 100)
        ax[2].axvline(full_results[dropout + '__dropout_overfit_proportion'].where(
            lambda x: (x < 1) & (x > -1)).median(), color='r', linestyle='--')
        ax[2].set_xlim(-1, 1)
    else:
...
def plot_over_fitting_summary(full_results, run_params, plot_dropouts=True):
    ...
    for index, d in enumerate(plot_list):
        if (d != "Full") & (not d.startswith('single-')):
            if d + '__dropout_overfit_proportion' in full_results:
                p.append(np.mean(
                    full_results[d + '__dropout_overfit_proportion'].where(lambda x: (x < 1) & (x > -1))))
                labels.append(d)
    ...
    plt.tight_layout()
    filename = os.path.join(
        run_params['fig_overfitting_dir'],
        'over_fitting_summary.png')
    plt.savefig(filename)
    ...
    for d in run_params['dropouts']:
        try:
            ...
            plot_over_fitting(
                full_results,
                d,
                save_file=run_params['fig_overfitting_dir'])
            plt.close(plt.gcf().number)
        except BaseException:
...
def plot_top_level_dropouts(
    ...
    '''
    if filter_cre:
        rsp = results_pivoted.query(
            '(variance_explained_full > 0.01) & (cre_line == @cre)').copy()
    else:
...
def plot_nested_dropouts(
    ...
    fig, ax = plt.subplots(
        1, num_levels + 1, figsize=((num_levels + 1) * 3 + 1, 4))
    cmap = plt.get_cmap("tab20c")
    ...
    if mixing:
        ...
        rsp['level1'] = [
            3 if (
                x[0] < thresh) & (
                x[1] < thresh) else x[2] for x in zip(
                rsp['visual'],
                rsp['behavioral'],
                rsp['level1'])]
    else:
    ...
    if force_nesting:
        ...
        rsp['level2_1'] = [
            np.argmin(x) for x in zip(
                rsp['face_motion_energy'],
                rsp['licking'],
                rsp['pupil_and_running'])]
        rsp['level2_2'] = [
            np.argmin(x) for x in zip(
                rsp['beh_model'],
                rsp['task'])]
        if mixing:
            rsp['level2_3'] = [
                np.argmin(x) for x in zip(
                    ...
                    rsp['licking'],
                    rsp['pupil_and_running'])]
            rsp['level2'] = [x[x[0] + 1] + 3 * x[0]
                             for x in zip(rsp['level1'], rsp['level2_0'], rsp['level2_1'], rsp['level2_2'], rsp['level2_3'])]
        else:
            rsp['level2'] = [x[x[0] + 1] + 3 * x[0]
                             for x in zip(rsp['level1'], rsp['level2_0'], rsp['level2_1'], rsp['level2_2'])]
    ...
    wedges, texts = ax[0].pie(
        level_1_props, radius=0, colors=outer_colors, wedgeprops=dict(
            width=size, edgecolor='w'))
    ax[0].legend(wedges, ['Visual', 'Behavioral', 'Cognitive',
                 'Mixed'], loc='center')  # ,bbox_to_anchor=(0,-.25,1,2))
    ax[0].set_title('Level 1')

    # Plot Layer 2 for legend
    if num_levels == 2:
        wedges, texts = ax[1].pie(
            level_2_props, radius=0, colors=inner_colors, wedgeprops=dict(
                width=size, edgecolor='w'))
        ax[1].legend(wedges,
                     ['all-images',
                      'expectation',
                      ...
                      'beh_model',
                      'task'],
                     loc='center')  # ,bbox_to_anchor=(0,-.4,1,2))
        if force_nesting:
    ...
    # Plot Full chart
    wedges, texts = ax[final_ax].pie(
        level_1_props, radius=r[0], colors=outer_colors, wedgeprops=dict(
            width=size, edgecolor='w'))
    if num_levels == 2:
        wedges, texts = ax[final_ax].pie(
            level_2_props, radius=r[1], colors=inner_colors, wedgeprops=dict(
                width=size, edgecolor='w'))
    if filter_cre:
...
def plot_lick_triggered_motion(
        ophys_experiment_id,
        cell_specimen_id,
        title=''):
    '''
    ...
    '''
    event_triggered_response = get_lick_triggered_motion_response(
...
def cosyne_make_dropout_summary_plot(dropout_summary, ax=None, palette=None):
    ...
    plot_dropout_summary_cosyne(
        dropout_summary,
        ax=ax,
        dropouts_to_show=dropouts_to_show,
        palette=palette)
    ax.tick_params(axis='both', labelsize=20)
    ax.set_ylabel(
        '% decrease in variance explained \n when removing sets of kernels',
        fontsize=24)
...
def plot_population_perturbation(
        results_pivoted,
        run_params,
        dropouts_to_show=[
            'all-images',
    ...
    if include_zero_cells:
        ...
        results_pivoted = results_pivoted.query(
            '(variance_explained_full > 0.005)&(not passive)').copy()
    ...
    experiment_table_columns = experiments_table.reset_index()[
        [
            ...
            'cell_type',
            'binned_depth']]
    results_pivoted = results_pivoted.merge(
        experiment_table_columns, on='ophys_experiment_id')
    ...
    # Make dataframe
    all_df = results_pivoted.groupby(['experience_level', 'cre_line'])[
        dropouts_to_show].mean()
    matched_df = results_pivoted.query('cell_specimen_id in @matched_cells').groupby(
        ['experience_level', 'cre_line'])[dropouts_to_show].mean()
    fig, ax = plt.subplots(
        6, len(dropouts_to_show), figsize=(
            10, 15), sharey=sharey, sharex=sharey)
    plot_population_perturbation_inner(
        'Exc', 'Sst', all_df, dropouts_to_show, sharey=sharey, ax=ax[0, :], add_title=True)
    plot_population_perturbation_inner(
        'Exc', 'Vip', all_df, dropouts_to_show, sharey=sharey, ax=ax[1, :])
    plot_population_perturbation_inner(
        'Sst', 'Vip', all_df, dropouts_to_show, sharey=sharey, ax=ax[2, :])
    plot_population_perturbation_inner(
        'Exc', 'Vip-Sst', all_df, dropouts_to_show, sharey=sharey, ax=ax[3, :])
    plot_population_perturbation_inner(
        'Exc-Vip', 'Sst', all_df, dropouts_to_show, sharey=sharey, ax=ax[4, :])
    plot_population_perturbation_inner(
        'Exc-Sst', 'Vip', all_df, dropouts_to_show, sharey=sharey, ax=ax[5, :])
    plt.tight_layout()
...
def add_perturbation_arrow(startxy, endxy, colors, experience_level, ax):
    ...
    sxy = (startxy[0] - dx, startxy[1] - dy)
    exy = (endxy[0] + dx, endxy[1] + dy)
    ax.annotate("", xy=exy, xytext=sxy, arrowprops=dict(
        arrowstyle="->", color=colors[experience_level]))


def plot_population_perturbation_inner(
        x,
        y,
        df,
        dropouts_to_show,
    ...
    if ax is None:
        fig, ax = plt.subplots(
            1, len(dropouts_to_show), figsize=(
                10, 2.5), sharey=sharey, sharex=sharey)
    for index, feature in enumerate(dropouts_to_show):
        ...
        if all_cells:
            ax[index].plot(
                ...
                familiarxy[1],
                'o',
                color=colors['Familiar'])
            ax[index].plot(
                novel1xy[0],
                novel1xy[1],
                'o',
                color=colors['Novel 1'])
            ax[index].plot(
                novelp1xy[0],
                novelp1xy[1],
                'o',
                color=colors['Novel >1'])
            add_perturbation_arrow(
                familiarxy,
                novel1xy,
                colors,
                'Familiar',
                ax[index])
            add_perturbation_arrow(
                novel1xy, novelp1xy, colors, 'Novel 1', ax[index])
        else:
            ax[index].plot(
                familiarxy[0],
                familiarxy[1],
        ...
        ax[index].xaxis.set_major_formatter(
            mpl.ticker.FormatStrFormatter('%.2f'))
        if add_title:
            if plot_big:
...
def plot_population_averages_by_depth(
        results_pivoted,
        run_params,
        dropouts_to_show=[
            'all-images',
    ...
    results_pivoted = results_pivoted.query(
        'targeted_structure in @area').copy()
    ...
    # Add binned depth
    results_pivoted['coarse_binned_depth'] = [
        coarse_bin_depth(x) for x in results_pivoted['imaging_depth']]
    ...
    for cell_type in cell_types:
        if len(dropouts_to_show) == 3:
            ...
            fig, ax = plt.subplots(
                1, len(dropouts_to_show), figsize=(
                    10, 2.5), sharey=sharey)
        all_data = results_pivoted.query('cell_type ==@cell_type')
        stats = {}
        summary[cell_type + ' data'] = {}
        # Iterate dropouts and plot each by experience
        for index, feature in enumerate(dropouts_to_show):
            stats[feature] = test_significant_dropout_averages_by_depth(
                all_data, feature)
            summary[cell_type + ' data'][feature] = all_data.groupby(
                ['experience_level', 'coarse_binned_depth'])[feature].describe()
            ...
            title_feature = title_feature.replace(
                'omissions_positive', 'excited')
            title_feature = title_feature.replace(
                'omissions_negative', 'inhibited')
            ...
            [t.set_color(i) for (i, t) in zip(
                experience_colors, ax[index].xaxis.get_ticklabels())]
        ...
        filename = run_params['figure_dir'] + \
            '/dropout_average_by_depth_' + cell_type[0:3] + extra + '.svg'
        if savefig:
            plt.savefig(run_params['figure_dir'] +
                        '/dropout_average_by_depth_' +
                        cell_type[0:3] +
                        extra +
                        '.png')
            print('Figure saved to: ' + filename)
...
def plot_population_averages_by_area(
        results_pivoted,
        run_params,
        dropouts_to_show=[
            'all-images',
    ...
    for cell_type in cell_types:
        ...
        # Iterate dropouts and plot each by experience
        for index, feature in enumerate(dropouts_to_show):
            stats[feature] = test_significant_dropout_averages_by_area(
                all_data, feature)
            # Plot all cells in active sessions
            summary[cell_type + ' data'][feature] = all_data.groupby(
                ['experience_level', 'targeted_structure'])[feature].describe()
            ax[index] = sns.pointplot(
                data=all_data,
                x='experience_level',
                y=feature,
        ...
        fig.subplots_adjust(wspace=0.5, hspace=0.3)
        filename = run_params['figure_dir'] + \
            '/dropout_average_by_area_' + cell_type[0:3] + extra + '.svg'
        if savefig:
            plt.savefig(run_params['figure_dir'] +
                        '/dropout_average_by_area_' +
                        cell_type[0:3] +
                        extra +
                        '.png')
            plt.savefig(filename)
...
def get_matched_cells_with_ve(cells_table, results_pivoted, threshold):
    ...
    cells_table = pd.merge(cells_table.reset_index(), results_pivoted[[
                           'cell_roi_id', 'variance_explained_full']], on='cell_roi_id', validate='1:1')

    # get cells with maxVE about threshold
    cells_with_ve = cells_table.groupby('cell_specimen_id')['variance_explained_full'].max(
    ).to_frame().query('variance_explained_full >= @threshold')
...
def plot_population_averages(
    ...
    add_stats=True,
    extra='',
    strict_experience_matching=False,
    plot_by_cell_type=False,
    across_session=False,
    stats_on_across=True,
    matched_with_variance_explained=False,
    ...
    if across_session:
        ...
        assert np.all([x.replace('_within', '_across') in results_pivoted for x in dropouts_to_show]
                      ), 'across_session dropout not available'
    ...
    for dropout in dropouts_to_show:
        if '_signed' in dropout:
            ...
            results_pivoted[dropout] = results_pivoted[dropout].abs()
        if across_session:
            # In addition to _within dropout, need to convert the corresponding
            # across session dropout
            results_pivoted[dropout.replace(
                '_within', '_across')] = results_pivoted[dropout.replace('_within', '_across')].abs()
    ...
    if strict_experience_matching:
        cells_table = loading.get_cell_table(
            ...
            include_4x2_data=run_params['include_4x2_data'])
        cells_table = cells_table.query('not passive').copy()
        cells_table = utilities.limit_to_last_familiar_second_novel_active(
            cells_table)
        cells_table = utilities.limit_to_cell_specimen_ids_matched_in_all_experience_levels(
            cells_table)
    ...
    if matched_with_variance_explained:
        matched_cells_with_ve = get_matched_cells_with_ve(
            cells_table, results_pivoted, matched_ve_threshold)
    ...
    if plot_by_cell_type:
        ...
        plt.tight_layout()
        filename = run_params['figure_dir'] + \
            '/dropout_average_combined' + extra + '.svg'
        if savefig:
            print('Figure saved to: ' + filename)
            plt.savefig(filename)
        ...
        for cell_type in cell_types:
            ...
            if strict_experience_matching:
                strict_matched_data = all_data.query(
                    'cell_specimen_id in @strict_matched_cells')
            if matched_with_variance_explained:
                matched_cells_with_ve_data = all_data.query(
                    'cell_specimen_id in @matched_cells_with_ve')

            stats = {}
            # Iterate dropouts and plot each by experience
            for index, feature in enumerate(dropouts_to_show):
                anova, tukey = test_significant_dropout_averages(
                    all_data, feature)
    ...
    for index, feature in enumerate(dropouts_to_show):
        ...
        for cindex, cell_type in enumerate(cell_types):
            ...
            summary_data[feature + ' data'][cell_type + \
                ' all data'] = all_data.groupby(['experience_level'])[feature].describe()
            summary_data[feature + ' data'][cell_type + ' matched data'] = matched_data.groupby(
                ['experience_level'])[feature].describe()
            # Plot all cells in active sessions
            if boxplot:
            ...
            if strict_experience_matching:
                # Plot cells in matched active sessions
                summary_data[feature + ' data'][cell_type + ' strict matched data'] = strict_matched_data.groupby(
                    ['experience_level'])[feature].describe()
                ax[cindex] = sns.pointplot(
                    data=strict_matched_data,
                    x='experience_level',
            ...
            ax[cindex].yaxis.set_major_formatter(
                mpl.ticker.FormatStrFormatter('%.2f'))
...
def test_significant_across_cell(data, feature):
    ...
    tukey_table = pd.read_html(
        post_hoc_res.summary().as_html(),
        header=0,
        index_col=0)[0]
...
def plot_dropout_individual_population(
        results,
        run_params,
        ax=None,
        palette=None,
    ...
    dropouts_to_show = [
        x for x in dropouts_to_show if (
            len(x) == 0) or (
            x in run_params['dropouts'])]
    if ax is None:
        height = 8
        width = 18
        horz_offset = 2
    ...
    if 'post-omissions' not in results.dropout.unique():
        dropouts_to_show = [
            x for x in dropouts_to_show if x not in [
                'all-omissions', 'post-omissions']]
    ...
    if use_single:
        dropouts_to_show = [
            x if x == '' else 'single-' +
            x for x in dropouts_to_show]

    data_to_plot = results.query('not passive').query(
        'dropout in @dropouts_to_show and variance_explained_full > {}'.format(threshold)).copy()
    data_to_plot['explained_variance'] = -1 * \
    ...
    if use_violin:
        ...
        filename = run_params['figure_dir'] + \
            '/dropout_individual_boxplot_single.svg'
    else:
        filename = run_params['figure_dir'] + '/dropout_individual_boxplot.svg'
        plt.savefig(
            run_params['figure_dir'] +
            '/dropout_individual_boxplot.png')

    if savefig:
        plt.savefig(filename)
        print('Figure saved to: ' + filename)

    return data_to_plot.groupby(['cre_line', 'dropout'])[
        'explained_variance'].describe()


def plot_dropout_summary_population_with_experience(
        results,
        run_params,
        dropouts_to_show=[
            'all-images',
    ...
    include_zero_cells=True,
    add_title=False,
    dropout_cleaning_threshold=None,
    ...
    cre_lines = ['Vip-IRES-Cre', 'Sst-IRES-Cre', 'Slc17a7-IRES2-Cre']
    groups = [
        'Vip-IRES-Cre Familiar',
        'Vip-IRES-Cre Novel',
        'Vip-IRES-Cre Novel+',
        'Sst-IRES-Cre Familiar',
        'Sst-IRES-Cre Novel',
        'Sst-IRES-Cre Novel+',
        'Slc17a7-IRES2-Cre Familiar',
        'Slc17a7-IRES2-Cre Novel',
        'Slc17a7-IRES2-Cre Novel+']

    if ('post-omissions' in results.dropout.unique()
        ) & ('omissions' in dropouts_to_show):
        dropouts_to_show = ['all-omissions' if x ==
                            'omissions' else x for x in dropouts_to_show]
    if ('post-hits' in results.dropout.unique()) & ('hits' in dropouts_to_show):
        dropouts_to_show = ['all-hits' if x ==
                            'hits' else x for x in dropouts_to_show]
    if ('post-misses' in results.dropout.unique()
        ) & ('misses' in dropouts_to_show):
        dropouts_to_show = ['all-misses' if x ==
                            'misses' else x for x in dropouts_to_show]
    if ('post-passive_change' in results.dropout.unique()
        ) & ('passive_change' in dropouts_to_show):
        dropouts_to_show = ['all-passive_change' if x ==
                            'passive_change' else x for x in dropouts_to_show]
    ...
    data_to_plot['experience_level'] = [clean_experience[x]
                                        for x in data_to_plot['experience_level']]
    data_to_plot['groups'] = data_to_plot['cre_line'] + \
        ' ' + data_to_plot['experience_level']
    if dropout_cleaning_threshold is not None:
        print(
            'Clipping dropout scores for cells with full model VE < ' +
    ...
    clean_labels = {
        ...
        'Sst-IRES-Cre': 'Sst Inhibitory',
        'Vip-IRES-Cre': 'Vip Inhibitory',
        'Slc17a7-IRES2-Cre Familiar': 'Excitatory F',
        'Sst-IRES-Cre Familiar': 'Sst Inhibitory F',
        'Vip-IRES-Cre Familiar': 'Vip Inhibitory F',
        'Slc17a7-IRES2-Cre Novel': 'Excitatory N',
        'Sst-IRES-Cre Novel': 'Sst Inhibitory N',
    ...
    if savefig:
        if use_violin:
            filename = run_params['figure_dir'] + \
                '/dropout_summary' + extra + '_by_experience.svg'
            plt.savefig(filename)
        else:
            filename = run_params['figure_dir'] + \
                '/dropout_summary_boxplot' + extra + '_by_experience.svg'
            print('Figure saved to: ' + filename)
            plt.savefig(filename)
            plt.savefig(
                run_params['figure_dir'] +
                '/dropout_summary_boxplot' +
                extra +
                '_by_experience.png')
    return data_to_plot.groupby(['cre_line', 'dropout'])[
        'explained_variance'].describe()
...
def plot_dropout_summary_population(
        results,
        run_params,
        dropouts_to_show=[
            'all-images',
    ...
    if savefig:
        if use_violin:
            filename = run_params['figure_dir'] + \
                '/dropout_summary' + extra + '.svg'
            plt.savefig(filename)
        else:
            filename = run_params['figure_dir'] + \
                '/dropout_summary_boxplot' + extra + '.svg'
            print('Figure saved to: ' + filename)
            plt.savefig(filename)
            plt.savefig(
                run_params['figure_dir'] +
...
def plot_fraction_summary_population(
        results_pivoted,
        run_params,
        sharey=True,
        kernel_excitation=False,
        kernel=None,
        savefig=False):
    ...
    summary_df = results_pivoted.groupby(['cre_line', 'experience_level'])[
        ['code_anything', 'code_images', 'code_omissions', 'code_behavioral', 'code_task']].mean()

    # Compute Confidence intervals
    summary_df['n'] = results_pivoted.groupby(['cre_line',
                                               'experience_level'])[['code_anything',
                                                                     'code_images',
                                                                     'code_omissions',
                                                                     'code_behavioral',
    ...
    summary_df['code_behavioral_ci'] = 1.96 * np.sqrt(
        (summary_df['code_behavioral'] * (1 - summary_df['code_behavioral'])) / summary_df['n'])
    summary_df['code_task_ci'] = 1.96 * \
        np.sqrt((summary_df['code_task'] * (1 - summary_df['code_task'])) / summary_df['n'])
    ...
    if kernel_excitation:
        ...
        summary_df = results_pivoted.groupby(['cre_line',
                                              'experience_level'])[['code_anything',
                                                                    'code_' + kernel,
                                                                    'code_' + kernel + '_excited',
        ...
        summary_df['code_' + kernel + '_ci'] = 1.96 * np.sqrt(
            (summary_df['code_' + kernel] * (1 - summary_df['code_' + kernel])) / summary_df['n'])
        summary_df['code_' + kernel + '_excited_ci'] = 1.96 * np.sqrt(
            (summary_df['code_' + kernel + '_excited'] * (1 - summary_df['code_' + kernel + '_excited'])) / summary_df['n'])
        summary_df['code_' + kernel + '_inhibited_ci'] = 1.96 * np.sqrt(
            (summary_df['code_' + kernel + '_inhibited'] * (1 - summary_df['code_' + kernel + '_inhibited'])) / summary_df['n'])
    ...
    if kernel_excitation:
        fig, ax = plt.subplots(
            1, len(coding_groups), figsize=(
                8.1, 4), sharey=sharey)
    else:
        fig, ax = plt.subplots(
            1, len(coding_groups), figsize=(
                10.8, 4), sharey=sharey)
    for index, feature in enumerate(coding_groups):
        # plots three cre-lines in standard colors
        ax[index].plot([0,
                        1,
                        2],
                       summary_df.loc['Vip-IRES-Cre'][feature],
                       '-',
                       color=colors['Vip-IRES-Cre'],
                       label='Vip Inhibitory',
                       linewidth=3)
        ax[index].plot([0,
                        1,
                        2],
                       summary_df.loc['Sst-IRES-Cre'][feature],
                       '-',
                       color=colors['Sst-IRES-Cre'],
                       label='Sst Inhibitory',
                       linewidth=3)
        ax[index].plot([0,
                        1,
                        2],
                       summary_df.loc['Slc17a7-IRES2-Cre'][feature],
                       '-',
                       color=colors['Slc17a7-IRES2-Cre'],
                       label='Excitatory',
                       linewidth=3)
    ...
    if savefig:
        if kernel_excitation:
            filename = run_params['figure_dir'] + \
                '/coding_fraction_' + kernel + '_summary.svg'
            plt.savefig(filename)
            plt.savefig(
                run_params['figure_dir'] +
                '/coding_fraction_' +
            ...
            filename = run_params['figure_dir'] + \
                '/coding_fraction_summary.svg'
            plt.savefig(filename)
            plt.savefig(
                run_params['figure_dir'] +
                '/coding_fraction_summary.png')
            print('Figure saved to: ' + filename)
    return summary_df


def make_cosyne_schematic(
        glm,
        cell=1028768972,
        t_range=5,
        time_to_plot=3291,
    ...
    ax['cell_response'].set_ylim(
        list(np.array(ax['cell_response'].get_ylim()) * 1.1))
    return fig, ax


def make_cosyne_summary_figure(
        glm,
        cell_specimen_id,
        t_span,
        dropout_df,
    ...
    ax = {
        'visual_kernels': vbp.placeAxesOnGrid(
            fig,
            xspan=[
                0,
                0.4],
            yspan=[
                0,
                0.33 - vbuffer]),
        'behavioral_kernels': vbp.placeAxesOnGrid(
            fig,
            xspan=[
                0,
                0.4],
            yspan=[
                0.33 + vbuffer,
                0.67 - vbuffer]),
        'cognitive_kernels': vbp.placeAxesOnGrid(
            fig,
            xspan=[
                0,
                0.4],
            yspan=[
                0.67 + vbuffer,
                1]),
        'cell_response': vbp.placeAxesOnGrid(
            fig,
            xspan=[
                0.6,
                1],
            yspan=[
                0,
                0.25]),
        'dropout_quant': vbp.placeAxesOnGrid(
            fig,
            xspan=[
                0.6,
                1],
    ...
    palette_df = dropout_df[['color']].reset_index().rename(
        columns={'color': 'kernel_color', 'index': 'kernel_name'})

    t0, t1 = t_span
    for regressor_category in regressors.keys():
        dropouts = np.sort(
            dropout_df[dropout_df['level-5'] == regressor_category].index.values).tolist()
        plot_kernels(
        ...
        plot_stimuli(glm.session.stimulus_presentations,
                     ax['{}_kernels'.format(regressor_category)], t_span=t_span)
        ax['{}_kernels'.format(regressor_category)].set_title(
            '{}'.format(regressor_category))
    ...
    this_cell = glm.cell_results_df.query(
        'cell_specimen_id == @cell_specimen_id')
    ...
    plot_stimuli(
        glm.session.stimulus_presentations,
        ax['cell_response'],
        t_span=t_span,
        alpha=alpha)
...
def plot_coding_fraction(
    ...
    if '-' in dropout:
        ...
        if old_dropout in results_pivoted_in:
            results_pivoted_in = results_pivoted_in.rename(
                {old_dropout: dropout}, axis=1)
    ...
    # Apply hard filters
    results_pivoted = results_pivoted_in.query(
        '(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&(session_number in @session_filter) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0])& (variance_explained_full > @threshold)').copy()
    ...
    sig_cells = results_pivoted.query(
        filter_str).groupby(conditions)['Full'].count()

    # Get Magnitude
    magnitude = results_pivoted.groupby(conditions)[dropout].mean()
    filtered_magnitude = results_pivoted.query(
        filter_str).groupby(conditions)[dropout].mean()
    ...
    df = pd.concat([num_cells, sig_cells, fraction,
                   magnitude, filtered_magnitude], axis=1)
    ...
    # Make a list of comparison groups to plot, but we group all the session
    # numbers together
    ...
    for dex, group in enumerate(groups):
        ...
        if df.index.nlevels > 2:
            linedex = np.where(
                group[1] == np.array(
                    df.index.get_level_values(1).unique()))[0][0]
        ...
        if df.index.nlevels > 3:
            markerdex = np.where(
                group[2] == np.array(
                    df.index.get_level_values(2).unique()))[0][0]
            markerstyle = markers[np.mod(markerdex, len(markers))]
        else:
            markerstyle = 'o'

        # Plot
        plot_coding_fraction_inner(
            plt.gca(),
            df.loc[group],
            color,
            labels_SAC[dex],
...
def plot_coding_fraction_inner(
        ax,
        df,
        color,
        label,
    ...
    for dex, val in enumerate(zip(frac, se)):
        plt.plot([dex, dex], [val[0] + val[1],
                 val[0] - val[1]], 'k', linewidth=1)
...
def view_cell_across_sessions(cell_specimen_id, glm_version):
    ...
    dropouts_for_cell = gat.retrieve_results(
        search_dict=search_dict, results_type='summary')
    for idx, row in dropouts_for_cell.sort_values(
            by='date_of_acquisition').reset_index().iterrows():
        # get the dataset
        ...
        mask = dataset.cell_specimen_table.query(
            'cell_roi_id == @cell_roi_id')['roi_mask'].iloc[0].astype(float)
        mask[mask == 0] = np.nan
        xmin, xmax, ymin, ymax = np.where(
            mask == 1)[1].min(), np.where(
            mask == 1)[1].max(), np.where(
            mask == 1)[0].min(), np.where(
                mask == 1)[0].max()

        # plot the max projection
        axes['exp{}_full'.format(idx)].imshow(
            dataset.max_projection, cmap='gray')
        ...
        # zoom in on the cell
        axes['exp{}_zoom'.format(idx)].imshow(
            dataset.max_projection, cmap='gray')
        ...
        axes['exp{}_zoom'.format(idx)].set_title(
            'zoom in on cell', fontsize=10)

        # zoom in on the cell and overlay the mask
        axes['exp{}_roi'.format(idx)].imshow(
            dataset.max_projection, cmap='gray')
        ...
        axes['exp{}_roi'.format(idx)].set_title(
            'zoom with mask overlaid', fontsize=10)

        # add a title
        title = '{}\nacquired_on {}\nroi_id {}\nfull model var explained = {:0.1f}%'.format(
            row['session_type'],
            row['date_of_acquisition'].split(' ')[0],
            row['cell_roi_id'],
            100 * row['variance_explained'])
    ...
    fig.suptitle(
        'Cell Specimen ID = {}, Cre line = {}, rig = {}, GLM Version = {}'.format(
            cell_specimen_id,
            row['cre_line'],
            row['equipment_name'],
            glm_version))
...
def plot_regressor_heatmap_by_cre_line_sorted_by_MI(
        results_pivoted,
        session_numbers,
        regressor,
        model_output_type='adj_fraction_change_from_full',
        limit_to_nonzero=True,
        save_dir=None):
    '''
    ...
    for i, cre_line in enumerate(results_pivoted['cre_lines'].unique()):
        ...
        reg_values = results.pivot(
            index='cell_specimen_id',
            columns='session_number',
            values=regressor)
        reg_values = reg_values.abs()
        reg_values['MI'] = [(reg_values.loc[row][session_numbers[1]] -
                             reg_values.loc[row][session_numbers[0]]) /
                            (reg_values.loc[row][session_numbers[1]] +
                             reg_values.loc[row][session_numbers[0]]) for row in reg_values.index.values]
        ...
        if limit_to_nonzero:
            ...
            suffix = ''
        index_label = '(' + str(session_numbers[1]) + '-' + str(session_numbers[0]) + ')/(' + str(
            session_numbers[1]) + '+' + str(session_numbers[0]) + ')'
        ax[i] = sns.heatmap(
            reg_values.values,
            ax=ax[i],
            cmap='RdBu',
            ...
            vmax=1,
            cbar_kws={
                'label': model_output_type + '\n' + index_label},
            cbar_ax=cbar_ax)
        ax[i].set_title(cre_line)
        ax[i].set_xticklabels(session_numbers + ['MI'])
        ax[i].set_ylabel('cell number')
    fig.suptitle(regressor +
                 ', sessions ' +
                 str(session_numbers[0]) +
                 ' - ' +
                 str(session_numbers[1]), x=0.51, y=0.99, fontsize=22)
    ...
    if save_dir:
        if 'single' in regressor:
            utils.save_figure(
                ...
                os.path.join(
                    save_dir,
                    'regressor_heatmaps\\single'),
                'sorted_by_MI' + suffix,
                model_output_type + '_' + regressor + '_' + str(
                    session_numbers[0]) + '_' + str(
                    session_numbers[1]) + '_MI')
        else:
            utils.save_figure(
                fig,
                figsize,
...
def plot_sample_cells(
    ...
    '''
    fig, ax = plt.subplots(len(cell_specimen_ids), 1, figsize=(
        figwidth, height_per_cell * len(cell_specimen_ids)), sharex=True)

    # load the cell results df. This takes about 30 seconds.
    # If it's called as an attribute in the loop, it has to be reloaded on
    # each call since it's not a cached attribute (at least for python < 3.8)
    ...
    for row, cell_specimen_id in enumerate(cell_specimen_ids):
        ...
        query_string = 'cell_specimen_id == {} and fit_trace_timestamps >= {} and fit_trace_timestamps <= {}'.format(
            cell_specimen_id, t0, t1, )
        local_df = cell_results_df.query(query_string)

        # determine whether the input model used dff or events, set some
...
def plot_sem_distribution(results_pivoted, cres=None):
    ...
    for cre in cres:
        plt.hist(
            results_pivoted.query('cre_line == @cre')['variance_explained_full_sem'],
            bins=100,
            alpha=.25,
            range=(
                0,
...
def plot_sem_comparison(results_pivoted):
    ...
    for cre in cres:
        ...
        plt.plot(
            cre_slice['variance_explained_full'],
            cre_slice['variance_explained_full_sem'],
            'o',
            alpha=.1,
            color=project_colors()[cre])
    plt.ylabel('SEM')
...
def compare_events_and_dff(
    ...
    for cre in cres:
        ...
        ax[0].plot(
            cre_slice['variance_explained_full_dff'],
            cre_slice['variance_explained_full_events'],
            'o',
            alpha=.2,
            color=project_colors()[cre],
            label=cre)
    ...
    joint['VE (df/f-events)'] = joint['variance_explained_full_dff'] - \
        joint['variance_explained_full_events']
    ax[1] = sns.histplot(
        data=joint,
        x='VE (df/f-events)',
        hue='cre_line_dff',
...
def compare_weight_index(weights_df, kernel='all-images'):
    ...
    for cre in cres:
        ...
        plt.plot(temp[kernel], temp[kernel + '_weights_index'],
                 'o', color=project_colors()[cre], alpha=.2, label=cre)
...
def shuffle_analysis(
    ...
    plt.figure()
    plt.hist(
        results.query('dropout=="Full"')[
            'shuffle_' + shuffle],
        bins=bins,
        label='Shuffle {}'.format(shuffle),
        density=True)
    plt.ylabel('Density')
    ...
    plt.axvline(
        ...
        linestyle='-',
        alpha=1,
        label='Max shuffled VE')
    plt.axvline(
        run_params['dropout_threshold'],
        color='m',
        linestyle='--',
        alpha=1,
        label='Threshold for minimum VE (non shuffled)')
    plt.legend()
    plt.title(run_params['version'])
    if savefig:
        filepath = os.path.join(
            run_params['fig_overfitting_dir'],
            'shuffle_{}.png'.format(shuffle))
        print(filepath)
        plt.savefig(filepath)
...
def compare_dropout_thresholds(results_in):
    ...
    plot_dropout_summary_population(results.query(
        'variance_explained_full > 0.005'), ax=ax[0, 1])
    ax[0, 1].set_title('VE < 0.5% removed')
    ax[0, 1].get_legend().remove()

    plot_dropout_summary_population(results.query(
        'variance_explained_full > 0.01'), ax=ax[1, 1])
    ax[1, 1].set_title('VE < 1.0% removed')
    ax[1, 1].get_legend().remove()

    plot_dropout_summary_population(results.query(
        'variance_explained_full > 0.02'), ax=ax[2, 1])
    ax[2, 1].set_title('VE < 2.0% removed')
    ax[2, 1].get_legend().remove()

    results.loc[results['variance_explained_full']
                < 0.01, 'adj_fraction_change_from_full'] = 0
    plot_dropout_summary_population(results, ax=ax[1, 0])
    ax[1, 0].set_title('VE < 1.0% set dropout = 0')
    ax[1, 0].get_legend().remove()

    results.loc[results['variance_explained_full']
                < 0.02, 'adj_fraction_change_from_full'] = 0
...
def clustering_kernels(
        weights_df,
        run_params,
        kernel,
        just_coding=False,
        pca_by_experience=True):
    problem_sessions = get_problem_sessions()
    colors = project_colors()
    ...
    for index, cre_line in enumerate(cre_lines):
        if just_coding:
            weights = weights_df.query(
                '(cre_line == @cre_line) & (ophys_session_id not in @problem_sessions) &(not passive)&({0} <0)'.format(kernel)).copy()
        else:
            weights = weights_df.query(
                '(cre_line == @cre_line) & (ophys_session_id not in @problem_sessions) &(not passive)').copy()
        ...
        if pca_by_experience:
            ...
            ax[0, 0].plot(pca.explained_variance_ratio_, 'o-',
                          color=colors[cre_line], label=cre_line)
            ax[0, 0].set_xlim(-.5, 5)
        ...
        if pca_by_experience:
            ...
            ax[index, 1].plot(time_vec, pca.components_[
                              0, :], 'r-', label='PC 1')
            ax[index, 1].plot(time_vec, pca.components_[
                              1, :], 'b-', label='PC 2')
            ax[index, 1].legend()
        ...
        for eindex, experience_level in enumerate(
                ['Familiar', 'Novel 1', 'Novel >1']):
            if pca_by_experience:
                eweights = weights.query(
                    'experience_level ==@experience_level').copy()
                ...
                eweights['pc1'] = y[:, 0]
                eweights['pc2'] = y[:, 1]
                sorted_x = np.vstack(eweights.query(
                    'experience_level ==@experience_level')[kernel + '_weights'].values)
                ax[index,
                   0].plot(pca.explained_variance_ratio_,
                           'o-',
                           label=experience_level,
                           color=colors[experience_level])
                ax[index, 1].plot(time_vec, pca.components_[
                                  0, :], label=experience_level, color=colors[experience_level])
                ax[index, 0].legend()
            else:
                sorted_x = np.vstack(weights.query(
                    'experience_level ==@experience_level')[kernel + '_weights'].values)

            cbar = ax[index,
                      2 + eindex].imshow(sorted_x,
                                         aspect='auto',
                                         extent=[time_vec[0],
                                                 time_vec[-1],
                                                 0,
...
def depth_heatmap(
        weights_df,
        run_params,
        metric='omission_responsive',
        just_coding=False,
        just_mesoscope=False):
    if just_mesoscope:
        df = weights_df.query('equipment_name == "MESO.1"').query(
            'experience_level == "Familiar"').copy()
    else:
    ...
    if just_coding:
        fraction = df.query('omissions < 0').groupby(
            ['cre_line', 'targeted_structure', 'binned_depth'])[metric].mean()
        fraction['n'] = df.query('omissions < 0').groupby(
            ['cre_line', 'targeted_structure', 'binned_depth'])[metric].count()
    else:
        fraction = df.groupby(['cre_line', 'targeted_structure', 'binned_depth'])[
            [metric]].mean()
        fraction['n'] = df.groupby(['cre_line', 'targeted_structure', 'binned_depth'])[
            metric].count()
    fraction[metric + '_ci'] = 1.96 * \
        np.sqrt((fraction[metric] * (1 - fraction[metric])) / fraction['n'])
    ...
    for index, cell in enumerate(cre_lines):
        ...
        ax[1, index].plot([75, 175, 275, 375],
                          values[1, :], 'ko-', label='VISp')
        ax[1, index].plot([75, 175, 275, 375],
                          values[0, :], 'bo-', label='VISl')
        ax[1, index].errorbar([75, 175, 275, 375], values[1, :],
                              yerr=ci[1, :], color='k', alpha=.25)
        ax[1, index].errorbar([75, 175, 275, 375], values[0, :],
                              yerr=ci[0, :], color='b', alpha=.25)
...
def plot_dropout_summary_population_cdf_area(
        results_pivoted,
        run_params,
        cre='Vip-IRES-Cre',
        savefig=False,
    ...
    for eindex, experience_level in enumerate(experiences):
        for dindex, dropout in enumerate(dropouts):
            ...
            plot_dropout_summary_population_cdf_area_inner(results_pivoted,
                                                           run_params,
                                                           ax=ax[eindex,
                                                                 dindex],
            ...
            if dindex == 0:
                ax[eindex, dindex].set_ylabel(
                    clean_experiences[eindex] + '\nprop. > x (1-CDF)', fontsize=16)
            if eindex == 2:
                ax[eindex, dindex].set_xlabel(
                    clean_dropouts[dindex], fontsize=16)
    ...
    if savefig:
        ...
        plt.savefig(
            run_params['figure_dir'] +
            '/dropout_summary_cdf_areas.png')

    # Return the statistical tests
    # return tests


def plot_dropout_summary_population_cdf(
        results_pivoted,
        run_params,
        savefig=False,
        use_hierarchical=True):
    ...
    for cindex, cre in enumerate(cres):
        for dindex, dropout in enumerate(dropouts):
            # Plot the CDFs for this cre/dropout combination
            plot_dropout_summary_population_cdf_inner(
                results_pivoted, run_params, ax=ax[cindex, dindex], cre_line=cre, dropout=dropout)

            # clean up x/y labels
            if not ((dindex == 3) & (cindex == 0)):
                ax[cindex, dindex].get_legend().remove()
            if dindex == 0:
                ax[cindex, dindex].set_ylabel(
                    clean_cres[cindex] + '\n % of cells > x', fontsize=16)
            if cindex == 2:
                ax[cindex, dindex].set_xlabel('coding score', fontsize=16)
            if cindex == 0:
                ax[cindex, dindex].set_title(
                    clean_dropouts[dindex], fontsize=16)
...
def plot_dropout_summary_population_cdf_area_inner(
    ...
    if area_type == 'area_depth':
        data_to_plot['area'] = data_to_plot['targeted_structure'] + \
            '_' + data_to_plot['coarse_binned_depth']
    elif area_type == 'area':
        data_to_plot['area'] = data_to_plot['targeted_structure']
...
def plot_dropout_summary_population_cdf_inner(
        results_pivoted,
        run_params,
        ax=None,
        cre_line='Vip-IRES-Cre',
...
def compute_cdf_shuffles(
    ...
    for cre in cres:
        ...
        for drop in dropouts:
            ...
            tests.append({'cre_line': cre,
                          'dropout': drop,
                          'test': 'F_N',
                          'pvalues': test[0],
                          'test_statistics': test[1],
                          'full_pvalue': full_test[1],
                          'full_statistic': full_test[0]})
    ...
    tests['bootstrap_pvalues'] = [
        1 - x / nshuffles for x in tests['num_pvalues']]

    # Do multiple comparisons corrections, but remove the sanity checks first
    checks = tests.query('test == "F_F"')
    tests = add_hochberg_correction(
        tests.query('test != "F_F"'),
        on='bootstrap_pvalues')
...
def shuffle_cdfs(x, y, dropout, nshuffles=100,
                 sample_on='ophys_experiment_id'):
    '''
    ...
    for option in options_x:
        dict_x[option] = x.query(
            '{} == @option'.format(sample_on))[dropout].values
    for option in options_y:
        dict_y[option] = y.query(
            '{} == @option'.format(sample_on))[dropout].values
