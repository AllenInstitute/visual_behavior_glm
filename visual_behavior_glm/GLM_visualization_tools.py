...
def get_problem_sessions():
    ...
    '''
    problem_sessions = [873720614, 962045676, 1048363441, 1049240847, 1050231786, 1050597678, 1051107431, 1051319542,
                        1052096166, 1052330675, 1052512524, 1056065360, 1056238781, 1052752249, 1049240847, 1050929040,
                        1052330675]
...
def plot_kernel_support(glm, include_cont=True, plot_bands=True, plot_ticks=True, start=10000, end=11000):
    ...
    '''
    discrete = [x for x in glm.run_params['kernels'] if
                (glm.run_params['kernels'][x]['type'] == 'discrete') or (x == 'lick_model') or (x == 'groom_model')]
    ...
    if plot_ticks:
        if 'omissions' in glm.run_params['kernels']:
            omitted = glm.session.stimulus_presentations.query('start_time >@start_t & start_time < @end_t & omitted')[
                'start_time']
            ...
            for index, time in enumerate(omitted):
                plt.axvline(time, color=project_colors()['schematic_omission'], linestyle='--', zorder=-np.inf,
                            linewidth=1.5)

    # Image Expectation
    if plot_ticks & ('image_expectation' in glm.run_params['kernels']):
        expectation = \
        glm.session.stimulus_presentations.query('start_time >@start_t & start_time < @end_t & not omitted')[
            'start_time']
...
def plot_glm_version_comparison_histogram(comparison_table=None, results=None, versions_to_compare=None, savefig=True):
    assert not (
                comparison_table is None and versions_to_compare is None), 'must pass either a comparison table or a list of two versions to compare'
    assert not (
                comparison_table is not None and results is not None), 'must pass either a comparison table or a results dataframe, not both'

    if results is not None:
        if versions_to_compare is None:
            versions_to_compare = results['glm_version'].unique()
        assert len(
            versions_to_compare) == 2, 'can only compare two glm_versions. Either pass a list of two versions, or pass a results table with two versions'

    if comparison_table is None:
        comparison_table = gat.get_glm_version_comparison_table(versions_to_compare=versions_to_compare,
                                                                results=results)
    ...
    if savefig and (versions_to_compare is not None):
        ...
        for version in versions_to_compare:
            ...
            filepath = os.path.join(run_params['figure_dir'],
                                    'version_comparison_histogram_' + version_strings + '.png')
...
def plot_significant_cells(results_pivoted, dropout, dropout_threshold=0, save_fig=False, filename=None):
    ...
    for i, c in enumerate(cre):
        ...
        num_cells = len(cells)
        cell_count = np.array(
            [np.sum(cells.query('session_number == @x')[dropout] < dropout_threshold) for x in sessions])
...
def var_explained_matched(results_pivoted, run_params):
    ...
    results_pivoted['matched'] = ['Matched' if x in np.array(matched_cells) else 'Non-matched' for x in
                                  results_pivoted['cell_specimen_id']]
...
def compare_var_explained_by_version(results=None, fig=None, ax=None, test_data=True, figsize=(9, 5), use_violin=True,
                                     cre=None, metric='Full', show_equipment=True, zoom_xlim=True, sort_by_signal=True):
    '''
...
def plot_kernels(kernel_df, ax, palette_df, t_span=None, legend=False, annotate=True, t0=0, t1=np.inf):
    ...
    # kernels_to_exclude_from_plot = ['intercept','time',]#['intercept','time','model_task0','model_timing1D','model_bias','model_omissions1']
    kernels_to_exclude_from_plot = ['intercept', 'time', 'model_task0', 'model_timing1D', 'model_bias',
                                    'model_omissions1']
...
def get_max_locs_df(df_in):
    ...
    max_df = df_in.groupby('kernel_name')[['kernel_outputs', 'kernel_outputs_abs']].max().sort_values(
        by='kernel_outputs', ascending=False)
    max_locs = []
    for kernel_name, row in max_df.iterrows():
        ...
        max_locs.append({
            ...
            'percentile': stats.percentileofscore(df_in['kernel_outputs_abs'],
                                                  kernel_subset['kernel_outputs_abs'].abs().max(), kind='strict')
        })
...
def plot_all_kernel_comparison(weights_df, run_params, drop_threshold=0, session_filter=[1, 2, 3, 4, 5, 6],
                               equipment_filter="all", depth_filter=[0, 1000], cell_filter="all",
                               area_filter=['VISp', 'VISl'], compare=['cre_line'], plot_errors=False):  ##TODO update
    '''
    ...
    for kernel in run_params['kernels']:
        ...
        try:
            ...
            plot_kernel_comparison(weights_df, run_params, kernel, drop_threshold=drop_threshold,
                                   session_filter=session_filter, equipment_filter=equipment_filter,
                                   depth_filter=depth_filter, cell_filter=cell_filter, area_filter=area_filter,
                                   compare=compare, plot_errors=plot_errors)
        except Exception as e:
            print(e)
            # Track failures
            fail.append(kernel)
...
def plot_compare_across_kernels(weights_df, run_params, kernels, session_filter=[1, 2, 3, 4, 5, 6],
                                equipment_filter="all", cell_filter="all", compare=[], area_filter=['VISp', 'VISl'],
                                title=None):  # TODO
    '''
    ...
    # Set up time vectors.
    time_vec = np.arange(run_params['kernels'][kernels[0]]['offset'],
                         run_params['kernels'][kernels[0]]['offset'] + run_params['kernels'][kernels[0]]['length'],
                         1 / 31)
...
def plot_compare_across_kernels_inner(ax, df, kernels, group, color, linestyles, time_vec, linewidth=4):
    ...
    for dex, k in enumerate(kernels):
        ax.plot(time_vec, mean_kernels[k], linestyle=linestyles[dex], color=color, label=group + ' ' + k,
                linewidth=linewidth)


## TODO update
def plot_perturbation(weights_df, run_params, kernel, drop_threshold=0, session_filter=[1, 2, 3, 4, 5, 6],
                      equipment_filter="all", depth_filter=[0, 1000], cell_filter="all", area_filter=['VISp', 'VISl'],
                      normalize=True, CMAP='Blues', in_ax=None):
    if 'dropout_threshold' in run_params:
        threshold = run_params['dropout_threshold']
    else:
        threshold = 0.005
    ...
    weights = weights_df.query(
        '(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&(session_number in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0])& (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(
            kernel))
    ...
    for dex, group in enumerate(groups):
        ...
        df_norm = [
            x if len(x) == len(time_vec) else scipy.interpolate.interp1d(meso_time_vec, x, fill_value="extrapolate",
    ...
    for dex, flash_num in enumerate(mids_range):
        ...
        ax.plot(avg_vals.loc[dex]['vip-sst'], avg_vals.loc[dex]['Slc17a7-IRES2-Cre'], 'o', color=cmap[dex],
...
def plot_kernel_comparison_by_experience(weights_df, run_params, kernels, threshold=0, drop_threshold=0, savefig=False):
    ...
    for kernel in kernels:
        # Plot the weights for all the cre-lines for each session-types
        k, fig_f, ax_f = plot_kernel_comparison(weights_df, run_params, kernel, save_results=False,
                                                session_filter=['Familiar'], threshold=threshold,
                                                drop_threshold=drop_threshold)
        k, fig_n, ax_n = plot_kernel_comparison(weights_df, run_params, kernel, save_results=False,
                                                session_filter=['Novel 1'], threshold=threshold,
                                                drop_threshold=drop_threshold)
        k, fig_np, ax_np = plot_kernel_comparison(weights_df, run_params, kernel, save_results=False,
                                                  session_filter=['Novel >1'], threshold=threshold,
                                                  drop_threshold=drop_threshold)
        ...
        results_dir = run_params['fig_kernels_dir'] + '/' + kernel + '/'
        if not os.path.isdir(results_dir):
            os.makedirs(results_dir)

        # Save each kernel in its own folder
        if savefig:
            fig_f.savefig(results_dir + 'familiar_kernel' + extra + '.png')
            print('Figure saved to: ' + results_dir + 'familiar_kernel' + extra + '.png')
            fig_n.savefig(results_dir + 'novel1_kernel' + extra + '.png')
            print('Figure saved to: ' + results_dir + 'novel1_kernel' + extra + '.png')
            fig_np.savefig(results_dir + 'novelp1_kernel' + extra + '.png')
            print('Figure saved to: ' + results_dir + 'novelp1_kernel' + extra + '.png')

        # Save the weights across all the session types for each cre-line
        k, fig_v, ax_v = plot_kernel_comparison(weights_df, run_params, kernel, save_results=False, cell_filter='vip',
                                                compare=['experience_level'], threshold=threshold,
                                                drop_threshold=drop_threshold)
        k, fig_s, ax_s = plot_kernel_comparison(weights_df, run_params, kernel, save_results=False, cell_filter='sst',
                                                compare=['experience_level'], threshold=threshold,
                                                drop_threshold=drop_threshold)
        k, fig_e, ax_e = plot_kernel_comparison(weights_df, run_params, kernel, save_results=False, cell_filter='slc',
                                                compare=['experience_level'], threshold=threshold,
                                                drop_threshold=drop_threshold)

        # Save each kernel in the corresponding folder
        if savefig:
            fig_v.savefig(results_dir + 'vip_kernel' + extra + '.png')
            print('Figure saved to: ' + results_dir + 'vip_kernel' + extra + '.png')
            fig_s.savefig(results_dir + 'sst_kernel' + extra + '.png')
            print('Figure saved to: ' + results_dir + 'sst_kernel' + extra + '.png')
            fig_e.savefig(results_dir + 'exc_kernel' + extra + '.png')
            print('Figure saved to: ' + results_dir + 'exc_kernel' + extra + '.png')
...
def plot_kernel_comparison_by_kernel_excitation(weights_df, run_params, kernel, savefig=False):
    ...
    plot_kernel_comparison(weights_df, run_params, kernel, session_filter=['Familiar'], cell_filter='Slc17a7-IRES2-Cre',
                           compare=[kernel + '_excited'], set_title='Excitatory, Familiar, ' + nk, save_results=savefig)
    plot_kernel_comparison(weights_df, run_params, kernel, session_filter=['Novel 1'], cell_filter='Slc17a7-IRES2-Cre',
                           compare=[kernel + '_excited'], set_title='Excitatory, Novel 1, ' + nk, save_results=savefig)
    plot_kernel_comparison(weights_df, run_params, kernel, session_filter=['Novel >1'], cell_filter='Slc17a7-IRES2-Cre',
                           compare=[kernel + '_excited'], set_title='Excitatory, Novel >1, ' + nk, save_results=savefig)

    plot_kernel_comparison(weights_df, run_params, kernel, session_filter=['Familiar'], cell_filter='Sst-IRES-Cre',
                           compare=[kernel + '_excited'], set_title='Sst Inhibitory, Familiar, ' + nk,
                           save_results=savefig)
    plot_kernel_comparison(weights_df, run_params, kernel, session_filter=['Novel 1'], cell_filter='Sst-IRES-Cre',
                           compare=[kernel + '_excited'], set_title='Sst Inhibitory, Novel 1, ' + nk,
                           save_results=savefig)
    plot_kernel_comparison(weights_df, run_params, kernel, session_filter=['Novel >1'], cell_filter='Sst-IRES-Cre',
                           compare=[kernel + '_excited'], set_title='Sst Inhibitory, Novel >1, ' + nk,
                           save_results=savefig)

    plot_kernel_comparison(weights_df, run_params, kernel, session_filter=['Familiar'], cell_filter='Vip-IRES-Cre',
                           compare=[kernel + '_excited'], set_title='Vip Inhibitory, Familiar, ' + nk,
                           save_results=savefig)
    plot_kernel_comparison(weights_df, run_params, kernel, session_filter=['Novel 1'], cell_filter='Vip-IRES-Cre',
                           compare=[kernel + '_excited'], set_title='Vip Inhibitory, Novel 1, ' + nk,
                           save_results=savefig)
    plot_kernel_comparison(weights_df, run_params, kernel, session_filter=['Novel >1'], cell_filter='Vip-IRES-Cre',
                           compare=[kernel + '_excited'], set_title='Vip Inhibitory, Novel >1, ' + nk,
                           save_results=savefig)
...
def plot_event_aligned_responses(run_params, results, event, kernels=['all'],
    ...
    for ind, cell in enumerate(cell_filter):
        if cell == 'vip':
            cell_filter[ind] = 'Vip-IRES-Cre'
        elif cell == 'sst':
            cell_filter[ind] = 'Sst-IRES-Cre'
        elif cell == 'slc':
            cell_filter[ind] = 'Slc17a7-IRES2-Cre'
        else:
...
def plot_kernel_comparison(weights_df, run_params, kernel, save_results=True, drop_threshold=0,
                           ...
                           compare=['cre_line'], plot_errors=False, save_kernels=False, ax=None, fs1=20, fs2=16,
                           show_legend=True, filter_sessions_on='experience_level', image_set=['familiar', 'novel'],
    ...
    filename = os.path.join(run_params['fig_kernels_dir'],
                            kernel + '_comparison_by_' + '_and_'.join(compare) + filter_string + '.svg')

    # Add kernels not originally in the dictionary to the dictionary
    if kernel in ['preferred_image', 'all-images']:
        run_params['kernels'][kernel] = run_params['kernels']['image0'].copy()
    if kernel == 'all-omissions':
        run_params['kernels'][kernel] = run_params['kernels']['omission0'].copy()
        run_params['kernels'][kernel]['length'] = run_params['kernels']['omission0']['length']
    if kernel == 'all-hits':
    ...
    if ('omissions' == kernel) & ('post-omissions' in run_params['kernels']):
        time_vec = time_vec[:-1]
    elif 'image' in kernel or 'omission' in kernel:
        time_vec = time_vec[:-1]
    elif ('hits' == kernel) & ('post-hits' in run_params['kernels']):
        time_vec = time_vec[:-1]
    elif ('misses' == kernel) & ('post-misses' in run_params['kernels']):
        time_vec = time_vec[:-1]
    elif ('passive_change' == kernel) & ('post-passive_change' in run_params['kernels']):
        time_vec = time_vec[:-1]
    elif 'intercept' == kernel:
        time_vec = np.arange(1)

    if '-' in kernel:
        weights_df.columns = weights_df.columns.str.replace('-', '_')
    ...
    if kernel in weights_df:
        weights = weights_df.query(
            '(passive==False) & (targeted_structure in @area_filter) & (cre_line in @cell_list) & (equipment_name in @equipment_list) & ({0} in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0]) & (variance_explained_full > @threshold) & ({1} <= @drop_threshold)'.format(
                filter_sessions_on, kernel))
        use_dropouts = True
    else:
        weights = weights_df.query(
            '(passive==False)&(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&({0} in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0])& (variance_explained_full > @threshold)'.format(
                filter_sessions_on))
    ...
    for dex, group in enumerate(groups):
        ...
        k = plot_kernel_comparison_inner(ax, weights_dfiltered, group, color, linestyle, time_vec,
    ...
    if show_legend:
        ax.legend(loc='upper left', bbox_to_anchor=(1.05, 1), title=' & '.join(compare).replace('_', ' '),
                  handlelength=4)
    ...
    if save_kernels:
        ...
        filename2 = os.path.join(run_params['fig_kernels_dir'],
                                 kernel + '_comparison_by_' + '_and_'.join(compare) + filter_string + '.pkl')
...
def plot_kernel_comparison_inner(ax, df, label, color, linestyle, time_vec, plot_errors=True, linewidth=4, alpha=.25):
    ...
    # Plot mean and error bar
    if plot_errors:
        ax.fill_between(time_vec, df_norm.mean(axis=0) - df_norm.std(axis=0) / np.sqrt(df_norm.shape[0]),
                        df_norm.mean(axis=0) + df_norm.std(axis=0) / np.sqrt(df_norm.shape[0]), facecolor=color,
                        alpha=alpha)
...
def kernel_evaluation(weights_df, run_params, kernel, savefig=False, drop_threshold=0,
    ...
    '''
        ...
        weights_df              = gat.build_weights_df(run_params, results_pivoted)
        kernel                  The name of the kernel to be plotted
        savefig                 if True, saves a figure to the directory in run_params['output_dir']
        drop_threshold,         the minimum adj_fraction_change_from_full for the dropout model of just dropping this kernel
        session_filter,         The list of session numbers to include
        equipment_filter,       "scientifica" or "mesoscope" filter, anything else plots both
    '''
    ...
    if ('omissions' == kernel) & ('post-omissions' in run_params['kernels']):
        time_vec = time_vec[:-1]
    if 'image' in kernel or 'omission' in kernel:
        time_vec = time_vec[:-1]
    ...
    drop_list = [d for d in run_params['dropouts'].keys() if (
            (run_params['dropouts'][d]['is_single']) & (kernel in run_params['dropouts'][d]['kernels']))
                 or ((not run_params['dropouts'][d]['is_single']) & (
    ...
    if kernel in weights_df:
        weights = weights_df.query(
            '(passive==False) & (targeted_structure in @area_filter) & (cre_line in @cell_list) & (equipment_name in @equipment_list) & ({0} in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0]) & (variance_explained_full > 0) & ({1} <= 0)'.format(
                filter_sessions_on, kernel))
        use_dropouts = True
    else:
        weights = weights_df.query(
            '(passive==False)&(targeted_structure in @area_filter)& (cre_line in @cell_list)&(equipment_name in @equipment_list)&({0} in @session_filter) & (ophys_session_id not in @problem_sessions) & (imaging_depth < @depth_filter[1]) & (imaging_depth > @depth_filter[0])& (variance_explained_full > 0)'.format(
                filter_sessions_on))
        print('Dropouts not included, cannot use drop filter')
        use_dropouts = False
    ...
    if len(sst) > 0:
        ...
        sst = np.full((2, len(time_vec)), np.nan)
    if len(vip) > 0:
        vip = np.vstack(vip)
    else:
        vip = np.full((2, len(time_vec)), np.nan)
    if len(slc) > 0:
        slc = np.vstack(slc)
    else:
        slc = np.full((2, len(time_vec)), np.nan)
    ...
    if len(sst_f) > 0:
        ...
        sst_f = np.full((2, len(time_vec)), np.nan)
    if len(vip_f) > 0:
        vip_f = np.vstack(vip_f)
    else:
        vip_f = np.full((2, len(time_vec)), np.nan)
    if len(slc_f) > 0:
        slc_f = np.vstack(slc_f)
    else:
        slc_f = np.full((2, len(time_vec)), np.nan)
    ...
    if use_dropouts:
        sst_df = weights.query(
            '(cre_line == "Sst-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(
                kernel))[kernel + '_weights']
        vip_df = weights.query(
            '(cre_line == "Vip-IRES-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(
                kernel))[kernel + '_weights']
        slc_df = weights.query(
            '(cre_line == "Slc17a7-IRES2-Cre") & (variance_explained_full > @threshold) & ({0} < @drop_threshold)'.format(
                kernel))[kernel + '_weights']
        if plot_dropout_sorted:
        ...
        if len(sst_df) > 0:
            ...
            sst_df = np.full((2, len(time_vec)), np.nan)
        if len(vip_df) > 0:
            vip_df = np.vstack(vip_df)
        else:
            vip_df = np.full((2, len(time_vec)), np.nan)
        if len(slc_df) > 0:
            slc_df = np.vstack(slc_df)
        else:
            slc_df = np.full((2, len(time_vec)), np.nan)

        ax[2, 0].plot(time_vec, sst_df.mean(axis=0), label='SST (n=' + str(n_sst) + ')', color=colors['sst'],
                      linewidth=2)
        ax[2, 0].plot(time_vec, vip_df.mean(axis=0), label='VIP (n=' + str(n_vip) + ')', color=colors['vip'],
                      linewidth=2)
        ax[2, 0].plot(time_vec, slc_df.mean(axis=0), label='SLC (n=' + str(n_slc) + ')', color=colors['slc'],
                      linewidth=2)
    ...
    cbar = ax[0, 1].imshow(weights_sorted.T, aspect='auto',
                           extent=[time_vec[0], time_vec[-1], 0, np.shape(weights_sorted)[1]], cmap='bwr')
    ...
    ax[0, 1].set_yticks([np.shape(vip)[1] / 2, np.shape(vip)[1] + np.shape(sst)[1] / 2,
                         np.shape(vip)[1] + np.shape(sst)[1] + np.shape(slc)[1] / 2])
    ...
    cbar = ax[1, 1].imshow(weights_sorted_f.T, aspect='auto',
                           extent=[time_vec[0], time_vec[-1], 0, np.shape(weights_sorted_f)[1]], cmap='bwr')
    ...
    ax[1, 1].set_yticks([np.shape(vip_f)[1] / 2, np.shape(vip_f)[1] + np.shape(sst_f)[1] / 2,
                         np.shape(vip_f)[1] + np.shape(sst_f)[1] + np.shape(slc_f)[1] / 2])
    ...
    if use_dropouts:
        ...
        cbar = ax[2, 1].imshow(weights_sorted_df.T, aspect='auto',
                               extent=[time_vec[0], time_vec[-1], 0, np.shape(weights_sorted_df)[1]], cmap='bwr')
        ...
        ax[2, 1].set_yticks([np.shape(vip_df)[1] / 2, np.shape(vip_df)[1] + np.shape(sst_df)[1] / 2,
                             np.shape(vip_df)[1] + np.shape(sst_df)[1] + np.shape(slc_df)[1] / 2])
    ...
    if plot_dropout_sorted:
        ...
        cbar = ax[0, 2].imshow(weights_sorted.T, aspect='auto',
                               extent=[time_vec[0], time_vec[-1], 0, np.shape(weights_sorted)[1]], cmap='bwr')
        cbar.set_clim(-np.nanpercentile(np.abs(weights_sorted), 95), np.nanpercentile(np.abs(weights_sorted), 95))
        ...
        ax[0, 2].set_yticks([np.shape(vip)[1] / 2, np.shape(vip)[1] + np.shape(sst)[1] / 2,
                             np.shape(vip)[1] + np.shape(sst)[1] + np.shape(slc)[1] / 2])
    ...
    if plot_dropout_sorted:
        ...
        cbar = ax[1, 2].imshow(weights_sorted_f.T, aspect='auto',
                               extent=[time_vec[0], time_vec[-1], 0, np.shape(weights_sorted_f)[1]], cmap='bwr')
        cbar.set_clim(-np.nanpercentile(np.abs(weights_sorted_f), 95), np.nanpercentile(np.abs(weights_sorted_f), 95))
        ...
        ax[1, 2].set_yticks([np.shape(vip_f)[1] / 2, np.shape(vip_f)[1] + np.shape(sst_f)[1] / 2,
                             np.shape(vip_f)[1] + np.shape(sst_f)[1] + np.shape(slc_f)[1] / 2])
        ...
        zlims = plot_kernel_heatmap(weights_sorted_f, time_vec, kernel, run_params, ncells_f, extra='full_model',
                                    zlims=zlims, session_filter=session_filter, savefig=savefig)
    else:
    ...
    if plot_dropout_sorted:
        ...
        cbar = ax[2, 2].imshow(weights_sorted_df.T, aspect='auto',
                               extent=[time_vec[0], time_vec[-1], 0, np.shape(weights_sorted_df)[1]], cmap='bwr')
        cbar.set_clim(-np.nanpercentile(np.abs(weights_sorted_df), 95), np.nanpercentile(np.abs(weights_sorted_df), 95))
        ...
        ax[2, 2].set_yticks([np.shape(vip_df)[1] / 2, np.shape(vip_df)[1] + np.shape(sst_df)[1] / 2,
                             np.shape(vip_df)[1] + np.shape(sst_df)[1] + np.shape(slc_df)[1] / 2])
        ...
        zlims = plot_kernel_heatmap(weights_sorted_df, time_vec, kernel, run_params, ncells_df, extra='dropout',
                                    zlims=None, session_filter=session_filter, savefig=savefig)
        zlims_test = plot_kernel_heatmap_with_dropout(vip_table_df, sst_table_df, slc_table_df, time_vec, kernel,
                                                      run_params, ncells_df, session_filter=session_filter, zlims=zlims,
                                                      extra='dropout', savefig=savefig)

        zlims = plot_kernel_heatmap(weights_sorted, time_vec, kernel, run_params, ncells, zlims=zlims,
                                    session_filter=session_filter, savefig=savefig)
        zlims_test = plot_kernel_heatmap_with_dropout(vip_table, sst_table, slc_table, time_vec, kernel, run_params,
                                                      ncells, session_filter=session_filter, zlims=zlims,
                                                      savefig=savefig)
    else:
        # For each dropout, plot score
        for index, dropout in enumerate(drop_list):
...
def all_kernels_evaluation(weights_df, run_params, drop_threshold=0, session_filter=['Familiar', 'Novel 1', 'Novel >1'],
...
def plot_kernel_heatmap(weights_sorted, time_vec, kernel, run_params, ncells={}, ax=None, extra='', zlims=None,
    ...
    cbar = ax.imshow(weights_sorted.T, aspect='auto',
                     extent=[time_vec[0], time_vec[-1], 0, np.shape(weights_sorted)[1]], cmap='bwr')
    if zlims is None:
    ...
    ax.set_yticks(
        [ncells['vip'] / 2, ncells['vip'] + ncells['sst'] / 2, ncells['vip'] + ncells['sst'] + ncells['exc'] / 2])
...
def plot_kernel_heatmap_with_dropout(vip_table, sst_table, slc_table, time_vec, kernel, run_params, ncells={}, ax=None,
                                     extra='', zlims=None, session_filter=['Familiar', 'Novel 1', 'Novel >1'],
                                     savefig=False):
    if ax == None:
        # fig,ax = plt.subplots(figsize=(8,4))
        height = 4
    ...
    cbar1 = ax1.imshow(vip_weights_sorted.T, aspect='auto',
                       extent=[time_vec[0], time_vec[-1], 0, np.shape(slc_weights_sorted)[1]], cmap='bwr')
    if zlims is None:
        zlims = [-np.nanpercentile(np.abs(weights_sorted), 95), np.nanpercentile(np.abs(weights_sorted), 95)]
    cbar1.set_clim(zlims[0], zlims[1])
    cbar2 = ax2.imshow(sst_weights_sorted.T, aspect='auto',
                       extent=[time_vec[0], time_vec[-1], 0, np.shape(sst_weights_sorted)[1]], cmap='bwr')
    cbar2.set_clim(zlims[0], zlims[1])
    cbar3 = ax3.imshow(slc_weights_sorted.T, aspect='auto',
                       extent=[time_vec[0], time_vec[-1], 0, np.shape(vip_weights_sorted)[1]], cmap='bwr')
    ...
    if savefig:
        filename = os.path.join(run_params['fig_kernels_dir'], kernel + '_heatmap_with_dropout_' + extra + '.svg')
        plt.savefig(filename)
        print('Figure saved to: ' + filename)
        filename = os.path.join(run_params['fig_kernels_dir'], kernel + '_heatmap_with_dropout_' + extra + '.png')
        plt.savefig(filename)
    return zlims
...
def add_stimulus_bars(ax, kernel, alpha=0.1):
    ...
    if kernel in ['change', 'hits', 'misses', 'false_alarms', 'omissions', 'image_expectation', 'image', 'image0',
                  'image1', 'image2', 'image3', 'image4', 'image5', 'image6', 'image7', 'avg_image']:
        ...
        if kernel in ['change', 'hits', 'misses', 'false_alarms']:
            ...
            for flash_start in times:
                if flash_start == 0:
                    ax.axvspan(flash_start, flash_start + 0.25, color=project_colors()['schematic_change'], alpha=0.2,
                               zorder=-np.inf)
                else:
...
def plot_over_fitting(full_results, dropout, save_file=""):
    ...
    if dropout != "Full":
        ...
        ax[2].axvline(
            full_results[dropout + '__dropout_overfit_proportion'].where(lambda x: (x < 1) & (x > -1)).median(),
            color='r', linestyle='--')
        ax[2].set_xlim(-1, 1)
    else:
...
def plot_nested_dropouts(results_pivoted, run_params, num_levels=2, size=0.3, force_nesting=True, filter_cre=False,
                         cre='Slc17a7-IRES2-Cre', invert=False, mixing=True, thresh=-.2, savefig=True,
                         force_subsets=True):
    '''
    ...
    if mixing:
        ...
        rsp['level1'] = [3 if (x[0] < thresh) & (x[1] < thresh) else x[2] for x in
                         zip(rsp['visual'], rsp['behavioral'], rsp['level1'])]
    else:
        rsp['level1'] = [np.argmin(x) for x in zip(rsp['visual'], rsp['behavioral'], rsp['cognitive'])]
    ...
    if force_nesting:
        ...
        rsp['level2_1'] = [np.argmin(x) for x in
                           zip(rsp['face_motion_energy'], rsp['licking'], rsp['pupil_and_running'])]
        rsp['level2_2'] = [np.argmin(x) for x in zip(rsp['beh_model'], rsp['task'])]
        if mixing:
            rsp['level2_3'] = [np.argmin(x) for x in
                               zip(rsp['all-images'], rsp['expectation'], rsp['omissions'], rsp['face_motion_energy'],
                                   rsp['licking'], rsp['pupil_and_running'])]
            rsp['level2'] = [x[x[0] + 1] + 3 * x[0] for x in
                             zip(rsp['level1'], rsp['level2_0'], rsp['level2_1'], rsp['level2_2'], rsp['level2_3'])]
        else:
            rsp['level2'] = [x[x[0] + 1] + 3 * x[0] for x in
                             zip(rsp['level1'], rsp['level2_0'], rsp['level2_1'], rsp['level2_2'])]
        ...
        rsp['level2_0'] = [np.argmin(x) for x in
                           zip(rsp['all-images'], rsp['expectation'], rsp['omissions'], rsp['face_motion_energy'],
                               rsp['licking'], rsp['pupil_and_running'], rsp['beh_model'], rsp['task'])]
        rsp['level2_1'] = [np.argmin(x) for x in
                           zip(rsp['all-images'], rsp['expectation'], rsp['omissions'], rsp['face_motion_energy'],
                               rsp['licking'], rsp['pupil_and_running'], rsp['beh_model'], rsp['task'])]
        rsp['level2_2'] = [np.argmin(x) for x in
                           zip(rsp['all-images'], rsp['expectation'], rsp['omissions'], rsp['face_motion_energy'],
                               rsp['licking'], rsp['pupil_and_running'], rsp['beh_model'], rsp['task'])]
        if mixing:
            rsp['level2_3'] = [np.argmin(x) for x in
                               zip(rsp['all-images'], rsp['expectation'], rsp['omissions'], rsp['face_motion_energy'],
                                   rsp['licking'], rsp['pupil_and_running'], rsp['beh_model'], rsp['task'])]
            rsp['level2'] = [x[x[0] + 1] + 100 * x[0] for x in
                             zip(rsp['level1'], rsp['level2_0'], rsp['level2_1'], rsp['level2_2'], rsp['level2_3'])]
        else:
            rsp['level2'] = [x[x[0] + 1] + 100 * x[0] for x in
                             zip(rsp['level1'], rsp['level2_0'], rsp['level2_1'], rsp['level2_2'])]
        ...
        rsp['level2'] = [np.argmin(x) for x in
                         zip(rsp['all-images'], rsp['expectation'], rsp['omissions'], rsp['face_motion_energy'],
                             rsp['licking'], rsp['pupil_and_running'], rsp['beh_model'], rsp['task'])]
        level_2_props = rsp.groupby('level2')['level2'].count()
    ...
    if num_levels == 2:
        wedges, texts = ax[1].pie(level_2_props, radius=0, colors=inner_colors,
                                  wedgeprops=dict(width=size, edgecolor='w'))
        ax[1].legend(wedges,
                     ['all-images', 'expectation', 'omissions', 'face_motion_energy', 'licking', 'pupil_and_running',
                      'beh_model', 'task'], loc='center')  # ,bbox_to_anchor=(0,-.4,1,2))
        if force_nesting:
    ...
    # Plot Full chart
    wedges, texts = ax[final_ax].pie(level_1_props, radius=r[0], colors=outer_colors,
                                     wedgeprops=dict(width=size, edgecolor='w'))
    if num_levels == 2:
        wedges, texts = ax[final_ax].pie(level_2_props, radius=r[1], colors=inner_colors,
                                         wedgeprops=dict(width=size, edgecolor='w'))
    if filter_cre:
...
def plot_all_nested_dropouts(results_pivoted, run_params):
    ...
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=False, force_nesting=False, num_levels=1,
    ...
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=True, force_nesting=False, num_levels=1,
                         cre='Slc17a7-IRES2-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=True, force_nesting=False, num_levels=1,
                         cre='Vip-IRES-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=True, force_nesting=False, num_levels=1,
    ...
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=False, force_nesting=False, num_levels=2,
                         cre='Slc17a7-IRES2-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=False, force_nesting=False, num_levels=2,
                         cre='Vip-IRES-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=False, force_nesting=False, num_levels=2,
    ...
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=True, force_nesting=False, num_levels=2,
                         cre='Slc17a7-IRES2-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=True, force_nesting=False, num_levels=2,
                         cre='Vip-IRES-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=True, force_nesting=False, num_levels=2,
    ...
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=False, force_nesting=True, num_levels=2,
                         cre='Slc17a7-IRES2-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=False, force_nesting=True, num_levels=2,
                         cre='Vip-IRES-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=False, force_nesting=True, num_levels=2,
    ...
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=True, force_nesting=True, num_levels=2,
                         cre='Slc17a7-IRES2-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=True, force_nesting=True, num_levels=2,
                         cre='Vip-IRES-Cre')
    plot_nested_dropouts(results_pivoted, run_params, filter_cre=True, mixing=True, force_nesting=True, num_levels=2,
...
def plot_population_perturbation(results_pivoted, run_params,
                                 dropouts_to_show=['all-images', 'omissions', 'behavioral', 'task'], sharey=False,
    ...
    experiment_table_columns = experiments_table.reset_index()[
        ['ophys_experiment_id', 'last_familiar_active', 'second_novel_active', 'cell_type', 'binned_depth']]
    ...
    matched_df = results_pivoted.query('cell_specimen_id in @matched_cells').groupby(['experience_level', 'cre_line'])[
        dropouts_to_show].mean()
    fig, ax = plt.subplots(6, len(dropouts_to_show), figsize=(10, 15), sharey=sharey, sharex=sharey)
    plot_population_perturbation_inner('Exc', 'Sst', all_df, dropouts_to_show, sharey=sharey, ax=ax[0, :],
                                       add_title=True)
    ...
    plot_population_perturbation_inner('Vip-Sst', 'Exc', all_df, dropouts_to_show, sharey=sharey, add_title=True,
                                       plot_big=True)
    plt.tight_layout()
...
def plot_population_perturbation_inner(x, y, df, dropouts_to_show, sharey=True, all_cells=True, ax=None,
                                       add_title=False, plot_big=False):
    # plot
...
def plot_population_averages_by_depth(results_pivoted, run_params,
                                      dropouts_to_show=['all-images', 'omissions', 'behavioral', 'task'], sharey=False,
                                      include_zero_cells=True, add_stats=True, extra='', equipment="mesoscope",
                                      area='VISp', savefig=False):
    '''
    ...
    for cell_type in cell_types:
        ...
        for index, feature in enumerate(dropouts_to_show):
            ...
            summary[cell_type + ' data'][feature] = all_data.groupby(['experience_level', 'coarse_binned_depth'])[
                feature].describe()
...
def plot_population_averages_by_area(results_pivoted, run_params,
                                     dropouts_to_show=['all-images', 'omissions', 'behavioral', 'task'], sharey=False,
                                     include_zero_cells=True, add_stats=True, extra='', equipment="mesoscope",
                                     savefig=False):
    '''
    ...
    for cell_type in cell_types:
        ...
        for index, feature in enumerate(dropouts_to_show):
            ...
            # Plot all cells in active sessions
            summary[cell_type + ' data'][feature] = all_data.groupby(['experience_level', 'targeted_structure'])[
                feature].describe()
            ax[index] = sns.pointplot(
                data=all_data,
                x='experience_level',
...
def get_matched_cells_with_ve(cells_table, results_pivoted, threshold):
    ...
    cells_table = pd.merge(cells_table.reset_index(), results_pivoted[['cell_roi_id', 'variance_explained_full']],
                           on='cell_roi_id', validate='1:1')

    # get cells with maxVE about threshold
    cells_with_ve = cells_table.groupby('cell_specimen_id')['variance_explained_full'].max().to_frame().query(
...
def plot_population_averages(results_pivoted, run_params,
                             dropouts_to_show=['all-images', 'all-omissions', 'behavioral', 'task'], sharey=True,
                             include_zero_cells=True, boxplot=False, add_stats=True, extra='',
                             strict_experience_matching=False, plot_by_cell_type=False, across_session=False,
                             stats_on_across=True, matched_with_variance_explained=False, matched_ve_threshold=0,
                             savefig=False):
    '''
        Plots the average dropout scores for each cre line, on each experience level.
        Includes all cells, and matched only cells.
    ...
    if across_session:
        assert np.all(['_within' in x for x in
                       dropouts_to_show]), 'if across_session, then dropouts_to_show must be within session dropouts'
        assert np.all([x.replace('_within', '_across') in results_pivoted for x in
                       dropouts_to_show]), 'across_session dropout not available'
    ...
    for dropout in dropouts_to_show:
        ...
        if across_session:
            ...
            results_pivoted[dropout.replace('_within', '_across')] = results_pivoted[
                dropout.replace('_within', '_across')].abs()
    ...
    for index, feature in enumerate(dropouts_to_show):
        ...
        for cindex, cell_type in enumerate(cell_types):
            ...
            summary_data[feature + ' data'][cell_type + ' all data'] = all_data.groupby(['experience_level'])[
                feature].describe()
            summary_data[feature + ' data'][cell_type + ' matched data'] = matched_data.groupby(['experience_level'])[
                feature].describe()
            # Plot all cells in active sessions
            if boxplot:
...
def plot_dropout_individual_population(results, run_params, ax=None, palette=None, use_violin=False, add_median=True,
                                       include_zero_cells=True, add_title=False, use_single=False, savefig=False):
    '''
    ...
    '''
    dropouts_to_show = ['all-images', 'image0', 'image1', 'image2', 'image3', 'image4', 'image5', 'image6', 'image7',
                        '', 'all-omissions', 'omissions', 'post-omissions', '', 'behavioral', 'licks', 'pupil',
                        'running', '', 'task', 'hits', 'misses', 'all-hits', 'all-misses', 'post-hits', 'post-misses']
...
def plot_dropout_summary_population(results, run_params,
    ...
    if dropout_cleaning_threshold is not None:
        ...
        data_to_plot.loc[
            data_to_plot['adj_variance_explained_full'] < dropout_cleaning_threshold, 'explained_variance'] = 0
...
def plot_fraction_summary_population(results_pivoted, run_params, sharey=True, kernel_excitation=False, kernel=None,
    ...
    summary_df['n'] = results_pivoted.groupby(['cre_line', 'experience_level'])[
        ['code_anything', 'code_images', 'code_omissions', 'code_behavioral', 'code_task']].count()['code_anything']
    ...
    if kernel_excitation:
        ...
        summary_df = results_pivoted.groupby(['cre_line', 'experience_level'])[
            ['code_anything', 'code_' + kernel, 'code_' + kernel + '_excited', 'code_' + kernel + '_inhibited']].mean()
        summary_df['n'] = results_pivoted.groupby(['cre_line', 'experience_level'])[
        ...
        summary_df['code_' + kernel + '_excited_ci'] = 1.96 * np.sqrt(
            (summary_df['code_' + kernel + '_excited'] * (1 - summary_df['code_' + kernel + '_excited'])) / summary_df[
                'n'])
        summary_df['code_' + kernel + '_inhibited_ci'] = 1.96 * np.sqrt(
            (summary_df['code_' + kernel + '_inhibited'] * (1 - summary_df['code_' + kernel + '_inhibited'])) /
            summary_df['n'])
    ...
    for index, feature in enumerate(coding_groups):
        # plots three cre-lines in standard colors
        ax[index].plot([0, 1, 2], summary_df.loc['Vip-IRES-Cre'][feature], '-', color=colors['Vip-IRES-Cre'],
                       label='Vip Inhibitory', linewidth=3)
        ax[index].plot([0, 1, 2], summary_df.loc['Sst-IRES-Cre'][feature], '-', color=colors['Sst-IRES-Cre'],
                       label='Sst Inhibitory', linewidth=3)
        ax[index].plot([0, 1, 2], summary_df.loc['Slc17a7-IRES2-Cre'][feature], '-', color=colors['Slc17a7-IRES2-Cre'],
                       label='Excitatory', linewidth=3)

        ax[index].errorbar([0, 1, 2], summary_df.loc['Vip-IRES-Cre'][feature],
                           yerr=summary_df.loc['Vip-IRES-Cre'][feature + '_ci'], color=colors['Vip-IRES-Cre'],
                           linewidth=3)
        ax[index].errorbar([0, 1, 2], summary_df.loc['Sst-IRES-Cre'][feature],
                           yerr=summary_df.loc['Sst-IRES-Cre'][feature + '_ci'], color=colors['Sst-IRES-Cre'],
                           linewidth=3)
        ax[index].errorbar([0, 1, 2], summary_df.loc['Slc17a7-IRES2-Cre'][feature],
                           yerr=summary_df.loc['Slc17a7-IRES2-Cre'][feature + '_ci'], color=colors['Slc17a7-IRES2-Cre'],
                           linewidth=3)
...
def plot_all_coding_fraction(results_pivoted, run_params, drop_threshold=0, metric='fraction', compare=['cre_line']):
    ...
    # Set up which sessions to plot
    active_only = ['licks', 'hits', 'misses', 'false_alarms', 'correct_rejects', 'model_bias', 'model_task0',
                   'model_omissions1', 'model_timing1D', 'beh_model', 'licking']
    passive_only = ['passive_change']
    ...
    for dropout in run_params['dropouts']:
        try:
            ...
            plot_coding_fraction(results_pivoted, run_params, dropout, drop_threshold=drop_threshold, savefile=filepath,
                                 session_filter=session, metric=metric, compare=compare)
        except Exception as e:
            print(e)
            # Track failures
            fail.append(dropout)
...
def plot_coding_fraction(results_pivoted_in, run_params, dropout, drop_threshold=0, savefig=True, savefile='',
                         metric='fraction', compare=['cre_line'], area_filter=['VISp', 'VISl'], cell_filter='all',
                         equipment_filter='all', depth_filter=[0, 1000], session_filter=[1, 2, 3, 4, 5, 6]):
    '''
    ...
    for dex, group in enumerate(groups):
        ...
        # Plot
        plot_coding_fraction_inner(plt.gca(), df.loc[group], color, labels_SAC[dex], metric=metric, linestyle=linestyle,
    ...
    if savefig:
        if len(compare) > 0:
            savefile = os.path.join(savefile, 'coding_' + metric + '_by_' + '_'.join(
                compare) + '_' + dropout + filter_string + '.png')
        else:
...
def plot_coding_fraction_inner(ax, df, color, label, metric='fraction', linestyle='-', markerstyle='o'):
    ...
    plt.plot(np.array(range(0, len(frac))), frac, marker=markerstyle, linestyle=linestyle, color=color, linewidth=4,
             label=label)
...
def view_cell_across_sessions(cell_specimen_id, glm_version):
    ...
    for idx, row in dropouts_for_cell.sort_values(by='date_of_acquisition').reset_index().iterrows():
        ...
        mask[mask == 0] = np.nan
        xmin, xmax, ymin, ymax = np.where(mask == 1)[1].min(), np.where(mask == 1)[1].max(), np.where(mask == 1)[
            0].min(), np.where(mask == 1)[0].max()
        ...
        # add a title
        title = '{}\nacquired_on {}\nroi_id {}\nfull model var explained = {:0.1f}%'.format(row['session_type'], row[
            'date_of_acquisition'].split(' ')[0], row['cell_roi_id'], 100 * row['variance_explained'])
    ...
    fig.suptitle(
        'Cell Specimen ID = {}, Cre line = {}, rig = {}, GLM Version = {}'.format(cell_specimen_id, row['cre_line'],
                                                                                  row['equipment_name'], glm_version))

    return fig, axes


def plot_regressor_heatmap_by_cre_line_sorted_by_MI(results_pivoted, session_numbers, regressor,
    ...
    for i, cre_line in enumerate(results_pivoted['cre_lines'].unique()):
        ...
        reg_values['MI'] = [(reg_values.loc[row][session_numbers[1]] - reg_values.loc[row][session_numbers[0]]) / (
                    reg_values.loc[row][session_numbers[1]] + reg_values.loc[row][session_numbers[0]]) for row in
                            reg_values.index.values]
    ...
    fig.suptitle(regressor + ', sessions ' + str(session_numbers[0]) + ' - ' + str(session_numbers[1]), x=0.51, y=0.99,
                 fontsize=22)
    plt.subplots_adjust(wspace=0.5)
    if save_dir:
        if 'single' in regressor:
            utils.save_figure(fig, figsize, os.path.join(save_dir, 'regressor_heatmaps\single'),
                              'sorted_by_MI' + suffix,
                              model_output_type + '_' + regressor + '_' + str(session_numbers[0]) + '_' + str(
                                  session_numbers[1]) + '_MI')
        else:
            utils.save_figure(fig, figsize, os.path.join(save_dir, 'regressor_heatmaps\standard'),
                              'sorted_by_MI' + suffix,
                              model_output_type + '_' + regressor + '_' + str(session_numbers[0]) + '_' + str(
                                  session_numbers[1]) + '_MI')
...
def plot_sample_cells(glm, cell_specimen_ids, t0, t1, figwidth=8, height_per_cell=1, title='cell_specimen_id'):
    ...
    '''
    fig, ax = plt.subplots(len(cell_specimen_ids), 1, figsize=(figwidth, height_per_cell * len(cell_specimen_ids)),
                           sharex=True)
...
def plot_sem_distribution(results_pivoted, cres=None):
    ...
    for cre in cres:
        plt.hist(results_pivoted.query('cre_line == @cre')['variance_explained_full_sem'], bins=100, alpha=.25,
                 range=(0, .1), density=True, label=cre, color=project_colors()[cre])
...
def plot_sem_comparison(results_pivoted):
    ...
    for cre in cres:
        ...
        plt.plot(cre_slice['variance_explained_full'], cre_slice['variance_explained_full_sem'], 'o', alpha=.1,
...
def compare_events_and_dff(results_pivoted_dff, results_pivoted_events, savefig=False, versions=None):
    ...
    for cre in cres:
        ...
        ax[0].plot(cre_slice['variance_explained_full_dff'], cre_slice['variance_explained_full_events'], 'o', alpha=.2,
                   color=project_colors()[cre], label=cre)
...
def shuffle_analysis(results, run_params, bins=50, savefig=True, shuffle='time'):
    ...
    '''
    plt.figure()
    plt.hist(results.query('dropout=="Full"')['shuffle_' + shuffle], bins=bins, label='Shuffle {}'.format(shuffle),
             density=True)
    plt.ylabel('Density')
    plt.xlabel('Variance explained, shuffling across {}'.format(shuffle))
    maxve = results.query('dropout=="Full"')['shuffle_' + shuffle].max()
    plt.axvline(maxve, color='r', linestyle='-', alpha=1, label='Max shuffled')
    plt.axvline(run_params['dropout_threshold'], color='m', linestyle='--', alpha=1,
...
def clustering_kernels(weights_df, run_params, kernel, just_coding=False, pca_by_experience=True):
    ...
    for index, cre_line in enumerate(cre_lines):
        if just_coding:
            weights = weights_df.query(
                '(cre_line == @cre_line) & (ophys_session_id not in @problem_sessions) &(passive==False)&({0} <0)'.format(
                    kernel)).copy()
        else:
            weights = weights_df.query(
                '(cre_line == @cre_line) & (ophys_session_id not in @problem_sessions) &(passive==False)').copy()
        ...
        for eindex, experience_level in enumerate(['Familiar', 'Novel 1', 'Novel >1']):
            if pca_by_experience:
                ...
                ax[index, 0].plot(pca.explained_variance_ratio_, 'o-', label=experience_level,
                                  color=colors[experience_level])
                ax[index, 1].plot(time_vec, pca.components_[0, :], label=experience_level,
                                  color=colors[experience_level])
                ax[index, 0].legend()
            else:
                sorted_x = np.vstack(weights.query('experience_level ==@experience_level')[kernel + '_weights'].values)

            cbar = ax[index, 2 + eindex].imshow(sorted_x, aspect='auto',
                                                extent=[time_vec[0], time_vec[-1], 0, np.shape(sorted_x)[1]],
                                                cmap='bwr')
...
def depth_heatmap(weights_df, run_params, metric='omission_responsive', just_coding=False, just_mesoscope=False):
    ...
    if just_coding:
        ...
        fraction['n'] = df.query('omissions < 0').groupby(['cre_line', 'targeted_structure', 'binned_depth'])[
            metric].count()
    else:
