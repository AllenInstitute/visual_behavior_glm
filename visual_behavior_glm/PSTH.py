    ...
    for cre_dex, cre in enumerate(cres):
        for e in experience:
            for c in conditions:
                try:
                    plot_heatmap(
                        dfs[cre_dex], cre, c, e, savefig=True, data=data)
...
def compare_conditions(
    ...
    num_rows = len(dfs)
    fig, ax = plt.subplots(num_rows, 3, figsize=(
        10, 2.75 * num_rows), sharey='row', squeeze=False)
    ...
    for index, full_df, in enumerate(dfs):
        ...
        for cdex, condition in enumerate(conditions):
            color = colors(cdex + 2)
            max_y.append(plot_condition_experience(full_df,
                                                   condition,
                                                   'Familiar',
...
def plot_condition_by_depth(
    ...
    num_rows = len(dfs)
    num_cols = len(depths)
    fig, ax = plt.subplots(num_rows, num_cols, figsize=(
        (10 / 3) * num_cols, 2.75 * num_rows), sharey='row', squeeze=False)
    ...
    for index, full_df in enumerate(dfs):
        ...
        for dex, col in enumerate(depths):
            max_y[dex] = plot_condition_experience(full_df,
                                                   condition,
                                                   'Familiar',
                                                   'visual_strategy_session',
                                                   ax=ax[index,
...
def plot_condition(
    ...
    for index, full_df in enumerate(dfs):
        ...
        if not split_by_engaged:
            max_y = [0, 0, 0]
            max_y[0] = plot_condition_experience(full_df,
                                                 condition,
                                                 'Familiar',
                                                 'visual_strategy_session',
                                                 ax=ax[index,
                                                 ...
                                                 depths=depths,
                                                 depth=depth)
            max_y[1] = plot_condition_experience(full_df,
                                                 condition,
                                                 'Novel 1',
                                                 'visual_strategy_session',
                                                 ax=ax[index,
                                                 ...
                                                 error_type=error_type,
                                                 depth=depth)
            max_y[2] = plot_condition_experience(full_df,
                                                 condition,
                                                 'Novel >1',
                                                 'visual_strategy_session',
                                                 ax=ax[index,
            ...
            max_y = []
            temp = plot_condition_experience(full_df,
                                             'engaged_v1_' + condition,
                                             'Familiar',
                                             'visual_strategy_session',
                                             ax=ax[index,
                                                   0],
                                             ...
                                             ylabel=ylabel,
                                             error_type=error_type,
                                             split_by_engaged=True,
                                             plot_strategy=plot_strategy,
                                             depth=depth)
            max_y.append(temp)
            temp = plot_condition_experience(full_df,
                                             'disengaged_v1_' + condition,
                                             'Familiar',
                                             'visual_strategy_session',
                                             ax=ax[index,
                                                   0],
...
def get_figure_4_psth(
        data='events',
        experience_level='Familiar',
        mesoscope_only=False):

    # Load each cell type
    vip_full_filtered = bd.load_population_df(
        data, 'full_df', 'Vip-IRES-Cre', experience_level=experience_level)
    sst_full_filtered = bd.load_population_df(
        data, 'full_df', 'Sst-IRES-Cre', experience_level=experience_level)
    exc_full_filtered = bd.load_population_df(
        data, 'full_df', 'Slc17a7-IRES2-Cre', experience_level=experience_level)
    ...
    if mesoscope_only:
        ...
        vip_full_filtered = vip_full_filtered.query(
            'equipment_name == "MESO.1"').copy()
        sst_full_filtered = pd.merge(sst_full_filtered,
                                     experiment_table[['ophys_experiment_id', 'equipment_name']],
                                     on='ophys_experiment_id')
        sst_full_filtered = sst_full_filtered.query(
            'equipment_name == "MESO.1"').copy()
        exc_full_filtered = pd.merge(exc_full_filtered,
                                     experiment_table[['ophys_experiment_id', 'equipment_name']],
                                     on='ophys_experiment_id')
        exc_full_filtered = exc_full_filtered.query(
            'equipment_name == "MESO.1"').copy()
...
def plot_figure_4_averages(
    ...
    fig, ax = plt.subplots(3, 3, figsize=(10, 7.75),
...
def add_cell_selection_labels(dfs, summary_df):
    for i in [0, 1, 2]:
        dfs[i] = pd.merge(dfs[i],
                          summary_df[['behavior_session_id',
                                      'strategy_labels',
                                      'strategy_labels_with_mixed',
                                      'strategy_labels_with_none']],
                          on='behavior_session_id')
    return dfs


def plot_figure_4_averages_cell_selection(
        dfs,
        data='filtered_events',
        savefig=False,
        areas=[
...
def plot_engagement(
    ...
    for index, full_df in enumerate(dfs):
        max_y = [0, 0, 0]
        ylabel = labels[index] + '\n(Ca$^{2+}$ events)'
        max_y[0] = plot_condition_engagement(full_df,
                                             'omission',
                                             ax=ax[index,
                                                   0],
                                             ylabel=ylabel,
                                             ...
                                             depths=depths,
                                             version=version)
        max_y[1] = plot_condition_engagement(full_df,
                                             'miss',
                                             ax=ax[index,
                                                   1],
                                             ylabel='',
                                             ...
                                             depths=depths,
                                             version=version)
        max_y[2] = plot_condition_engagement(full_df,
                                             'image',
                                             ax=ax[index,
                                                   2],
                                             ylabel='',
...
def plot_condition_engagement(
    ...
    df = full_df.query(
        '(condition ==@condition1)&(experience_level ==@experience_level)' +
        '&(targeted_structure in @areas)&({} in @depths)'.format(depth))
    r = plot_split(df.query('not visual_strategy_session'), ax,
                   color=colors['timing'], error_type=error_type)
    responses.append(r)
    r = plot_split(
        df.query('visual_strategy_session'),
        ax,
        color=colors['visual'],
        error_type=error_type)
    responses.append(r)
    ...
    df = full_df.query(
        '(condition ==@condition2)&(experience_level ==@experience_level)' +
        '&(targeted_structure in @areas)&({} in @depths)'.format(depth))
    r = plot_split(df.query('not visual_strategy_session'), ax,
                   color='lightblue', error_type=error_type)
    responses.append(r)
...
def plot_condition_experience(
    ...
    df = full_df.query(
        '(condition ==@condition)&(experience_level ==@experience_level)&(targeted_structure in @areas)&({} in @depths)'.format(depth))
    colors = gvt.project_colors()
...
def plot_heatmap(full_df, cre, condition, experience_level, savefig=False,
    ...
    # process data
    df = full_df .query(
        '(condition==@condition)&(experience_level==@experience_level)') .copy()
    ...
    ax2.imshow(x_visual, aspect='auto', vmax=vmax,
               extent=[time[0], time[-1], 0, num_visual])
    ...
    ax1.imshow(x_timing, aspect='auto', vmax=vmax,
               extent=[time[0], time[-1], 0, num_timing])
...
def plot_QQ_engagement(
    ...
    df_e = full_df .query(
        '(condition==@e_condition)&(experience_level==@experience_level)') .copy()
    df_e['max'] = [np.nanmax(x) for x in df_e['response']]

    df_d = full_df .query(
        '(condition==@d_condition)&(experience_level==@experience_level)') .copy()
...
def plot_strategy_histogram(
    ...
    if ax is None:
        fig, ax = plt.subplots()
    vis, bins, _ = ax.hist(x, bins=bins, color='darkorange',
                           alpha=.5, density=True, label='visual')
    time, bins, _ = ax.hist(y, bins=bins, color='blue',
                            alpha=.5, density=True, label='timing')
...
def compute_running_bootstrap_bin(
        df,
        condition,
        cell_type,
        bin_num,
    ...
    filename = get_hierarchy_filename(
        cell_type,
        condition,
        data,
        'all',
    ...
    temp = df.query('running_bins == @bin_num')[
        ['visual_strategy_session', 'ophys_experiment_id', 'cell_specimen_id', 'response']]
    means = hb.bootstrap(
        temp,
        levels=[
            'visual_strategy_session',
...
def compute_running_bootstrap(
    ...
    for b in bins:
        ...
        if os.path.isfile(filename):
            ...
            temp = df.query(
                'running_bins == @b')[[split, 'ophys_experiment_id', 'cell_specimen_id', 'response']]
            means = hb.bootstrap(
                temp,
                levels=[
                    split,
    ...
    bootstraps['p_boot'] = [
        0.5 if (
            (row.visual_sem == 0) or (
                row.timing_sem == 0)) else row.p_boot for index,
        row in bootstraps.iterrows()]
    bootstraps['p_boot'] = [
        1 - x if x > .5 else x for x in bootstraps['p_boot']]
    ...
    # Save file
    if not missing:
        filepath = get_hierarchy_filename(
            cell_type,
            condition,
            data,
            'all',
...
def compute_pre_change_running_bootstrap_bin(
        df,
        condition,
        cell_type,
        strategy,
    ...
    # Compute bootstrap for this bin
    temp = df.query('running_bins == @bin_num')[
        ['pre_hit_1', 'ophys_experiment_id', 'cell_specimen_id', 'response']]
    means = hb.bootstrap(
        temp,
        levels=[
            'pre_hit_1',
...
def compute_pre_change_running_bootstrap(
        df,
        condition,
        cell_type,
        strategy,
    ...
    bootstraps['p_boot'] = [
        0.5 if (
            (row.pre_hit_sem == 0) or (
                row.pre_miss_sem == 0)) else row.p_boot for index,
        row in bootstraps.iterrows()]
    bootstraps['p_boot'] = [
        1 - x if x > .5 else x for x in bootstraps['p_boot']]
    bootstraps['ind_significant'] = bootstraps['p_boot'] <= 0.05
    ...
    if not missing:
        filepath = get_hierarchy_filename(
            ...
            nboots,
            ['visual_strategy_session'],
            'running_pre_change_{}'.format(strategy))
        print('saving bootstraps to: ' + filepath)
        bootstraps.to_feather(filepath)
    return bootstraps


def compute_engagement_running_bootstrap_bin(
        df,
        condition,
        cell_type,
        strategy,
    ...
    # Compute bootstrap for this bin
    temp = df.query('running_bins == @bin_num')[
        ['engagement_v2', 'ophys_experiment_id', 'cell_specimen_id', 'response']]
    means = hb.bootstrap(
        temp,
        levels=[
            'engagement_v2',
...
def compute_engagement_running_bootstrap(
        df,
        condition,
        cell_type,
        strategy,
    ...
    bootstraps['p_boot'] = [
        0.5 if (
            (row.engaged_sem == 0) or (
                row.disengaged_sem == 0)) else row.p_boot for index,
        row in bootstraps.iterrows()]
    bootstraps['p_boot'] = [
        1 - x if x > .5 else x for x in bootstraps['p_boot']]
    bootstraps['ind_significant'] = bootstraps['p_boot'] <= 0.05
    ...
    if not missing:
        filepath = get_hierarchy_filename(
            ...
            nboots,
            ['visual_strategy_session'],
            'running_engaged_{}'.format(strategy),
            meso=meso,
            first=first,
            second=second)
        print('saving bootstraps to: ' + filepath)
...
def get_engagement_running_bootstraps(
        cell_type,
        condition,
        data,
        nboots,
...
def get_pre_change_running_bootstraps(
        cell_type, condition, data, nboots, strategy):
    filepath = get_hierarchy_filename(
        cell_type,
        condition,
        data,
    ...
    if bootstraps is not None:
        ...
        for b in visual['running_bins'].unique():
            if b in bootstraps.index.values:
                plt.errorbar(
                    b * bin_width,
                    vtemp.loc[b].response,
                    yerr=bootstraps.loc[b]['visual_sem'],
                    color=vis_color,
                    fmt='o')
            else:
                print('missing value')
        for b in timing['running_bins'].unique():
            if b in bootstraps.index.values:
                plt.errorbar(
                    b * bin_width,
                    ttemp.loc[b].response,
                    yerr=bootstraps.loc[b]['timing_sem'],
                    color=tim_color,
                    fmt='o')
            else:
                print('missing value')
    ...
    if (bootstraps is not None) and ('bh_significant' in bootstraps.columns):
        ...
        for index, row in bootstraps.iterrows():
            if row.bh_significant:
                if row.remove:
                    print(
                        'Not significant b/c of low count: {}'.format(row.running_bin))
                else:
...
def pre_change_running_responses(
        df,
        condition,
        cre='vip',
        bootstraps=None,
    ...
    counts = df.groupby(['running_bins', 'visual_strategy_session'])[
        'response'].count().unstack().reset_index()
    counts['remove'] = [
        (row[False] < min_events) or (
            row[True] < min_events) for index,
        row in counts.iterrows()]
    counts['running_bin'] = counts['running_bins'].astype(int)
    if bootstraps is not None:
        bootstraps = pd.merge(bootstraps, counts[['running_bin', 'remove']],
                              on='running_bin')
    ...
    if bootstraps is not None:
        ...
        bootstraps = bootstraps.set_index('running_bin')
        bins = set(bootstraps.index.values).intersection(
            set(temp1.index.values))
        for b in bins:
            plt.errorbar(
                b * bin_width,
                temp1.loc[b].response,
                yerr=bootstraps.loc[b]['pre_hit_sem'],
                color=hit_color,
                fmt='o')
        bins = set(bootstraps.index.values).intersection(
            set(temp2.index.values))
        for b in bins:
            plt.errorbar(
                b * bin_width,
                temp2.loc[b].response,
                yerr=bootstraps.loc[b]['pre_miss_sem'],
                color=mis_color,
                fmt='x')
...
def engagement_running_responses(
    df,
    condition,
    cre='vip',
    vis_boots=None,
    ...
    dtiming = summary.query(
        '(not visual_strategy_session) &(not engagement_v2)')
    ...
    evisual_sem = summary_sem.query(
        '(visual_strategy_session) &(engagement_v2)')
    dvisual_sem = summary_sem.query(
        '(visual_strategy_session) &(not engagement_v2)')
    etiming_sem = summary_sem.query(
        '(not visual_strategy_session) &(engagement_v2)')
    dtiming_sem = summary_sem.query(
        '(not visual_strategy_session) &(not engagement_v2)')
    ...
    if 'visual' in plot_list:
        if vis_boots is not None:
            ...
            bins = set(vis_boots.index.values).intersection(
                set(evis.index.values))
            for b in bins:
                plt.errorbar(
                    b * bin_width,
                    evis.loc[b].response,
                    yerr=vis_boots.loc[b]['engaged_sem'],
                    color=evis_color,
                    fmt='o')
                plt.errorbar(
                    b * bin_width,
                    dvis.loc[b].response,
                    yerr=vis_boots.loc[b]['disengaged_sem'],
                    color=dvis_color,
                    fmt='o')
        else:
            plt.errorbar(
                evisual.running_bins *
                bin_width,
                evisual.response,
                yerr=evisual_sem.response,
                color=evis_color,
                fmt='o',
                label=evis_label)
            plt.errorbar(
                dvisual.running_bins *
                bin_width,
                dvisual.response,
                yerr=dvisual_sem.response,
                color=dvis_color,
                fmt='o',
                label=dvis_label)
    if 'timing' in plot_list:
        if tim_boots is not None:
            ...
            bins = set(tim_boots.index.values).intersection(
                set(etim.index.values))
            for b in bins:
                plt.errorbar(
                    b * bin_width,
                    etim.loc[b].response,
                    yerr=tim_boots.loc[b]['engaged_sem'],
                    color=etim_color,
                    fmt='o')
                plt.errorbar(
                    b * bin_width,
                    dtim.loc[b].response,
                    yerr=tim_boots.loc[b]['disengaged_sem'],
                    color=dtim_color,
                    fmt='o')
        else:
            plt.errorbar(
                etiming.running_bins *
                bin_width,
                etiming.response,
                yerr=etiming_sem.response,
                color=etim_color,
                fmt='o',
                label=etim_label)
            plt.errorbar(
                dtiming.running_bins *
                bin_width,
                dtiming.response,
                yerr=dtiming_sem.response,
                color=dtim_color,
                fmt='o',
                label=dtim_label)
...
def compute_summary_bootstrap_image_strategy(
        df,
        data='events',
        nboots=10000,
        cell_type='exc',
    ...
    filepath = PSTH_DIR + data + '/bootstraps/' + \
        cell_type + '_image_strategy_summary_' + str(nboots)
    if first:
        filepath += '_first'
    if second:
        filepath += '_second'
...
def get_summary_bootstrap_image_strategy(
        data='events',
        nboots=10000,
        cell_type='exc',
        first=True,
...
def plot_summary_bootstrap_image_strategy(
        df,
        cell_type,
        savefig=False,
        data='events',
    ...
    ax.plot([0, 0], [visual_mean -
                     visual_sem, visual_mean +
                     visual_sem], '-', color='darkorange')
    ax.plot([1, 1], [timing_mean -
                     timing_sem, timing_mean +
                     timing_sem], '-', color='blue')
    ...
    if savefig:
        filepath = PSTH_DIR + data + '/summary/' + \
            cell_type + '_image_strategy_summary_' + str(nboots)
        if first:
            filepath += '_first'
        if second:
...
def compute_summary_bootstrap_omission_strategy(
        df,
        data='events',
        nboots=10000,
        cell_type='exc',
    ...
    filepath = PSTH_DIR + data + '/bootstraps/' + \
        cell_type + '_omission_strategy_summary_' + str(nboots)
    if first:
        filepath += '_first'
    if second:
        filepath += '_second'
...
def get_summary_bootstrap_omission_strategy(
        data='events',
        nboots=10000,
        cell_type='exc',
        first=True,
...
def plot_summary_bootstrap_omission_strategy(
        df,
        cell_type,
        savefig=False,
        data='events',
    ...
    bootstrap = get_summary_bootstrap_omission_strategy(
        data, nboots, cell_type, first, second, post, meso, image)
    ...
    if savefig:
        filepath = PSTH_DIR + data + '/summary/' + cell_type + \
            '_omission_strategy_summary_' + str(nboots)
        if first:
            filepath += '_first'
        if second:
...
def compute_summary_bootstrap_strategy_omission_lick(
        df,
        data='events',
        nboots=10000,
        cell_type='exc',
    ...
    df['group'] = df['visual_strategy_session'].astype(
        str) + df['pre_omission_lick'].astype(str)
    mapper = {
    ...
    filepath = PSTH_DIR + data + '/bootstraps/' + cell_type + \
        '_omission_lick_strategy_summary_' + str(nboots)
    if first:
        filepath += '_first'
    if second:
        filepath += '_second'
...
def get_summary_bootstrap_strategy_omission_lick(
        data='events',
        nboots=10000,
        cell_type='exc',
        first=True,
...
def plot_summary_bootstrap_strategy_omission_lick(
        df,
        cell_type,
        savefig=False,
        data='events',
    ...
    bootstrap = get_summary_bootstrap_strategy_omission_lick(
        data, nboots, cell_type, first, second)

    fig, ax = plt.subplots(figsize=(2.5, 2.75))
    visual_lick_mean = df.query('(visual_strategy_session)&(pre_omission_lick)')[
        'response'].mean()
    visual_no_lick_mean = df.query(
        '(visual_strategy_session)&(pre_omission_no_lick)')['response'].mean()
    timing_lick_mean = df.query('(not visual_strategy_session)&(pre_omission_lick)')[
        'response'].mean()
    timing_no_lick_mean = df.query(
        '(not visual_strategy_session)&(pre_omission_no_lick)')['response'].mean()
    ...
    plt.plot(-.05, visual_lick_mean, 'o',
             color='darkorange', label='visual lick')
    plt.plot(
        0.05,
        visual_no_lick_mean,
        'x',
        color='darkorange',
        label='visual no_lick')
    ...
    plt.plot([-.05, -0.05], [visual_lick_mean - visual_lick_sem,
             visual_lick_mean + visual_lick_sem], '-', color='darkorange')
    plt.plot([.05, 0.05], [visual_no_lick_mean -
                           visual_no_lick_sem, visual_no_lick_mean +
                           visual_no_lick_sem], '-', color='darkorange')
    plt.plot([.95, .95], [timing_lick_mean -
                          timing_lick_sem, timing_lick_mean +
                          timing_lick_sem], '-', color='blue')
    plt.plot([1.05, 1.05], [timing_no_lick_mean - timing_no_lick_sem,
             timing_no_lick_mean + timing_no_lick_sem], '-', color='blue')
...
def compute_summary_bootstrap_strategy_pre_change(
        df,
        data='events',
        nboots=10000,
        cell_type='exc',
    ...
    filepath = PSTH_DIR + data + '/bootstraps/' + cell_type + \
        '_pre_change_strategy_summary_' + str(nboots)
    if first:
        filepath += '_first'
    if second:
        filepath += '_second'
...
def get_summary_bootstrap_strategy_pre_change(
        data='events',
        nboots=10000,
        cell_type='exc',
        first=True,
...
def plot_summary_bootstrap_strategy_pre_change(
        df,
        cell_type,
        savefig=False,
        data='events',
    ...
    df = df.query('(pre_hit_1 ==1)or(pre_miss_1==1)').copy()
    bootstrap = get_summary_bootstrap_strategy_pre_change(
        data, nboots, cell_type, first, second)

    fig, ax = plt.subplots(figsize=(2.5, 2.75))
    visual_hit_mean = df.query('(visual_strategy_session)&(pre_hit_1==1)')[
        'response'].mean()
    visual_miss_mean = df.query('(visual_strategy_session)&(pre_hit_1==0)')[
        'response'].mean()
    timing_hit_mean = df.query('(not visual_strategy_session)&(pre_hit_1==1)')[
        'response'].mean()
    timing_miss_mean = df.query('(not visual_strategy_session)&(pre_hit_1==0)')[
        'response'].mean()
    ...
    plt.plot([x1, x1], [visual_hit_mean -
                        visual_hit_sem, visual_hit_mean +
                        visual_hit_sem], '-', color='darkorange')
    plt.plot([x2, x2], [visual_miss_mean -
                        visual_miss_sem, visual_miss_mean +
                        visual_miss_sem], '-', color='darkorange')
    plt.plot([x3, x3], [timing_hit_mean -
                        timing_hit_sem, timing_hit_mean +
                        timing_hit_sem], '-', color='blue')
    plt.plot([x4, x4], [timing_miss_mean -
                        timing_miss_sem, timing_miss_mean +
                        timing_miss_sem], '-', color='blue')
    ...
    print_bootstrap_summary(
        means, bootstrap, phh, keys=[
            'visual_hit', 'timing_hit'])
    print_bootstrap_summary(
        means, bootstrap, pmm, keys=[
            'visual_miss', 'timing_miss'])
    print_bootstrap_summary(
        means, bootstrap, pvhm, keys=[
            'visual_hit', 'visual_miss'])
    print_bootstrap_summary(
        means, bootstrap, pthm, keys=[
...
def compute_summary_bootstrap_strategy_hit(
        df,
        data='events',
        nboots=10000,
        cell_type='exc',
    ...
    filepath = PSTH_DIR + data + '/bootstraps/' + \
        cell_type + '_hit_strategy_summary_' + str(nboots)
    if first:
        filepath += '_first'
    if second:
        filepath += '_second'
...
def get_summary_bootstrap_strategy_hit(
        data='events',
        nboots=10000,
        cell_type='exc',
        first=True,
...
def plot_summary_bootstrap_strategy_hit(
        df,
        cell_type,
        savefig=False,
        data='events',
    ...
    fig, ax = plt.subplots(figsize=(2.5, 2.75))
    visual_hit_mean = df.query('(visual_strategy_session)&(hit==1)')[
        'response'].mean()
    visual_miss_mean = df.query('(visual_strategy_session)&(hit==0)')[
        'response'].mean()
    timing_hit_mean = df.query('(not visual_strategy_session)&(hit==1)')[
        'response'].mean()
    timing_miss_mean = df.query('(not visual_strategy_session)&(hit==0)')[
        'response'].mean()
    visual_hit_sem = np.std(bootstrap['visual_hit'])
    timing_hit_sem = np.std(bootstrap['timing_hit'])
    visual_miss_sem = np.std(bootstrap['visual_miss'])
...
def compute_summary_bootstrap_strategy_engaged_miss(
        df,
        data='events',
        nboots=10000,
        cell_type='exc',
...
def get_summary_bootstrap_strategy_engaged_miss(
        data='events',
        nboots=10000,
        cell_type='exc',
        first=True,
...
def plot_summary_bootstrap_strategy_engaged_miss(
        df,
        cell_type,
        savefig=False,
        data='events',
    ...
    bootstrap = get_summary_bootstrap_strategy_engaged_miss(
        data, nboots, cell_type, first, second, image=image, meso=meso)

    fig, ax = plt.subplots(figsize=(3, 2.75))
    df = df.query('miss==1').copy()
    visual_engaged_mean = df.query('(visual_strategy_session)&(engagement_v2)')[
        'response'].mean()
    visual_disengaged_mean = df.query(
        '(visual_strategy_session)&(not engagement_v2)')['response'].mean()
    timing_engaged_mean = df.query('(not visual_strategy_session)&(engagement_v2)')[
        'response'].mean()
    timing_disengaged_mean = df.query(
        '(not visual_strategy_session)&(not engagement_v2)')['response'].mean()
    ...
    plt.plot(-.05, visual_engaged_mean, 'x',
             color='darkorange', label='visual engaged')
    plt.plot(
        0.05,
        visual_disengaged_mean,
        'x',
        color='burlywood',
        label='visual disengaged')
    plt.plot(.95, timing_engaged_mean, 'x',
             color='blue', label='timing engaged')
    plt.plot(
        1.05,
        timing_disengaged_mean,
        'x',
        color='lightblue',
        label='timing disengaged')
    ...
    pdd = bootstrap_significance(
        bootstrap,
        'visual_disengaged',
        'timing_disengaged')
    pved = bootstrap_significance(
        bootstrap,
        'visual_engaged',
        'visual_disengaged')
    pted = bootstrap_significance(
        bootstrap,
        'timing_engaged',
        'timing_disengaged')
    print_bootstrap_summary(
        means, bootstrap, pee, keys=[
            'visual_engaged', 'timing_engaged'])
    print_bootstrap_summary(
        means, bootstrap, pdd, keys=[
            'visual_disengaged', 'timing_disengaged'])
    print_bootstrap_summary(
        means, bootstrap, pved, keys=[
            'visual_engaged', 'visual_disengaged'])
    print_bootstrap_summary(
        means, bootstrap, pted, keys=[
            'timing_engaged', 'timing_disengaged'])


def compute_summary_bootstrap_strategy_engaged_omission(
        df,
        data='events',
        nboots=10000,
        cell_type='exc',
...
def get_summary_bootstrap_strategy_engaged_omission(
        data='events',
        nboots=10000,
        cell_type='exc',
        first=True,
...
def plot_summary_bootstrap_strategy_engaged_omission(
        df,
        cell_type,
        savefig=False,
        data='events',
    ...
    bootstrap = get_summary_bootstrap_strategy_engaged_omission(
        data, nboots, cell_type, first, second, post, meso, image)
...
def load_df_and_compute_running(
        summary_df,
        cell_type,
        response,
        data,
...
def load_df_and_compute_engagement_running(
        summary_df,
        cell_type,
        response,
        data,
    ...
    if response == 'image':
        ...
        df = load_omission_df(summary_df, mapper[cell_type], data,
                              meso=meso, first=first, second=second)
    compute_engagement_running_bootstrap_bin(
        df,
        response,
        cell_type,
        'visual',
...
def load_df_and_compute_hierarchy(
        summary_df,
        cell_type,
        response,
        data,
...
def compute_hierarchy(
    ...
    counts = df.groupby(splits + ['targeted_structure',
                                  depth])[['ophys_experiment_id',
                                           'cell_specimen_id']].nunique().reset_index()
    mean_df = pd.merge(mean_df, counts, on=splits +
                       ['targeted_structure', depth], validate='1:1')
    ...
    if bootstrap:
        ...
        for index, row in groups.iterrows():
            ...
            if len(splits) == 0:
                bootstrap = hb.bootstrap(temp,
                                         levels=splits + ['ophys_experiment_id',
                                                          'cell_specimen_id'],
                                         nboots=nboots,
                                         no_top_level=True)
                sem = np.std(bootstrap)
                dex = (
                    mean_df['targeted_structure'] == row.targeted_structure) & (
                ...
                if len(keys) == 2:
                    diff = np.array(bootstrap[keys[0]]) > np.array(
                        bootstrap[keys[1]])
        ...
        if len(splits) > 0:
            mean_df['p_boot'] = [
                1 - x if x > .5 else x for x in mean_df['p_boot']]
            ...
            if strategy is not None:
                ...
                if strategy == 'visual':
                    hierarchy = hierarchy.query(
                        'visual_strategy_session').copy()
                    label = 'visual ' + cell_type + ' ' + r
                else:
                    hierarchy = hierarchy.query(
                        'not visual_strategy_session').copy()
                    label = 'timing ' + cell_type + ' ' + r
            else:
                hierarchy = get_hierarchy(
                    cell_type, r, data, depth, nboots, splits, extra, first, second)
                label = cell_type + ' ' + r
            ax = plot_hierarchy(
                hierarchy,
                cell_type,
        ...
        for index, value in enumerate(splits):
            ...
            for sdex, split_value in enumerate(unique):
                ...
                if isinstance(split_value, str):
                    ...
                    temp = hierarchy.query(
                        '{} == {}'.format(
                            value, split_value))
                ax.errorbar(
                    temp['xloc'],
                    temp['response'],
                    yerr=temp['bootstrap_sem'],
                    fmt='o',
                    color=color,
                    alpha=.5)
                ax.plot(
...
def load_change_df(
    ...
    df = pd.merge(df,
                  experiment_table.reset_index()[['ophys_experiment_id',
                                                  'binned_depth',
                                                  'equipment_name']],
                  on='ophys_experiment_id')
    df = pd.merge(df,
...
def load_image_df(
    ...
    df.drop(df[~df['behavior_session_id'].isin(
        familiar_bsid)].index, inplace=True)

    # Add additional details
    df = pd.merge(
        df, summary_df[['behavior_session_id', 'visual_strategy_session']])
    experiment_table = glm_params.get_experiment_table()
    df = bd.add_area_depth(df, experiment_table)
    df = pd.merge(df,
                  experiment_table.reset_index()[['ophys_experiment_id',
...
def load_image_and_change_df(
    ...
    experiment_table = glm_params.get_experiment_table()
    df = bd.add_area_depth(df, experiment_table)
    df = pd.merge(df, experiment_table.reset_index()[
                  ['ophys_experiment_id', 'binned_depth']], on='ophys_experiment_id')
    return df


def load_omission_df(
        summary_df,
        cre,
        data='events',
        first=False,
...
def load_vip_omission_df(summary_df, bootstrap=False, data='events'):
    ...
    vip_image_filtered = bd.load_population_df(
        data, 'image_df', 'Vip-IRES-Cre')
    ...
    if bootstrap:
        # processing
        print('bootstrapping')
        vip_omission = vip_omission[['visual_strategy_session',
                                     'ophys_experiment_id',
                                     'cell_specimen_id',
                                     'stimulus_presentations_id',
                                     'response']]
        bootstrap_means = hb.bootstrap(
            vip_omission,
            levels=[
                'visual_strategy_session',
                'ophys_experiment_id',
...
def plot_exc_change_summary():
    ...
    plt.plot(-.05, visual.loc[1]['response'], 'o',
             color='darkorange', label='visual hit')
    plt.plot(
        0.05,
        visual.loc[0]['response'],
        'x',
        color='darkorange',
        label='visual miss')
    plt.plot(.95, timing.loc[1]['response'], 'o',
             color='blue', label='timing hit')
    plt.plot(
        1.05,
        timing.loc[0]['response'],
        'x',
        color='blue',
        label='timing mis')
...
def plot_figure_4_by_equipment(
        sci_df,
        mes_df,
        cell_type,
        cell_counts,
    ...
    fig, ax = plt.subplots(
        3, 3, figsize=(
            10, 7.5), sharey='row', squeeze=False)
    labels = [
        'Sci. ' +
        cell_type,
        'Meso. ' +
    ...
    exclusion_df = exclusion_df. query(
        '~((targeted_structure == @areas[0])&(binned_depth ==@depths[0]))')
    max_y = [0, 0, 0]
    index = 2
    ylabel = labels[index] + '\n(Ca$^{2+}$ events)'
    max_y[0] = plot_condition_experience(exclusion_df,
                                         'omission',
                                         experience_level,
                                         strategy,
                                         ax=ax[index,
    ...
    max_y[1] = plot_condition_experience(exclusion_df, 'hit', experience_level,
                                         ...
                                         error_type=error_type)
    max_y[2] = plot_condition_experience(exclusion_df,
                                         'miss',
                                         experience_level,
                                         strategy,
                                         ax=ax[index,
    ...
    title = (
        ...
        'Mesoscope cells: {: >8.0f} visual, {: >8.0f} timing'). format(
        cell_type,
        areas[0],
        depths[0],
        sci_vis,
    ...
    plt.tight_layout()
    if savefig:
        filename = PSTH_DIR + data + '/population_averages/' + 'equipment_psth_' + \
            cell_type + '_{}_{}.png'.format(depths[0], areas[0])
        print('Figure saved to: ' + filename)
        plt.savefig(filename)
...
def get_equipment_counts(BEHAVIOR_VERSION=21):
    ...
    experiment_table = glm_params.get_experiment_table()
    cell_table = pd.merge(cell_table, experiment_table.reset_index()[
                          ['ophys_experiment_id', 'area_binned_depth']], on='ophys_experiment_id')
    ...
    cell_table = pd.merge(cell_table,
                          summary_df[['behavior_session_id',
                                      'strategy_labels']],
                          on='behavior_session_id')
    counts = cell_table.groupby(['cre_line', 'equipment', 'area_binned_depth', 'strategy_labels'])[
        'cell_specimen_id'].nunique().unstack().fillna(value=0)
    return counts
...
def plot_equipment_comparison(
    ...
    all_df = pd.merge(all_df, experiment_table.reset_index()[
                      ['ophys_experiment_id', 'equipment_name']], on='ophys_experiment_id')
    ...
    plot_equipment_comparison_inner(
        df,
        cell_type,
        comparison_type,
        depths,
...
def plot_equipment_comparison_inner(
        df,
        cell_type,
        comparison_type,
        depths,
    ...
    if comparison_type == 'omission':
        for index, val in enumerate(['MESO', 'SCI']):
            for jndex, jval in enumerate([True, False]):
                ...
                ax[index].plot([jndex + 1, jndex + 1], [mean -
                               sem, mean + sem], '-', color=colors[jndex])
            ...
            for jndex, jval in enumerate([True, False]):
                for kndex, kval, in enumerate([0, 1]):
                    ...
                    ax[index].plot(
                        ...
                        markers[kndex],
                        color=colors[jndex])
                    ax[index].plot([jndex + 1 + dx, jndex + 1 + dx],
                                   [mean - sem, mean + sem], '-', color=colors[jndex])
    ...
    # Save
    if savefig:
        filename = PSTH_DIR + data + '/summary/' + 'equipment_psth_' + \
            cell_type + '_{}_{}.png'.format(comparison_type, '_'.join(depths))
        print('Figure saved to: ' + filename)
        plt.savefig(filename)
...
def bootstrap_summary_multiple_comparisons():
    ...
    vip_image = get_summary_bootstrap_image_strategy(
        cell_type='vip', second=True, first=False, meso=True)
    p = bootstrap_significance(vip_image, 'visual', 'timing')
    tests['vip_image'] = p

    # Sst, image repeats, comparing strategies
    sst_image = get_summary_bootstrap_image_strategy(
        cell_type='sst', second=False, first=True, meso=True)
    p = bootstrap_significance(sst_image, 'visual', 'timing')
    tests['sst_image'] = p

    # Sst, omissions, comparing strategies
    sst_omission = get_summary_bootstrap_omission_strategy(
        cell_type='sst', first=False, second=True, meso=True)
    p = bootstrap_significance(sst_omission, 'visual', 'timing')
    tests['sst_omission'] = p

    # Vip, omissions, comparing strategies
    vip_omission = get_summary_bootstrap_omission_strategy(
        cell_type='vip', first=False, second=False, meso=True)
    p = bootstrap_significance(vip_omission, 'visual', 'timing')
    tests['vip_omission'] = p

    # Exc, changes, comparing strategies
    exc_hit = get_summary_bootstrap_strategy_hit(
    ...
    sst_hit = get_summary_bootstrap_strategy_hit(
        cell_type='sst', first=False, second=True, image=False, meso=True)
    ...
    vip_hit = get_summary_bootstrap_strategy_pre_change(
        cell_type='vip', first=False, second=True, meso=True)
    ...
    exc_miss = get_summary_bootstrap_strategy_engaged_miss(
        data='events',
        nboots=10000,
        cell_type='exc',
        first=False,
    ...
    exc_post_engagement = get_summary_bootstrap_strategy_engaged_omission(
        data='events',
        nboots=10000,
        cell_type='exc',
        first=False,
    ...
    print(
        tests.query('significant & (not bh_significant)').sort_values(
            by='imq')[cols])
