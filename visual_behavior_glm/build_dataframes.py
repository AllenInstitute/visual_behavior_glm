    ...
    for idx, value in tqdm(enumerate(oeids), total=num_rows):
        ...
        if batch:
            if np.mod(idx, batch_size) == batch_size - 1:
                ...
                path = '/allen/programs/braintv/workgroups/nc-ophys/visual_behavior/ophys_glm/' + df_type + \
                    's/' + data + '/temp_' + experience_level + str(batch_num) + '_' + cre + '.feather'
...
def load_data(oeid, include_invalid_rois=False):
    ...
    session = loading.get_ophys_dataset(
...
def build_behavior_df_experiment(
        session,
        first=False,
        second=False,
        image=False):
    ...
    for data in data_types:
        try:
            ...
            for c in conditions:
                averages = get_full_average(
                    session, averages, full_df, conditions[c])
...
def build_response_df_experiment(
        session,
        data,
        first=False,
        second=False,
    ...
    image_dfs = []
    for index, cell_id in tqdm(enumerate(cell_specimen_ids), total=len(
            cell_specimen_ids), desc='Iterating Cells'):
        try:
    ...
    path = get_path(
        '',
        session.metadata['ophys_experiment_id'],
        'experiment',
        'full_df',
        data,
        first=False,
...
def get_path(
    ...
    if first:
        ...
        extra = ''
    filepath = root + df_type + 's/' + data + '/' + filetype + \
        's/' + str(oeid) + '_' + str(cell_id) + extra + '.h5'
...
def build_response_df_cell(session, cell_specimen_id, data, time=[0.05, 0.8],
    ...
    try:
        ...
        pupil_df = pupil.groupby('stimulus_presentations_id')[
            'pupil_width'].mean()
    except Exception as e:
...
def get_engagement_check(session, condition):
    ...
    if disengaged:
        more_than_threshold_disengaged = engaged_fraction < (
            1 - min_engaged_fraction)
