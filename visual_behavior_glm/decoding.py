...
def decode_experiment(oeid, data='events', window=[0, .75]):
    ...
    reformat.add_licks_each_flash(
        session.stimulus_presentations,
        session.licks)
    reformat.add_rewards_each_flash(
        session.stimulus_presentations,
        session.rewards)
    session.stimulus_presentations['licked'] = [True if len(
        licks) > 0 else False for licks in session.stimulus_presentations.licks.values]
...
def get_cell_table(session, cell_specimen_id, data='events',
    ...
    # Pivot the table such that all responses at the same time point are a
    # column
...
def decode_cells_sample(cells, n_cells, index=None):
    ...
    model['test_score_hit_vs_miss'] = np.mean(
        y == model['cv_prediction_hit_vs_miss'])
...
def plot_behavior_ratio(df, ncells=2, savefig=False, version=None):
    ...
    summary = x.groupby(['n_cells', 'visual_strategy_session'])[
        'behavior_correlation'].mean()
