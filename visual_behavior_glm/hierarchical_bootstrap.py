...
def summary(bootstrap):
    ...
    for key in keys:
        print('{}: {} +/- {}'.format(key,
                                     np.mean(bootstrap[key]),
                                     np.std(bootstrap[key])))
...
def bootstrap(
    ...
    if no_top_level:
        ...
        for g in groups:
            ...
            # Perform each bootstrap
            for i in tqdm(range(0, nboots)):
                sum_val, count = sample_hierarchically(
                    temp, metric, levels[1:])
...
def sample_hierarchically(df, metric, levels):
    ...
    if len(levels) == 1:
        ...
        for i in range(0, n):
            ...
            temp_sum_val, temp_count = sample_hierarchically(
                temp, metric, levels[1:])
...
def plot_data_simple(df, means):
    ...
    mean = df.groupby(['group', 'subject'])[
        'response'].mean().to_frame().reset_index()
    sem = df.groupby(['group', 'subject'])[
        'response'].sem().to_frame().reset_index()
...
def plot_data(df, means):
    ...
    ax[3].plot(df.group + np.random.rand(len(df))
               * .2, df.response, 'ko', alpha=.01)
    ax[3].set_ylabel('response')
    ax[3].set_xlabel('groups')
