...
run_params, results, results_pivoted, weights_df = gfd.get_analysis_dfs(
    GLM_VERSION)
...
results_pivoted_beh = gst.add_behavior_session_metrics(
    results_pivoted, summary_df)
...
gst.plot_kernels_by_strategy_by_session(
    weights_beh,
    run_params,
    ym='omissions',
...
vip_image_filtered = bd.load_population_df(
    'filtered_events', 'image_df', 'Vip-IRES-Cre')
vip_full_filtered = bd.load_population_df(
    'filtered_events', 'full_df', 'Vip-IRES-Cre')
...
# Load image_dfs
vip_omission = psth.load_omission_df(
    summary_df, cre='Vip-IRES-Cre', data='events')
...
psth.compute_running_bootstrap_bin(
    vip_omission,
    'omission',
    'vip',
    bin_num,
...
bootstraps_omission = psth.compute_running_bootstrap(
    vip_omission, 'omission', 'vip', data='events', nboots=nboots)
...
bootstraps_omission = psth.get_running_bootstraps(
    'vip', 'omission', 'events', nboots)
bootstraps_image = psth.get_running_bootstraps(
    'vip', 'image', 'events', nboots)

# Generate figures with bootstraps
psth.running_responses(
    vip_omission,
    'omission',
    bootstraps=bootstraps_omission)
...
boot_image_visual = psth.get_engagement_running_bootstraps(
    'vip', 'image', 'events', nboots, 'visual')
boot_image_timing = psth.get_engagement_running_bootstraps(
    'vip', 'image', 'events', nboots, 'timing')
psth.engagement_running_responses(
    vip_image,
    'image',
    vis_boots=boot_image_visual,
...
sst_full_filtered = bd.load_population_df(
    'filtered_events', 'full_df', 'Sst-IRES-Cre')
exc_full_filtered = bd.load_population_df(
    'filtered_events', 'full_df', 'Slc17a7-IRES2-Cre')
...
ax = psth.plot_condition(
    ...
    labels,
    split_by_engaged=True,
    plot_strategy='timing',
    data='filtered_events')

# Can compare any set of conditions
ax = psth.compare_conditions(
    dfs_filtered, [
        'hit', 'miss'], labels, plot_strategy='visual', data='filtered_events')
...
psth.plot_figure_4_by_equipment(
    sci_dfs,
    mes_dfs,
    'Excitatory',
    counts,
...
psth.plot_equipment_comparison(
    summary_df,
    experiment_table,
    'Vip',
    'omission',
...
psth.running_responses(
    sst_image,
    'image',
    bootstraps=bootstraps_image,
    cre='sst')

sst_omission = psth.load_omission_df(
    summary_df, cre='Sst-IRES-Cre', data='events')
bootstraps_omission = psth.get_running_bootstraps(
    'sst', 'omission', 'events', 10000)
psth.running_responses(
    sst_omission,
    'omission',
    bootstraps=bootstraps_omission,
...
exc_omission = psth.load_omission_df(
    summary_df, cre='Slc17a7-IRES2-Cre', data='events')
bootstraps_omission = psth.get_running_bootstraps(
    'exc', 'omission', 'events', 10000)
psth.running_responses(
    exc_omission,
    'omission',
    bootstraps=bootstraps_omission,
...
pv.scatter_df(
    summary_df,
    'visual_only_dropout_index',
    'timing_only_dropout_index',
...
psth.plot_figure_4_averages_cell_selection(
    dfs, data='events', strategy='strategy_labels_with_none')
psth.plot_figure_4_averages_cell_selection(
    dfs, data='events', strategy='strategy_labels_with_mixed')
