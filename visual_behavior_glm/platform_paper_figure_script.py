...
# Load data
run_params, results, results_pivoted, weights_df = gfd.load_analysis_dfs(
    VERSION)
...
# This generates several figures, Panel B is
# `omissions_heatmap_with_dropout_Familiar.svg`
gvt.kernel_evaluation(
    weights_df,
    run_params,
    'omissions',
...
gvt.kernel_evaluation_by_experience(
    weights_df,
    run_params,
    'all-images',
    limited=False)
gvt.kernel_evaluation_by_experience(
    weights_df, run_params, 'omissions', limited=True)
...
stats_D = gvt.plot_dropout_summary_population_with_experience(
    results, run_params)
results_pivoted_active = results_pivoted.query('not passive').copy()
anova, tukey = gvt.test_significant_across_cell(
    results_pivoted_active, 'all-images')
anova, tukey = gvt.test_significant_across_cell(
    results_pivoted_active, 'omissions')
anova, tukey = gvt.test_significant_across_cell(
    results_pivoted_active, 'behavioral')
...
# cells coding for each regressor, and the CI value (which is the value
# +/- from the mean)
...
stats_S1B = gvt.plot_dropout_individual_population(
    results, run_params, use_single=True)


# S2 - all dropouts by experience
stats_S2 = gvt.plot_population_averages(
    results_pivoted, run_params, dropouts_to_show=[
        'licks', 'running', 'pupil', 'hits', 'misses'])


# S3 - cell selection
# Panel A - Comparing strictly matched cells (last familiar, first novel
# repeat)
...
stats_S3B = gvt.plot_population_averages(
    results_pivoted,
    run_params,
    matched_with_variance_explained=True,
    matched_ve_threshold=0.05)
...
# S5 - Depth V1
stats_S5_V1 = gvt.plot_population_averages_by_depth(
    results_pivoted, run_params, area=['VISp'])


# S5 - Depth LM
stats_S5_LM = gvt.plot_population_averages_by_depth(
    results_pivoted, run_params, area=['VISl'])
...
run_params_b, results_b, results_pivoted_b, weights_df_b = gfd.get_analysis_dfs(
    VERSION_b)
...
stats_S6 = gvt.plot_population_averages(
    results_pivoted_b, run_params_b, dropouts_to_show=[
        'hits', 'post-hits'], extra='_breakdown')


# S7 - misses breakdown
gsm.change_breakdown_schematic(run_params)
stats_S7 = gvt.plot_population_averages(
    results_pivoted_b, run_params_b, dropouts_to_show=[
        'misses', 'post-misses'], extra='_breakdown')


# S8 - task breakdown
gsm.change_breakdown_schematic(run_params)
stats_S8 = gvt.plot_population_averages(
    results_pivoted_b, run_params_b, dropouts_to_show=[
        'task', 'post-task'], extra='_breakdown')


# S9 - omission breakdown
gsm.omission_breakdown_schematic(run_params)
stats_S9 = gvt.plot_population_averages(
    results_pivoted_b, run_params_b, dropouts_to_show=[
        'omissions', 'post-omissions'], extra='_breakdown')
...
gvt.plot_kernel_comparison_by_kernel_excitation(
    weights_df, run_params, 'omissions')

# Panel B - Coding Fraction
# Returns a dataframe with rows for cre/experience, and columns with the fraction of
# cells coding for each regressor, and the CI value (which is the value
# +/- from the mean)
stats_S10B = gvt.plot_fraction_summary_population(
    results_pivoted, run_params, kernel_excitation=True, kernel='omissions')

# Panel C - dropout averages
stats_S10C = gvt.plot_population_averages(
    results_pivoted, run_params, dropouts_to_show=[
        'omissions', 'omissions_positive', 'omissions_negative'])
...
# Returns a pandas dataframe describing the explained variance by
# experience/cre-line
...
across_run_params, across_df_fold_0 = gas.load_cv_across_session(
    run_params, fold=0)
